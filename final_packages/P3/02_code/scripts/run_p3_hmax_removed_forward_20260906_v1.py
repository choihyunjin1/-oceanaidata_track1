"""Frozen hmax information-removal ablation on the v5 forward surface.

No official data, legacy predictions/models, full-fit or answer branch exists.
The completed same-cycle categorical baseline OOF is explicitly hash-authorized.
This reuse is counted separately from forbidden old-model/prediction ancestry.
"""
...
NAME = "p3_hmax_removed_forward_20260906_v1"
...
def access_guard(source, config):
    ...
    allowed.add((ROOT / config["baseline"]["out"] / "baseline_oof.parquet").resolve())
...
def verify_pins(config, source, *, sealed=False):
    ...
    if sealed:
        expected_baseline = json.loads((REPORT / "preflight.json").read_text())["baseline_pins"]
        if baseline_pins(config) != expected_baseline:
            raise ValueError("same-cycle baseline changed after seal")
    return verified


def selected_columns(columns):
    kept = [c for c in columns if not c.startswith("hmax_")]
    if len(columns) != 591 or len(kept) != 527:
        raise ValueError("frozen hmax dependency feature count differs")
    return kept


def baseline_pins(config):
    root = ROOT / config["baseline"]["out"]
    reports = ROOT / config["baseline"]["report"]
    result = json.loads((reports / "result.json").read_text())
    qa = json.loads((reports / "independent-qa.json").read_text())
    replay = json.loads((reports / "fresh-process-replay.json").read_text())
    if qa["status"] != "PASS" or qa["failed_checks"] or replay["status"] != "PASS":
        raise ValueError("same-cycle baseline must have completed independent QA/replay")
    digest = sha(reports / "result.json")
    if qa["result_sha256"] != digest or replay["result_sha256"] != digest:
        raise ValueError("baseline QA/replay result linkage mismatch")
    if (
        result["config_sha256"] != config["inputs"][config["baseline"]["config"]]
        or result["runner_sha256"] != config["inputs"][config["baseline"]["runner"]]
    ):
        raise ValueError("baseline fixed source/config differs")
    if (
        result["fit_count"] != 15
        or result["router_fit_count"] != 8
        or result["source_sha256"] != config["source_files"]
    ):
        raise ValueError("baseline incomplete or source mismatch")
    if sha(root / "baseline_oof.parquet") != result["artifacts"]["baseline_oof.parquet"]:
        raise ValueError("baseline OOF hash mismatch")
    paths = [
        root / "baseline_oof.parquet",
        root / "seal.json",
        reports / "result.json",
        reports / "preflight.json",
        reports / "independent-qa.json",
        reports / "fresh-process-replay.json",
    ]
...
def preflight(config, source):
    ...
    same_cycle_baseline = baseline_pins(config)
    kept = selected_columns(columns)
    example_context = raw_context(station_grids[example.station], example)
    first = summarize_context(example_context)
    for changed_value in (999.0, np.nan):
        changed_context = example_context.copy()
        changed_context["hmax"] = changed_value
        altered = summarize_context(changed_context)
        if not np.allclose(
            [first[c] for c in kept], [altered[c] for c in kept], rtol=0, atol=0, equal_nan=True
        ):
            raise ValueError("hmax influences a retained feature")
    ...
    receipt = {
        ...
        "candidate_features": len(kept),
        "removed_hmax_features": [c for c in columns if c not in kept],
        "router_removed_features": ["hmax_current"],
        "hmax_raw_perturbation_retained_features_exact": True,
        "baseline_pins": same_cycle_baseline,
...
def router_material(frame, features):
    ...
    observed_x = observed_x.drop(columns=["hmax_current"])
...
def execute(config, source):
    ...
    baseline = pd.read_parquet(ROOT / config["baseline"]["out"] / "baseline_oof.parquet")
    contract = cv.load_contract(ROOT / config["contract"])
    recipe = json.loads((ROOT / config["recipe"]).read_text())
    ...
    def progress(stage):
        ...
        if time.perf_counter() >= deadline:
            raise TimeoutError("60-minute fixed hmax execution cap reached")

    for number, (fold, split) in enumerate(masks):
        progress(f"HMAX_REMOVED_{fold['id']}_SINGLE")
        ...
        fits.append(receipt)
        progress(f"HMAX_REMOVED_{fold['id']}_MULTI")
        multi, receipt = fit_one(
            "multi", fold, number, split, features, anchors, kept, recipe, config, deadline
        )
        fits.append(receipt)
        parts.append(component_frame(anchors, split["validation"], fold["id"], single, multi))
        if number == 0:
            ratio = sum(config["expected_train"]) / config["expected_train"][0]
            estimate = (
                config["budget"]["pilot_estimate_margin"]
                * ratio
                * (fits[0]["seconds"] + fits[1]["seconds"])
            )
            pilot = {
                "status": "CONTINUE"
            ...
            if estimate >= config["budget"]["wall_seconds"]:
                write_json(OUT / "fit-receipts.json", {"fits": fits}, progress=True)
                raise TimeoutError(
                    "hmax first planned fits forecast exceeds60min; no outcome-driven restart"
                )
    progress("CANDIDATE_COMPLETE_FIXED_ROUTER")
    candidate, candidate_router = fixed_policy(
        pd.concat(parts, ignore_index=True), features, anchors, contract, arm="candidate", fit=True
    )
    if not baseline[KEYS].equals(candidate[KEYS]) or not np.array_equal(
        baseline.target_hs, candidate.target_hs
    ):
        raise ValueError("paired baseline/candidate keys and target mismatch")
    ...
    final["control"], final["candidate"] = baseline.final_prediction, candidate.final_prediction
    wind_columns = [
        c
        for c in kept
    ...
    result = {
        ...
        else "NO_OP_HMAX_REMOVAL_NOT_IMPROVED",
        "pid": os.getpid(),
        "comparison": stats,
        "fit_count": 10,
        "router_fit_count": 4,
        "reused_baseline_backbone_fits": 10,
        "reused_baseline_router_fits": 4,
        "fit_receipts": fits,
        "router_receipts": {"candidate": candidate_router},
        ...
        "baseline_pins": pre["baseline_pins"],
        "candidate_features": kept,
        "router_removed_features": ["hmax_current"],
        "numeric_lead_used": False,
        "artifacts": {
            name: sha(OUT / name)
            for name in (
                "anchors.parquet",
        ...
        "authorized_same_cycle_baseline_prediction_rows": len(baseline),
        **zero_access(),
    }
...
def replay(config, source):
    ...
    for fold, split in fold_masks(anchors, contract):
        ...
        mx, _, current = multi_matrix(feature, anchors, ids, kept)
        x, _, meta = expand_leads(feature, anchors, ids, kept)
        values = {}
        for kind in ("multi", "categorical_single"):
            ...
            if any(name.startswith("hmax_") for name in model.feature_names_):
                raise ValueError("hmax feature remained in native model")
            if kind == "multi":
                values[kind] = np.clip(current[:, None] + model.predict(mx, thread_count=2), 0, 30)
            else:
                prediction = np.clip(
                    meta.current_hs.to_numpy() + model.predict(matrix(x, False), thread_count=2),
                    0,
                    30,
                )
                ordered = (
    ...
    predicted, _ = fixed_policy(
        ...
        arm="candidate",
        fit=False,
        expected_receipts=result["router_receipts"]["candidate"],
    )
    stored = pd.read_parquet(OUT / "candidate_oof.parquet")
