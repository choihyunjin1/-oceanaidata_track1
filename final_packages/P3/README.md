...
This extraction is the numeric candidate only. `TRAIN.ipynb` runs prepare →
numeric training → internal replay QA, each in a new process. `PREDICT.ipynb`
runs inference → exact answer replay. Select the documented Python environment
and set `P3_DATA_DIR` before opening the notebooks. The notebooks store launcher
logs in `06_docs/notebook_launches`, outside the initially empty training workspace.
This code-only package has not yet completed a cold run. Do not infer a successful
training or official score from its existence. Keep the entire folder structure.
