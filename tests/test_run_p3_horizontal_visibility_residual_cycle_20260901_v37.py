...
RUNNER = ROOT / "scripts/run_p3_horizontal_visibility_residual_cycle_20260901_v37.py"
SPEC = importlib.util.spec_from_file_location("p3_v37", RUNNER)
...
def test_feature_shape_finite_and_deterministic() -> None:
    ...
    first = MODULE.horizontal_visibility_features(sequence)
    second = MODULE.horizontal_visibility_features(sequence)
    assert first.shape == (64,)
    assert np.array_equal(first, second)
    assert np.isfinite(first).all()


def test_order_obstruction_monotone_invariance() -> None:
    values = np.asarray([0.2, 1.0, 0.1, 0.8, 0.3, 1.2, 0.4])
    assert np.array_equal(
        MODULE.horizontal_visibility_adjacency(values),
        MODULE.horizontal_visibility_adjacency(np.exp(values)),
    )


def test_graph_has_adjacent_edges_and_asymmetry_signal() -> None:
    receipt = MODULE.graph_preflight_receipt()
    assert receipt["edge_count"] >= receipt["node_count"] - 1
    assert receipt["mean_absolute_left_right_asymmetry"] > 0.0
    assert receipt["reverse_mean_absolute_left_right_asymmetry"] > 0.0
...
def test_future_perturbation_invariance() -> None:
    ...
    second[289:] = 1e9
    assert np.array_equal(
        MODULE.horizontal_visibility_features(first[:289]),
        MODULE.horizontal_visibility_features(second[:289]),
    )
...
def test_sealed_nonduplicate_contract() -> None:
    ...
    assert config["encoder"]["fixed_windows_rows"] == [145, 73]
...
def test_fold_purge_and_exact_fit_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        _, receipts = MODULE.v36.v32.v28.crossfit(
            cases, np.zeros((len(cases), 64)), targets, reference
        )
    finally:
...
def test_preflight_or_consumed_namespace() -> None:
    if MODULE.ARTIFACT.exists() or MODULE.LOCK.exists():
        ...
        assert value["synthetic"]["feature_count"] == 64
