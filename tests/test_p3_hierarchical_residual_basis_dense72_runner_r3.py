...
RUNNER = ROOT / "scripts/run_p3_hierarchical_residual_basis_dense72_r3.py"
ENGINE = ROOT / "src/p3_wave/hierarchical_residual_basis_dense72_execution_r3.py"
...
def test_full_predecessor_transitive_dependency_closure_is_live_pinned() -> None:
    ...
    expected = {
        f"R2_TRANSITIVE_{role}" for role in predecessor["implementation_roles"]
    }
    assert expected.issubset(pins)
    assert len(expected) == len(predecessor["implementation_roles"])
    for role, relative in predecessor["implementation_roles"].items():
        pin = pins[f"R2_TRANSITIVE_{role}"]
        assert pin["path"] == relative
        assert guard.sha256_file(ROOT / relative) == pin["sha256"]
...
def test_operational_snapshot_deep_binds_in_memory_comparator_object(
    ...
    metrics["__r3_test_poison"] = True
    try:
        assert guard.operational_snapshot(preflight) != preflight[
            "operational_snapshot"
        ]
    finally:
        metrics.pop("__r3_test_poison")
