...
RUNNER = ROOT / "scripts/run_p3_realized_bipower_jump_residual_cycle_20260901_v53.py"
SPEC = importlib.util.spec_from_file_location("p3_v53", RUNNER)
...
def test_feature_shape_finite_and_deterministic() -> None:
    ...
    first = MODULE.realized_bipower_features(sequence)
    second = MODULE.realized_bipower_features(sequence)
    assert first.shape == (72,)
    assert np.array_equal(first, second)
    assert np.isfinite(first).all()


def test_smooth_spike_and_constant_guards() -> None:
    receipt = MODULE.synthetic_receipt()
    assert receipt["single_spike_jump_share"] > receipt["smooth_jump_share"] + 0.15
...
def test_positive_affine_invariance() -> None:
    ...
    assert np.allclose(MODULE.realized_bipower_statistics(values, 3), MODULE.realized_bipower_statistics(4.0 * values + 8.0, 3), atol=1e-12, rtol=1e-12)
...
def test_future_perturbation_invariance() -> None:
    ...
    second = first.copy()
    second[289:] = 1e9
    assert np.array_equal(MODULE.realized_bipower_features(first[:289]), MODULE.realized_bipower_features(second[:289]))


def test_sealed_nonduplicate_contract() -> None:
    config = MODULE.load_config()
    encoder = config["encoder"]
    assert config["duplication_audit"]["semantic_verdict"] == "NON_DUPLICATE_P3_REALIZED_BIPOWER_JUMP_AXIS"
    assert tuple(encoder["lags_rows"]) == (1, 3, 6)
    assert encoder["epsilon"] == 1e-12
...
def test_fold_purge_and_exact_fit_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    base = MODULE.v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.v38.v36.v32.v28
    cases, targets, reference, _ = MODULE.v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.v38.v36.v32.v23.case_surface()
    calls: list[str] = []
