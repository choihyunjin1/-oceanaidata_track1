...
RUNNER = ROOT / "scripts/run_p3_variational_mode_residual_cycle_20260901_v52.py"
SPEC = importlib.util.spec_from_file_location("p3_v52", RUNNER)
...
def test_feature_shape_finite_and_deterministic() -> None:
    ...
    first = MODULE.variational_mode_features(sequence)
    second = MODULE.variational_mode_features(sequence)
    assert first.shape == (72,)
    assert np.array_equal(first, second)
    assert np.isfinite(first).all()


def test_two_tone_frequency_recovery_and_reconstruction() -> None:
    receipt = MODULE.two_tone_receipt()
    assert receipt["finite"]
    assert max(receipt["target_frequency_distances"]) <= 0.035
    assert receipt["reconstruction_max_abs_error"] < 1e-12


def test_positive_affine_invariance() -> None:
    rng = np.random.default_rng(520)
    values = rng.normal(size=145)
    assert np.allclose(MODULE.mode_features(values), MODULE.mode_features(3.0 * values + 7.0), atol=1e-10, rtol=1e-10)
...
def test_future_perturbation_invariance() -> None:
    ...
    second = first.copy()
    second[289:] = 1e9
    assert np.array_equal(MODULE.variational_mode_features(first[:289]), MODULE.variational_mode_features(second[:289]))


def test_sealed_nonduplicate_contract() -> None:
    config = MODULE.load_config()
    encoder = config["encoder"]
    assert config["duplication_audit"]["semantic_verdict"] == "NON_DUPLICATE_P3_JOINT_VARIATIONAL_NARROWBAND_AXIS"
    assert encoder["mode_count"] == 3
    assert encoder["bandwidth_penalty"] == 800.0
    assert encoder["iterations"] == 40
...
def test_fold_purge_and_exact_fit_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    base = MODULE.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.v38.v36.v32.v28
