...
RUNNER = ROOT / "scripts/run_p3_level_crossing_excursion_residual_cycle_20260901_v40.py"
SPEC = importlib.util.spec_from_file_location("p3_v40", RUNNER)
...
def test_feature_shape_finite_and_deterministic() -> None:
    ...
    first = MODULE.level_crossing_features(sequence)
    second = MODULE.level_crossing_features(sequence)
    assert first.shape == (72,)
    assert np.array_equal(first, second)
    assert np.isfinite(first).all()


def test_synthetic_directional_crossings() -> None:
    receipt = MODULE.directional_receipt()
    assert receipt["monotone_increasing"][0] > 0
    assert receipt["monotone_increasing"][1] == 0
    assert receipt["monotone_decreasing"][1] > 0
    assert receipt["monotone_decreasing"][0] == 0
    assert receipt["periodic"][0] > 0 and receipt["periodic"][1] > 0


def test_positive_affine_feature_invariance() -> None:
    sequence = sample_sequence()
    transformed = sequence.copy()
    for channel in MODULE.CHANNELS:
        transformed[:, channel] = 3.0 * transformed[:, channel] + 5.0
    assert np.allclose(
        MODULE.level_crossing_features(sequence),
        MODULE.level_crossing_features(transformed),
        atol=1e-12,
    )
...
def test_future_perturbation_invariance() -> None:
    ...
    assert np.array_equal(
        MODULE.level_crossing_features(first[:289]),
        MODULE.level_crossing_features(second[:289]),
    )
...
def test_sealed_nonduplicate_contract() -> None:
    ...
    assert config["encoder"]["levels_iqr_units"] == [-1.0, 0.0, 1.0]
...
def test_fold_purge_and_exact_fit_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    cases, targets, reference, _ = MODULE.v39.v38.v36.v32.v23.case_surface()
    calls: list[str] = []
    ...
    monkeypatch.setattr(MODULE.v39.v38.v36.v32.v26, "fit_predict", fake_fit_predict)
    original = MODULE.v39.v38.v36.v32.v28.SPECS
    MODULE.v39.v38.v36.v32.v28.SPECS = MODULE.SPECS
    try:
        _, receipts = MODULE.v39.v38.v36.v32.v28.crossfit(cases, np.zeros((len(cases), 72)), targets, reference)
    finally:
        MODULE.v39.v38.v36.v32.v28.SPECS = original
