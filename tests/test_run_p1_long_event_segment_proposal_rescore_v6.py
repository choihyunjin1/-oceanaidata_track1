...
RUNNER_PATH = ROOT / "scripts" / "run_p1_long_event_segment_proposal_rescore_v6.py"
MODULE_PATH = ROOT / "src" / "p1_qc" / "long_event_segment_proposal_rescore_execution_v6.py"
DESIGN_PATH = (
...
def runner():
    return _load(RUNNER_PATH, "p1_segment_rescore_v6_runner_test")


@pytest.fixture(scope="module")
def numerical():
    return _load(MODULE_PATH, "p1_qc.long_event_segment_proposal_rescore_execution_v6_test")
...
def test_full_oos_support_surface_uses_estimator_without_sentinel_fill(
    ...
    journal = Journal()
    historical = train.loc[:, ["station", "year", "layer", "time", "label", "anomaly_type"]].copy()
    historical.insert(0, "row_position", np.arange(len(historical), dtype=np.int64))
    result = numerical._fit_anchor_surfaces(
        {
            "train_target_free": train.drop(columns=["label", "anomaly_type"]),
...
def test_authorization_template_matches_exact_runner_schema(runner) -> None:
    ...
    assert template["schema_version"] == runner.AUTHORIZATION_SCHEMA_VERSION
    assert template["experiment_id"] == runner.EXPERIMENT_ID
    assert template["runner_sha256"] == template["runner_normalized_sha256"] == _sha(
        RUNNER_PATH
    )
    ...
    assert set(template["preexecution_seal"]) == {"path", "bytes", "sha256"}
...
def test_external_digest_authorization_chain_is_acyclic_without_runner_mutation(
    ...
    monkeypatch.setattr(runner, "R6_QA_PATH", qa_path)
    ...
    seal = {
        ...
        "bootstrap_target_firewall_v6_sha256": runner.BOOTSTRAP_TARGET_FIREWALL_V6_SHA256,
        "r5_no_go_qa_sha256": runner.R5_QA_NO_GO_SHA256,
...
def test_exact_snapshot_inventory_rejects_self_declared_legacy_substitution_preimport(
    ...
    root.mkdir()
    sources = {
        runner._relative_literal(RUNNER_PATH): RUNNER_PATH,
...
def test_isolated_snapshot_bootstrap_ignores_malicious_pythonpath_sitecustomize(
    runner, tmp_path: Path
) -> None:
    snapshot = tmp_path / "snapshot"
    bootstrap = snapshot / "scripts" / RUNNER_PATH.name
    bootstrap.parent.mkdir(parents=True)
    bootstrap.write_bytes(RUNNER_PATH.read_bytes())
    manifest = snapshot / "snapshot_manifest.json"
    manifest.write_text("{}\n", encoding="utf-8")
    malicious = tmp_path / "malicious"
    malicious.mkdir()
    marker = tmp_path / "startup-hook-executed.txt"
    ...
    environment[runner.AUTHORIZATION_ENV_VAR] = "a" * 64
...
def test_worker_command_uses_snapshot_bootstrap_after_live_runner_swap(
    runner, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    live_root = tmp_path / "live"
    snapshot_root = tmp_path / "snapshot"
    live_runner = live_root / "scripts" / RUNNER_PATH.name
    snapshot_runner = snapshot_root / "scripts" / RUNNER_PATH.name
    live_runner.parent.mkdir(parents=True)
    snapshot_runner.parent.mkdir(parents=True)
    live_runner.write_bytes(RUNNER_PATH.read_bytes())
    snapshot_runner.write_bytes(RUNNER_PATH.read_bytes())
    manifest = snapshot_root / "snapshot_manifest.json"
    manifest.write_text("{}\n", encoding="utf-8")
    monkeypatch.setattr(runner, "PROJECT_ROOT", live_root)
    monkeypatch.setattr(runner, "__file__", str(live_runner))
    ...
    marker = tmp_path / "live-substitution-executed.txt"
    live_runner.write_text(
        f"from pathlib import Path\nPath({str(marker)!r}).write_text('BAD')\n",
        encoding="utf-8",
    )
    assert command[1:4] == ["-I", "-S", "-B"]
    assert Path(command[4]).resolve() == snapshot_runner.resolve()
    environment = dict(os.environ)
    environment[runner.AUTHORIZATION_ENV_VAR] = "a" * 64
    completed = subprocess.run(
        command,
...
def test_parent_held_authority_denies_source_mutation_until_release(
    runner, tmp_path: Path
) -> None:
    path = tmp_path / "authority.json"
    path.write_bytes(b'{"sealed":true}\n')
    expected = _sha(path)
    with runner.HeldReadDenyMutation([path]) as guard:
        guard.assert_digest(path, expected)
        if os.name == "nt":
...
def test_protected_outer_target_suffix_remains_opaque_before_freeze(runner) -> None:
    header = b"station,year,layer,time,temp,psal,depth,label,anomaly_type\n"
    historical = b"I-ORS,2025,1,2025-03-01 23:50:00+09:00,10,30,10,1,historical\n"
    # Invalid UTF-8 in protected target bytes makes any accidental conversion
    # observable while leaving the seven target-free fields valid.
    protected = b"I-ORS,2025,1,2025-04-01 00:00:00+09:00,11,31,10,\xff,\xfe\n"
    decoded_ordinals: list[int] = []

    def decoder(suffix: bytes, ordinal: int) -> tuple[int, str]:
        decoded_ordinals.append(ordinal)
        assert ordinal == 0
        return runner._decode_historical_target_suffix(suffix, ordinal)

    target_free, bounded, receipt = runner._target_free_train_and_bounded_historical_targets(
        header + historical + protected,
        SimpleNamespace(pd=pd, np=np),
        historical_suffix_decoder=decoder,
    )
    assert list(target_free.columns) == list(runner.TARGET_FREE_TRAIN_COLUMNS)
    assert not {"label", "anomaly_type"}.intersection(target_free.columns)
    assert len(bounded) == 1 and decoded_ordinals == [0]
    assert receipt["pre_freeze_protected_target_rows_decoded"] == 0
    assert receipt["protected_outer_rows_left_opaque"] == 1


@pytest.mark.parametrize(
    ("mutation", "message"),
    [
        (lambda value: value.__setitem__("schema_version", "wrong"), "schema version"),
        (
            lambda value: value["preexecution_seal"].__setitem__("unknown", 1),
            "seal schema membership",
        ),
        (
            lambda value: value["independent_qa"].pop("verdict"),
            "QA schema membership",
        ),
        (
            lambda value: value["preexecution_seal"].__setitem__("bytes", True),
            "seal primitive type",
        ),
        (lambda value: value.__setitem__("unknown_capability", True), "schema membership"),
    ],
)
def test_malformed_authorization_fails_before_static_read_import_or_claim(
    runner,
    tmp_path: Path,
    ...
    value = {
        ...
        "readonly_preflight_verification_sha256": "e" * 64,
        "zero_prior_state": {
            "claims": 0,
            "physical_fits": 0,
            "scientific_materializations": 0,
    ...
    monkeypatch.setattr(runner, "AUTHORIZATION_PATH", auth_path)
    monkeypatch.setenv(runner.AUTHORIZATION_ENV_VAR, hashlib.sha256(raw).hexdigest())
    original_read = runner._read_bound_bytes
    reads: list[str] = []

    def authorization_only(path: Path, *args, **kwargs):
        reads.append(Path(path).name)
        if len(reads) > 1:
            raise AssertionError("schema failure occurred after a repository/static read")
        return original_read(path, *args, **kwargs)

    monkeypatch.setattr(runner, "_read_bound_bytes", authorization_only)
    monkeypatch.setattr(
        runner,
        "_static_package_checks",
        lambda *_a, **_k: (_ for _ in ()).throw(AssertionError("static read")),
    )
    monkeypatch.setattr(
        runner.AttemptJournal,
        "begin",
        lambda *_a, **_k: (_ for _ in ()).throw(AssertionError("claim")),
    )
    with pytest.raises(runner.AuthorizationError, match=message):
        runner._require_execution_authorization_preimport()
    assert reads == [auth_path.name]
...
def test_outer_truth_is_first_accessed_only_after_prediction_freeze(
    ...
    monkeypatch.setattr(
        numerical,
        "_parse_full_train_targets_after_freeze",
        lambda state: (
            state["train_source_opaque_bytes"],
            pd.DataFrame({"label": [0], "anomaly_type": ["none"]}),
        )[1],
    )
    monkeypatch.setattr(
        numerical,
        "_left_censor_audit_after_freeze",
        lambda *_a, **_k: {fold: 0 for fold in numerical.FOLD_ORDER},
    )
    monkeypatch.setattr(
        numerical,
        "_score_outer",
    ...
    result = numerical.run_authorized_screen(
        GuardedState(
            ...
            train_source_opaque_bytes=b"held-train-once",
    ...
    assert marker == {"frozen": True, "truth_reads": 2}
