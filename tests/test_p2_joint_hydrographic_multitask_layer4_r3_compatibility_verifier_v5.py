...
BOOTSTRAP = (
    ROOT / "scripts" / "bootstrap_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5.py"
)
LAUNCHER = (
    ROOT / "scripts" / "launch_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5.ps1"
)
STAGE_ZERO = (
    ROOT
    / "scripts"
    / "launch_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5_stage0.ps1"
)
EXTERNAL_QA_HOLDER = (
    ROOT
    / "scripts"
    / "invoke_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5_external_qa.py"
)
PYCACHE_SENTINEL = (
    ROOT
    / "configs"
...
HELPER = (
    ...
    / "src"
    / "p2_restore"
    / "joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5.py"
)
CLI = ROOT / "scripts" / "verify_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5.py"
V3_CONTROL = (
    ROOT
    / "artifacts"
...
V5_CONTROL = (
    ROOT
    / "artifacts"
    / "p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5_control"
)
...
def _canonical_command(root: Path = ROOT) -> list[str]:
    ...
    return [
        ...
        "-S",
        "-B",
        str(root / EXTERNAL_QA_HOLDER.relative_to(ROOT)),
        "--root",
        str(root),
        "--host",
...
def canonical_envelope() -> dict[str, Any]:
    before = {
        ...
        "v4": _tree_snapshot(V4_CONTROL),
        "sentinel": _pin(PYCACHE_SENTINEL),
        "v5_exists": V5_CONTROL.exists(),
    }
...
def test_v4_no_go_records_exact_three_findings_and_no_receipt() -> None:
    owner_path = V4_CONTROL / "OWNER_STATIC_QA_NO_GO_20260823.json"
    tombstone_path = V4_CONTROL / "EXECUTION_TOMBSTONE.json"
    assert _pin(owner_path) == (
        7401,
        "a6d9ce6cbdd329fb9f8cfe41ac4b0ca447c3e50737605e444bc356277fd058f8",
    )
    assert _pin(tombstone_path) == (
        1500,
        "4e02b9f73db9e2345b2d155c3da3c8b7b5b7e4c3dc7b891dbb9de2e6afeac070",
    )
    owner = json.loads(owner_path.read_text("utf-8"))
    tombstone = json.loads(tombstone_path.read_text("utf-8"))
    assert owner["verdict"] == "P0=0_P1=3_NO_GO"
    assert owner["independent_qa_review_performed"] is True
    assert owner["independent_qa_receipt_created"] is False
    assert [item["code"] for item in owner["findings"]] == [
        "PREHOOK_PYC_NONEXISTENCE_NOT_IMMUTABLY_BOUND",
        "ZERO_ACTION_DIRECT_WINAPI_CAPABILITY_REMAINS",
        "EXTERNAL_LAUNCH_TRUST_ROOT_NOT_CLOSED",
    ]
    assert tombstone["compatibility_must_fail_closed"] is True
    assert tombstone["execution_prohibited"] is True
    assert tombstone["compatibility_receipt_created"] is False


def test_v5_config_declares_noncyclic_external_startup_root() -> None:
    ...
    assert len(runtime["external_startup_file_pins"]) == 12
    assert runtime["inline_stage_zero_path_execution_allowed"] is False
    assert runtime["stage_zero_holds_stage_one_until_child_exit"] is True
    assert runtime["powershell_runtime_inventory"]["files"] == 983
    assert runtime["powershell_runtime_inventory"]["directories"] == 53
    assert config["canonical_pycache_sentinel"]["path"] == PYCACHE_SENTINEL.relative_to(
        ROOT
    ).as_posix()
...
def test_canonical_launcher_contains_required_native_handle_contract() -> None:
    ...
    stage_zero = STAGE_ZERO.read_text(encoding="utf-8")
    for token in (
        "AssertExternallyHeld",
        "P2_V5_PATH_IMMUTABLE_INLINE_STAGE_ZERO_V1",
        "ScriptBlock]::Create",
        "stageOneExpectedSha256",
        "SHA256_SORTED_ORDINAL_TYPE_NUL_RELATIVE_NUL_BYTES_NUL_FILE_SHA256_LF",
    ):
        assert token in stage_zero
    assert "-File" not in stage_zero


def test_direct_bootstrap_without_external_attestation_fails() -> None:
    prefix = PYCACHE_SENTINEL
    result = _run(
        [
            str(PYTHON),
            "-I",
    ...
    assert _pin(prefix) == (67, "932aa01b4026ed06cc675d1312fdf5f29f2247327319c3eaceec24fe7991f903")
...
def test_regular_file_pycache_sentinel_executes_four_sources_not_pyc() -> None:
    prefix = PYCACHE_SENTINEL
    result = _run(
        [
            str(PYTHON),
...
def _load_hook() -> tuple[Any, dict[str, Any]]:
    ...
    namespace: dict[str, Any] = {
        "_CAPABILITY_STATE": {"sealed": False},
...
def _load_helper_for_unit_contract() -> Any:
    module = types.ModuleType("p2_v5_unit_helper")
    module.__file__ = str(HELPER)
    module.__package__ = "p2_restore"
    module.__dict__["_P2_V5_BOOTSTRAP_CONTEXT"] = {}
...
def test_corrected_fold_predicate_rejects_semantic_forgery(forgery: str) -> None:
    ...
    with pytest.raises(helper.CompatibilityVerifierV5Error):
...
def test_external_launcher_envelope_is_noncyclic_and_stable(
    ...
    assert trust["startup_files"] == 12
    assert trust["startup_files_prehashed"] == 12
    assert trust["startup_files_post_child_rehashed"] == 12
    ...
    assert trust["canonical_pycache_sentinel_regular_held_before"] is True
    assert trust["canonical_pycache_sentinel_regular_held_after"] is True
    assert trust["stage_zero_encoded_command"] is True
    assert trust["stage_zero_path_execution"] is False
    assert trust["stage_zero_observed_pin"] == {
        "path": STAGE_ZERO.relative_to(ROOT).as_posix(),
        "bytes": _pin(STAGE_ZERO)[0],
        "sha256": _pin(STAGE_ZERO)[1],
    }
    assert trust["stage_one_authenticated_before_evaluation"] is True
    assert trust["stage_one_evaluated_from_memory"] is True
    assert trust["powershell_runtime_inventory"]["files"] == 983
    assert trust["powershell_runtime_inventory"]["directories"] == 53
    assert trust["powershell_runtime_inventory"][
        "externally_share_deny_held_before_host_start"
    ] is True
...
def test_canonical_report_closes_dependency_and_stable_byte_boundaries(
    ...
    capability = bootstrap["native_capability_surface"]
    assert capability["sealed"] is True
    assert capability["blocked_calls_total"] == 7
    assert capability["real_self_test_probe_count"] == 12
    assert dependency["meta_path"] == []
    assert capability["real_self_test_effect_count"] == 0
    assert capability["retained_main_native_callables"] == []
    assert capability["wrapped_dangerous_callable_count"] > 100
    assert all(
        value == 0
        for key, value in capability["operational_firewall"].items()
...
def test_canonical_report_has_zero_action_state(canonical_report: dict[str, Any]) -> None:
    ...
    assert canonical_report["v5_control_exists"] is False
...
def test_no_absolute_personal_path_is_persisted_in_v5_sources() -> None:
