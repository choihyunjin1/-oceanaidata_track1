...
SCRIPT_PATH = (
    PROJECT_ROOT / "scripts" / "run_p1_round_b_nonspike_long_event_residual_v1r4.py"
)
...
def _runner():
    name = f"p1_round_b_residual_v1r4_test_{uuid.uuid4().hex}"
...
def test_numerical_contract_is_exact_v1r3_and_still_nine_fits() -> None:
    runner = _runner()
    current = _json(CONFIG_PATH)
    prior = _json(OLD_CONFIG_PATH)
    assert current["experiment_id"].endswith("v1r4")
...
def test_import_creates_no_execution_claim_or_fit() -> None:
    ...
    assert not (artifact / "execution.lock").exists()
    assert not (artifact / "attempt_journal").exists()
    assert os.environ.get("P1_V1R4_NUMERICAL_EXECUTED") is None
