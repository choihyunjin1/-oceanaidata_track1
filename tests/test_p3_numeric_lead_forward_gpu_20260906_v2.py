...
SPEC = importlib.util.spec_from_file_location(
    "numeric_lead",
    Path(__file__).resolve().parents[1] / "scripts/run_p3_numeric_lead_forward_gpu_20260906_v2.py",
)
...
def synthetic_matrix():
    ...
    frame = pd.DataFrame(
        {
            "station": ["A", "B"] * 36,
            "lead_h": np.tile(mod.LEADS, 12),
            "current_hs_for_residual": 2.0,
...
def test_mixed_device_native_compatibility_save_load(tmp_path, kind):
    ...
    assert params["task_type"] == ("GPU" if is_multi else "CPU") and params["thread_count"] == 2
    assert (params.get("devices") == "0") if is_multi else "devices" not in params
    model = CatBoostRegressor(**params)
    kwargs = {} if is_multi else {"callbacks": [mod.Deadline(time.perf_counter() + 60)]}
    model.fit(x, y, cat_features=cats, sample_weight=np.ones(len(x)), **kwargs)
    assert model.tree_count_ == 3
    path = tmp_path / f"{kind}.cbm"
    model.save_model(path)
    restored = CatBoostRegressor().load_model(path)
    np.testing.assert_array_equal(
        model.predict(x, thread_count=2), restored.predict(x, thread_count=2)
    )
...
def test_purge_prior_oof_even_if_from_previous_fold():
    ...
    anchors = pd.DataFrame(
        {"anchor_id": [0, 1, 2], "station": "A", "anchor_time": times, "episode_id": [1, 2, 3]}
    )
    meta = anchors.loc[[0, 1]].assign(fold="Q2_2024", lead_h=24)
    np.testing.assert_array_equal(
        mod.safe_meta_mask(meta, anchors, "Q3_2024", contract), [True, False]
    )
...
def test_budget_and_no_official_modes():
    ...
    assert cfg["budget"]["gpu"] == 1 and cfg["budget"]["wall_seconds"] == 5400
