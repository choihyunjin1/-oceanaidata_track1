...
from p2_restore import public_trajectory_dtw_v2r4 as dtw

REPO_ROOT = Path(__file__).resolve().parents[1]
RUNNER_PATH = REPO_ROOT / "scripts/run_p2_public_trajectory_dtw_curvature_transfer_v2r4.py"
...
def _synthetic_verified_snapshot(runner, *, seal_bytes: bytes = b'{"seal":"A"}\n'):
    ...
    execution_bytes = (
        REPO_ROOT
        / "configs/experiments/p2_public_trajectory_dtw_curvature_transfer_v2r4_execution.json"
    ).read_bytes()
    module_bytes = (REPO_ROOT / "src/p2_restore/public_trajectory_dtw_v2r4.py").read_bytes()
    bundle_bytes = {
...
def test_design_hash_is_immutable_and_not_authorized() -> None:
    ...
    assert repair["post_verification_sha256_seal_path_calls"] == 0
    assert repair["post_verification_seal_path_reopens"] == 0
    assert repair["post_verification_module_path_reopens"] == 0


def test_v2r4_numerical_module_diff_is_docstring_version_only() -> None:
    v2r3 = (REPO_ROOT / "src/p2_restore/public_trajectory_dtw_v2r3.py").read_text(
        encoding="utf-8"
    )
    v2r4 = (REPO_ROOT / "src/p2_restore/public_trajectory_dtw_v2r4.py").read_text(
        encoding="utf-8"
    )
    assert v2r4.count("v2r4") == 1
    assert v2r4.replace("v2r4", "v2r3", 1).rstrip() == v2r3.rstrip()
...
def test_v2r3_qa_lineage_is_actual_no_go_and_fail_closed() -> None:
    report_path = (
        REPO_ROOT
        / "reports/p2_public_trajectory_dtw_curvature_transfer_v2r3_independent_preexecution_qa_20260826.json"
    )
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert _sha256(report_path) == "4adf0778a69817fd01d758e34852e8194522dc3459a0cce35136c1f3487c2c4d"
    assert report["verdict"] == "NO_GO"
    assert report["severity_counts"] == {"P0": 0, "P1": 1, "P2": 0}
...
def test_no_personal_absolute_observations_path_in_v2r4_static_sources() -> None:
    relatives = [
        "configs/experiments/p2_public_trajectory_dtw_curvature_transfer_v2r4_design.json",
        "configs/experiments/p2_public_trajectory_dtw_curvature_transfer_v2r4_execution.json",
        "scripts/run_p2_public_trajectory_dtw_curvature_transfer_v2r4.py",
    ]
...
def test_runner_and_numerical_module_have_no_fit_call() -> None:
    source = RUNNER_PATH.read_text(encoding="utf-8") + (REPO_ROOT / "src/p2_restore/public_trajectory_dtw_v2r4.py").read_text(encoding="utf-8")
...
def test_authorization_verifies_real_qa_receipt_and_snapshot_lineage(
    ...
    qa_path = tmp_path / "reports/p2_v2r4_independent_qa.json"
    ...
    authorization = {
        "schema_version": "p2_public_trajectory_dtw_v2r4.authorization.v1",
...
def test_seal_transitive_inventory_is_exact_and_fail_closed(
    ...
    assert len(verification["transitive_static_inputs"]) == 15
