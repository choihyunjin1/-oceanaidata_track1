...
from p2_restore import joint_hydrographic_multitask_layer4_execution_r2 as engine
...
spec = importlib.util.spec_from_file_location(name, root / 'src/p2_restore/joint_hydrographic_multitask_layer4_contract_r2.py')
...
def test_v1_failure_receipt_and_tombstone_are_exact_and_v1_is_frozen() -> None:
    ...
    assert evidence["pins"] == {
        "FAILURE_RECEIPT": config["v1_failure_evidence"]["failure_receipt"],
        "FAILURE_TOMBSTONE": config["v1_failure_evidence"]["failure_tombstone"],
...
def test_r2_control_is_distinct_and_missing_qa_fails_closed() -> None:
    config = _config()
    paths = guard.stage_paths(ROOT, config)
    assert paths["control"].is_dir()
    assert "execution_r2_control" in paths["control"].as_posix()
    assert "execution_v1_control" not in paths["control"].as_posix()
    assert (paths["control"] / "v1_failure_receipt.json").is_file()
    assert (paths["control"] / "v1_failure_tombstone.json").is_file()
...
def test_r2_correction_scope_mutations_fail_closed() -> None:
    config = _config()
    changed = copy.deepcopy(config)
