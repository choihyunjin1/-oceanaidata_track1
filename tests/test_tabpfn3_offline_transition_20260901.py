...
from ocean_tabpfn3.offline import (
    CLASSIFIER_FILENAME,
    REGRESSOR_FILENAME,
    inspect_preflight,
)
...
def test_local_receipt_does_not_allow_wrong_checkpoint_bytes(tmp_path: Path) -> None:
    ...
    result = inspect_preflight(
        workspace=tmp_path,
        env={
            "TABPFN3_LICENSE_RECEIPT_PATH": str(receipt),
            "TABPFN3_CLASSIFIER_PATH": str(classifier),
            "TABPFN3_REGRESSOR_PATH": str(regressor),
        },
    )
...
def test_license_receipt_rejects_credentials(tmp_path: Path) -> None:
    ...
    result = inspect_preflight(
        workspace=tmp_path,
        env={"TABPFN3_LICENSE_RECEIPT_PATH": str(receipt)},
    )
