...
from p2_restore import public_trajectory_dtw_v2r3 as dtw

REPO_ROOT = Path(__file__).resolve().parents[1]
RUNNER_PATH = REPO_ROOT / "scripts/run_p2_public_trajectory_dtw_curvature_transfer_v2r3.py"
...
def test_design_hash_is_immutable_and_not_authorized() -> None:
    ...
    assert "Open once" in design["additional_audit_hardening"]["authorization"]
    assert "Open once" in design["additional_audit_hardening"]["numerical_module"]
...
def test_v2r2_qa_lineage_is_actual_no_go_and_fail_closed() -> None:
    report_path = (
        REPO_ROOT
        / "reports/p2_public_trajectory_dtw_curvature_transfer_v2r2_independent_preexecution_qa_20260826.json"
    )
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert _sha256(report_path) == "5c754f4dfaa99a97a9084563db3a343c42cd7127da4de7c1c8e58181791fb340"
    assert report["verdict"] == "NO_GO"
    assert report["severity_counts"] == {"P0": 0, "P1": 2, "P2": 0}
    assert report["authorization_must_remain_false"] is True
...
def test_no_personal_absolute_observations_path_in_v2r3_static_sources() -> None:
    relatives = [
        "configs/experiments/p2_public_trajectory_dtw_curvature_transfer_v2r3_design.json",
        "configs/experiments/p2_public_trajectory_dtw_curvature_transfer_v2r3_execution.json",
        "scripts/run_p2_public_trajectory_dtw_curvature_transfer_v2r3.py",
    ]
...
def test_runner_and_numerical_module_have_no_fit_call() -> None:
    source = RUNNER_PATH.read_text(encoding="utf-8") + (REPO_ROOT / "src/p2_restore/public_trajectory_dtw_v2r3.py").read_text(encoding="utf-8")
    assert ".fit(" not in source
    assert "PHYSICAL_FIT_CALLS = 0" in source


def test_authorized_worker_loads_one_held_module_snapshot_without_package_init(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    runner = _load_runner()
    ...
    assert "_held_verified_bytes" in worker
    assert "compile(module_bytes.decode" in worker
    assert "from p2_restore" not in worker
    module_path = tmp_path / "sealed_module.py"
    module_path.write_bytes(runner.MODULE_PATH.read_bytes())
    pin = {"bytes": module_path.stat().st_size, "sha256": _sha256(module_path)}
    ...
    loaded = runner._load_sealed_numerical_module(
        {"bundle": {runner.MODULE_RELATIVE: pin}}
    )
    assert opens == 1
...
def test_module_compilation_uses_captured_bytes_after_path_mutation(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    runner = _load_runner()
    ...
    @contextmanager
    def captured_then_mutated(path: Path, observed_pin):
        assert path == module_path
        assert observed_pin == pin
        module_path.write_text("raise RuntimeError('reopened mutated path')\n", encoding="utf-8")
        yield None, original, pin["sha256"]

    monkeypatch.setattr(runner, "_held_verified_bytes", captured_then_mutated)
    loaded = runner._load_sealed_numerical_module(
        {"bundle": {runner.MODULE_RELATIVE: pin}}
    )
...
def test_authorization_verifies_real_qa_receipt_and_lineage(
    ...
    qa_path = tmp_path / "reports/p2_v2r3_independent_qa.json"
    ...
    authorization = {
        "schema_version": "p2_public_trajectory_dtw_v2r3.authorization.v1",
...
def test_actual_kst_clock_and_synthetic_filesystem_chronology_fail_closed(
    ...
    design = {"created_at_kst": to_kst(base_ns - 1_000_000_000)}
    seal = {
        "chronology": {
            "implementation_completed_at_kst": to_kst(base_ns + 3_500_000_000),
            "tests_completed_at_kst": to_kst(base_ns + 4_500_000_000),
            "seal_created_at_kst": to_kst(base_ns + 5_500_000_000),
            "filesystem_mtime_ns": {
                runner.DESIGN_RELATIVE: ordered_ns[0],
                runner.EXECUTION_CONFIG_RELATIVE: ordered_ns[1],
                runner.MODULE_RELATIVE: ordered_ns[2],
                runner.RUNNER_RELATIVE: ordered_ns[3],
                runner.TEST_RELATIVE: ordered_ns[4],
                runner.CLOSURE_MATRIX_RELATIVE: ordered_ns[5],
            },
        }
    }
...
def test_seal_transitive_inventory_is_exact_and_fail_closed(
    ...
    assert len(verification["transitive_static_inputs"]) == 13
