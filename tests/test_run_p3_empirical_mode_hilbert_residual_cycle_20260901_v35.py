...
RUNNER = ROOT / "scripts/run_p3_empirical_mode_hilbert_residual_cycle_20260901_v35.py"
SPEC = importlib.util.spec_from_file_location("p3_v35", RUNNER)
...
def test_feature_shape_finite_and_deterministic() -> None:
    ...
    first = MODULE.empirical_mode_hilbert_features(sequence)
    second = MODULE.empirical_mode_hilbert_features(sequence)
    assert first.shape == (80,)
    assert np.array_equal(first, second)
    assert np.isfinite(first).all()


def test_fixed_emd_reconstructs_and_recovers_am_fm() -> None:
    receipt = MODULE.am_fm_receipt()
    assert receipt["finite"]
    assert receipt["reconstruction_max_abs_error"] < 1e-10
    assert receipt["amplitude_envelope_correlation"] > 0.50
    assert 0.07 <= receipt["median_instantaneous_frequency"] <= 0.13


def test_positive_affine_invariance_after_case_normalization() -> None:
    rng = np.random.default_rng(37)
    values = rng.normal(size=145)
    assert np.allclose(
        MODULE.mode_features(values),
        MODULE.mode_features(4.0 * values + 11.0),
        atol=1e-10,
        rtol=1e-10,
    )
...
def test_future_perturbation_invariance() -> None:
    ...
    assert np.array_equal(
        MODULE.empirical_mode_hilbert_features(first[:289]),
        MODULE.empirical_mode_hilbert_features(second[:289]),
    )
...
def test_sealed_nonduplicate_contract() -> None:
    ...
    assert config["encoder"]["mode_count"] == 3
    assert config["encoder"]["sifts_per_mode"] == 8
...
def test_fold_purge_and_exact_fit_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        _, receipts = MODULE.v32.v28.crossfit(
            cases, np.zeros((len(cases), 80)), targets, reference
        )
    finally:
