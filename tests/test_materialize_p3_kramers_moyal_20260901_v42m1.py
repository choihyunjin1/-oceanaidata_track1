...
SPEC = importlib.util.spec_from_file_location("p3_v42m1", RUNNER)
...
def test_candidate_is_exactly_frozen() -> None:
    ...
    assert config["candidate"]["ridge_alpha"] == 512.0
...
def test_guard_accepts_small_safe_action() -> None:
    ...
    _, checks = MODULE.geometry(frame, champion + 0.01, champion, MODULE.load_config())
...
def test_guard_rejects_large_action() -> None:
    frame = pd.DataFrame({"station": np.repeat(["G-ORS", "I-ORS", "S-ORS"], 6), "lead_h": np.tile([3, 6, 9, 12, 18, 24], 3)})
    champion = np.full(18, 2.0)
    _, checks = MODULE.geometry(frame, champion + 0.25, champion, MODULE.load_config())
