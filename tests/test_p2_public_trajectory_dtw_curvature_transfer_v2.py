...
from p2_restore import public_trajectory_dtw_v2 as dtw

REPO_ROOT = Path(__file__).resolve().parents[1]
RUNNER_PATH = REPO_ROOT / "scripts/run_p2_public_trajectory_dtw_curvature_transfer_v2.py"
...
def test_continuous_block_and_seven_day_source_embargo() -> None:
    ...
    exact = dtw.validate_source_end_before_query_start(
...
def test_pre_exact_source_readiness_never_verifies_p100(
    ...
    paths = {
        ...
        "p100_anchor": (tmp_path / "must_not_be_touched.parquet", {"bytes": 7, "sha256": "c" * 64}),
    }
    monkeypatch.setattr(runner, "_source_pins", lambda seal: paths)
    ...
    assert not paths["p100_anchor"][0].exists()
...
def test_terminal_success_failure_conflict_is_rejected(
    ...
    runner._append_journal(
        runner.JOURNAL_PATH,
        runner._event("ATTEMPT_TERMINAL_FAILED", attempt),
    )
    runner.FINAL_DIR.mkdir(parents=True)
    runner._exclusive_create_json(
        runner.FINAL_DIR / "terminal_success.json",
        {"experiment_id": runner.EXPERIMENT_ID, "attempt_id": attempt},
    )
...
def test_runner_and_numerical_module_have_no_fit_call() -> None:
    source = RUNNER_PATH.read_text(encoding="utf-8") + (REPO_ROOT / "src/p2_restore/public_trajectory_dtw_v2.py").read_text(encoding="utf-8")
