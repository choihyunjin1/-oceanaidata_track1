...
RUNNER_PATH = (
    ROOT / "scripts/run_p1_incumbent_rule_distillation_neural_residual_v5r4.py"
)
SPEC = importlib.util.spec_from_file_location("test_p1_gen5r4_runner", RUNNER_PATH)
...
def test_r3_owner_no_go_and_tombstone_are_exact() -> None:
    result = runner._verify_r3_tombstone(runner._paths(ROOT))
    assert result["execution_prohibited"] is True
    assert result["r3_config"] == runner.R3_CONFIG_SHA256
    assert result["r3_runner"] == runner.R3_RUNNER_SHA256
    assert result["r3_preregistration"] == runner.R3_PREREGISTRATION_SHA256
    assert result["r3_preseal"] == runner.R3_PRESEAL_SHA256
    assert result["r3_owner_no_go"] == runner.R3_OWNER_NO_GO_SHA256
    assert result["r3_tombstone"] == runner.R3_TOMBSTONE_SHA256
...
def test_canonical_check_only_is_target_free_and_side_effect_free() -> None:
    ...
    assert result["status"] == "CANONICAL_GEN5R4_CHECK_ONLY_PASS"
