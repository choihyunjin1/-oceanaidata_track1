...
RUNNER = ROOT / "scripts/run_p3_delay_persistence_residual_cycle_20260901_v36.py"
SPEC = importlib.util.spec_from_file_location("p3_v36", RUNNER)
...
def test_h0_lifetimes_have_exact_component_merge_count() -> None:
    axis = np.arange(145, dtype=np.float64)
    values = np.sin(2.0 * np.pi * axis / 18.0)
    for delay in MODULE.DELAYS:
        points = MODULE.delay_embedding(values, delay)
        lifetimes = MODULE.persistence_lifetimes(values, delay)
        assert lifetimes.shape == (len(points) - 1,)
        assert np.all(np.diff(lifetimes) >= 0.0)
        assert np.all(lifetimes >= 0.0)


def test_synthetic_topology_sensitivity() -> None:
    receipt = MODULE.topology_sensitivity_receipt()
    assert receipt["finite"]
    assert receipt["periodic_feature_sha256"] != receipt["regime_feature_sha256"]
    assert receipt["l2_difference"] > 0.05
...
def test_positive_affine_invariance_after_case_normalization() -> None:
    ...
    for delay in MODULE.DELAYS:
        assert np.allclose(
            MODULE.lifetime_features(values, delay),
            MODULE.lifetime_features(4.0 * values + 11.0, delay),
            atol=1e-10,
            rtol=1e-10,
        )
