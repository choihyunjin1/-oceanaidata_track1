...
> **2026-09-05 재평가:** 아래 기존 최고 기록/패키지 표의 "규정 준수"는 9월 2일 점수 역산 금지 공지를 반영하지 않은 역사적 표현이다. P2 bin17 계보와 P3 refined-public alpha를 그대로 최종 적격 후보로 사용하지 않는다. 현재 실행 계획은 `docs/SCORE_IMPROVEMENT_PLAN_20260905.md`, 최상위 보완은 `00_ORGANIZER_DATA_POLICY.md` §0이다. P1도 router/GI를 포함한 전체 학습 경로 재생성이 필요하다. 기존 자산을 삭제하거나 최신 후보로 자동 제출하지 않는다.
