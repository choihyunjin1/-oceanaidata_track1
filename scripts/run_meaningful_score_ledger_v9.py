"""Canonical CLI for the v9 P3 research-curve-compatible score ledger."""
...
from ocean_goal.meaningful_score_ledger_v9 import (
    CONTRACT_RELATIVE,
    CONTRACT_SHA256,
    LEDGER_RELATIVE,
...
def check(root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "meaningful_score_ledger_v9.check.v1",
        ...
        "predecessor_v8_anchor": contract["predecessor_v8_ledger"],
        ...
        "completion_lineage_policy": contract["completion_lineage_policy"],
        "ledger_path": LEDGER_RELATIVE,
        "ledger_exists": _ledger(root).exists(),
        "local_ledger_event_count": len(records),
        "global_ledger_event_count": 2 + len(records),
        "ledger_head_event_sha256": (
            records[-1]["event_sha256"]
            if records
            else contract["predecessor_v8_ledger"]["head_event_sha256"]
        ),
        "pre_init_qa_path": PRE_INIT_QA_RELATIVE,
        "pre_init_qa_exists": _receipt(root).exists(),
        "pre_init_qa_schema": "meaningful_score_ledger_v9.pre_init_qa.v1",
        "pre_init_qa_required_decision": "GO_INITIALIZE_V9_LEDGER",
        "p3_structure_curve_decision": "RESEARCH_ONLY",
        "p3_structure_official_score_allowed": False,
        "p3_structure_upload_readiness_allowed": False,
...
def initialize(root: Path, *, append: bool, qa_receipt: Path | None) -> dict[str, Any]:
    ...
    if report["ledger_exists"]:
        if append:
            raise ContractError("v9 ledger is already initialized; second init forbidden")
    ...
    if qa_receipt is None:
        raise ContractError("canonical independent v9 pre-init QA receipt is required")
...
def evaluate_completion(root: Path, evidence_path: Path, *, append: bool) -> dict[str, Any]:
    ...
    payload = build_goal_completion_payload(
        root,
        _source_path(root, evidence_path),
