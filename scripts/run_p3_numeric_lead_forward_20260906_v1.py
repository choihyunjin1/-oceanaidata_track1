"""Frozen CPU-only lead representation ablation on the v5 forward surface.
...
NAME = "p3_numeric_lead_forward_20260906_v1"
...
def zero_access():
    return {name: 0 for name in ("official_rows", "sample_rows", "hidden_rows", "csv_rows", "uploads", "external_rows", "old_model_reads", "old_prediction_reads", "full_fits")}
...
def access_guard(source, config):
    ...
    def audit(event, args):
        ...
        if path.suffix.lower() in {".parquet", ".npz", ".cbm", ".joblib", ".pt", ".ckpt", ".tabpfn_fit"} and not owned and path not in allowed:
...
def load_cache(config):
    ...
    if len(anchor) != 24360 or not feature[["anchor_id", "station"]].equals(anchor[["anchor_id", "station"]]):
        raise ValueError("cache alignment/population mismatch")
    if any(c in columns for c in ("anchor_time", "time", "target_hs", "episode_id")) or any(c.startswith("target_") for c in columns):
...
def parameters(recipe, kind, seed, *, synthetic=False):
    ...
    params.update(task_type="CPU", thread_count=2, random_seed=int(seed), verbose=False, allow_writing_files=False)
    if synthetic:
...
def raw_context(grid, anchor):
    ...
    frame = grid.loc[(grid.station == anchor.station) & grid.time.between(time_value - pd.Timedelta(hours=48), time_value)].sort_values("time")
...
def fold_masks(anchors, contract):
    return [(fold, cv.p3_split(anchors, fold["id"], contract)) for fold in contract["P3"]["folds"] if not fold["warmup"]]
...
def preflight(config, source):
    ...
    if prepare["source_sha256"] != config["source_files"] or prepare["old_cache_reads"] != 0 or prepare["official_rows"] != 0:
    ...
    wave = pd.read_csv(source / "train_wave.csv", usecols=["station", "time", "hs", "tp", "hmax", "wvdir"])
    atmos = pd.read_csv(source / "train_atmos.csv", usecols=["station", "time", "wspd", "gust", "wdir", "airt", "relh", "caph"])
    ...
    anchors = anchors.merge(episodes, on=["station", "anchor_time"], validate="one_to_one", how="outer", indicator=True)
    ...
    for lead in (0, *LEADS):
        target_keys = pd.MultiIndex.from_arrays([anchors.station, anchors.anchor_time + pd.Timedelta(hours=lead)])
    ...
    for number, (fold, split) in enumerate(masks):
        ...
        if len(train_ids) != config["expected_train"][number] or len(valid) != config["expected_validation"][number]:
            raise ValueError("support fold count mismatch")
        if support["folds"][number]["fold"] != fold["id"] or support["folds"][number]["validation_anchors"] != len(valid):
            raise ValueError("source support receipt mismatch")
        prior = anchors.anchor_id.isin(oof_ids) & split["train"]
        rows.append({"fold": fold["id"], "train_anchors": len(train_ids), "validation_anchors": len(valid), "meta_prior_safe_anchors": int(prior.sum()), "prior_anchors_excluded_by_78h_episode_rule": len(oof_ids) - int(prior.sum()), "station_episodes": len(set(cv.bootstrap_groups(valid, "P3", contract)))})
    ...
    outside = (example_grid.time > example.anchor_time) | (example_grid.time < example.anchor_time - pd.Timedelta(hours=48))
    ...
    if recipe["router"]["alpha"] != 10 or recipe["shrink"]["persistence_weight"] != .2 or recipe["model"]["single_weight"] != .5 or recipe["model"]["multi_weight"] != .5:
        raise ValueError("fixed clean postprocessing recipe differs")
    OUT.mkdir(parents=True)
    anchors.to_parquet(OUT / "anchors.parquet", index=False)
    receipt = {"status": "FEATURE_AND_TARGET_SUPPORT_PASS", "seconds": time.perf_counter() - begin, "source_rows": {"wave": len(wave), "atmos": len(atmos)}, "anchors": len(anchors), "features": len(columns), "validation_anchors": len(oof_ids), "validation_rows": len(oof_ids) * 6, "folds": rows, "raw_context_recomputed_cases": len(sample_ids), "raw_context_max_feature_error": maximum_error, "all_anchor_six_targets_source_recomputed": True, "future_outside_context_perturbation_no_effect": True, "episode_source": "raw20min high hs>=1.5 runs; missing/low/gap break; target eligibility does not split runs", "dense_validation_not_sparse_official_case_distribution": True, "onset": "NOT_ENABLED", "cpu_threads": 2, "gpu": 0, "verified_inputs": verified, "anchors_sha256": sha(OUT / "anchors.parquet"), **zero_access()}
    write_json(REPORT / "preflight.json", receipt)
    write_json(OUT / "seal.json", {"runner_sha256": sha(__file__), "config_sha256": sha(CONFIG), "preflight_sha256": sha(REPORT / "preflight.json"), "created_utc": now()})
    print(json.dumps({key: value for key, value in receipt.items() if key not in {"verified_inputs"}}), flush=True)
...
def multi_matrix(features, anchors, ids, columns):
    ...
    target = lookup[[f"target_{lead}" for lead in LEADS]].to_numpy(float) - lookup.current_hs.to_numpy()[:, None]
...
def fit_one(kind, fold, number, split, features, anchors, columns, recipe, config, deadline):
    ...
    if model_kind == "single":
        ...
        current, cats = meta.current_hs.to_numpy(), ["station"] if numeric else ["station", "lead_h"]
    else:
        x, y, current = multi_matrix(features, anchors, ids, columns)
        vx, _, valid_current = multi_matrix(features, anchors, valid_ids, columns)
        cats = ["station"]
    model.fit(x, y, sample_weight=threshold_case_weights(current), cat_features=cats, callbacks=[Deadline(deadline)], verbose=False)
    ...
    receipt = {"fold": fold["id"], "kind": kind, "seed": config["fold_seeds"][number], "rows": len(x), "train_anchors": len(ids), "validation_anchors": len(valid_ids), "iterations": model.tree_count_, "seconds": time.perf_counter() - started, "model_path": path.relative_to(OUT).as_posix(), "model_sha256": sha(path), "parameters": params, "cat_features": cats, "lead_dtype": "float64" if kind == "numeric_single" else "string" if kind == "categorical_single" else "not a multi input"}
    del model, x, y, vx
    gc.collect()
...
def component_frame(anchors, mask, fold, single, multi):
    ...
    frame = part.loc[part.index.repeat(6), ["anchor_id", "station", "anchor_time", "episode_id", "current_hs"]].reset_index(drop=True)
...
def router_material(frame, features):
    ...
    if frame.duplicated(["anchor_id", "lead_h"]).any() or not frame.groupby("anchor_id").lead_h.agg(tuple).map(lambda value: value == LEADS).all():
        raise ValueError("router keys not six complete ordered leads")
    cases = frame.iloc[::6].reset_index(drop=True)
    components = frame[["single_prediction", "multi_prediction", "persistence"]].to_numpy().reshape(-1, 6, 3)
    observed = features.set_index("anchor_id").loc[cases.anchor_id, list(OBSERVED_FEATURES)].reset_index(drop=True)
    observed_x = build_inference_router_features(observed, cases.station.to_numpy(), cases.current_hs.to_numpy(), components)
    metadata = cases[["fold", "anchor_id", "station", "anchor_time", "episode_id"]]
    x, meta, component_rows, loss = expand_case_router_rows(observed_x, metadata, components, frame.target_hs.to_numpy().reshape(-1, 6))
...
def fixed_policy(component, features, anchors, contract, *, arm, fit, expected_receipts=None):
    ...
    weights = np.tile([.5, .5, 0.], (len(meta), 1))
    completed = []
    receipts = []
    for index, (fold, _) in enumerate(fold_masks(anchors, contract)):
        current = meta.fold.eq(fold["id"]).to_numpy()
        prior = meta.fold.isin(completed).to_numpy() & safe_meta_mask(meta, anchors, fold["id"], contract)
        ...
        if index:
            ...
            if not (meta.loc[prior, "anchor_time"] + pd.Timedelta(hours=24) < start - pd.Timedelta(hours=48)).all():
                raise ValueError("router target/context footprint overlap")
            if fit:
                model = ComponentLossRouter(RouterConfig(10., 2., .5, "smooth_medium")).fit(x.loc[prior], loss[prior])
                path.parent.mkdir(parents=True, exist_ok=True)
                joblib.dump(model, path)
            else:
                model = joblib.load(path)
            weights[current] = model.predict_weights(x.loc[current])
        key_payload = meta.loc[prior, ["fold", "anchor_id", "station", "lead_h"]].to_json(orient="split", index=False)
        receipts.append({"arm": arm, "fold": fold["id"], "past_fit_rows": int(prior.sum()), "past_fit_anchors": int(meta.loc[prior, "anchor_id"].nunique()), "prior_key_sha256": hashlib.sha256(key_payload.encode()).hexdigest(), "current_fold_labels_used": False, "target_context_separation_verified": True, "model_path": path.relative_to(OUT).as_posix() if index else None, "model_sha256": sha(path) if index else None})
        completed.append(fold["id"])
    weights[~meta.lead_h.isin([12, 18, 24]).to_numpy()] = [.5, .5, 0.]
    routed = route_row_predictions(components, weights)
    final = apply_long_lead_persistence_shrink(routed, components[:, 2], meta.lead_h.to_numpy(), config=LongLeadPersistenceShrink(.2, (12, 18, 24)))
...
def summary(frame, contract):
    ...
    output = cv.paired_bootstrap(frame.target_hs, frame.control, frame.candidate, cv.bootstrap_groups(frame, "P3", contract), "rmse", contract)
    ...
    for dimension in ("fold", "station", "lead_h", "wind_observed"):
        for key, part in frame.groupby(dimension):
            ...
            slices.append({"dimension": dimension, "key": str(key), "rows": len(part), "anchors": int(part.anchor_id.nunique()), "control_rmse_m": control, "candidate_rmse_m": candidate, "delta_candidate_minus_control_m": candidate - control})
    output["slices"] = slices
    output["worst_quarter_delta_m"] = max(row["delta_candidate_minus_control_m"] for row in slices if row["dimension"] == "fold")
    output["maximum_slice_harm_m"] = max(row["delta_candidate_minus_control_m"] for row in slices if row["dimension"] != "fold")
    output["onset_diagnostic"] = "NOT_ENABLED"
    output["greedy_diagnostic"] = "NOT_ENABLED"
    output["cluster_caveat"] = "raw station high-runs, not independent meteorological storms; dense adjacent anchors and cross-station storm dependence remain"
...
def execute(config, source):
    ...
    write_json(OUT / "ATTEMPT_LOCK.json", {"pid": os.getpid(), "created_utc": now(), "max_backbone_fits": 15, "max_router_fits": 8, "wall_seconds": 5400, "gpu": 0})
    ...
    def progress(stage):
        receipt = {"stage": stage, "pid": os.getpid(), "completed_backbone_fits": len(fits), "max_backbone_fits": 15, "elapsed_seconds": time.perf_counter() - begin, "cpu_threads": 2, "gpu": 0}
    ...
    for number, (fold, split) in enumerate(masks):
        ...
        single, receipt = fit_one("categorical_single", fold, number, split, features, anchors, columns, recipe, config, deadline)
        fits.append(receipt)
        progress(f"BASELINE_{fold['id']}_MULTI")
        multi, receipt = fit_one("multi", fold, number, split, features, anchors, columns, recipe, config, deadline)
        fits.append(receipt)
        multis[fold["id"]] = multi
        baseline_parts.append(component_frame(anchors, split["validation"], fold["id"], single, multi))
        if number == 0:
            ratio = sum(config["expected_train"]) / config["expected_train"][0]
            estimate = config["budget"]["pilot_estimate_margin"] * ratio * (2 * fits[0]["seconds"] + fits[1]["seconds"])
            pilot = {"status": "CONTINUE" if estimate < config["budget"]["wall_seconds"] else "RESOURCE_CAP_PREDICTED_STOP_BEFORE_OUTCOME_REVIEW", "planned_first_two_fits_reused": True, "single_seconds": fits[0]["seconds"], "multi_seconds": fits[1]["seconds"], "train_anchor_scale_sum": ratio, "margin": 1.2, "estimated_total_fit_seconds": estimate, "wall_cap_seconds": 5400, "model_metrics_inspected": False}
            write_json(REPORT / "resource-pilot.json", pilot)
            if estimate >= config["budget"]["wall_seconds"]:
                write_json(OUT / "fit-receipts.json", {"fits": fits}, progress=True)
                raise TimeoutError("first planned fits forecast exceeds90min; no outcome-driven restart")
    progress("BASELINE_COMPLETE_FIXED_ROUTER")
    baseline_components = pd.concat(baseline_parts, ignore_index=True)
    baseline, base_router = fixed_policy(baseline_components, features, anchors, contract, arm="control", fit=True)
    baseline.to_parquet(OUT / "baseline_oof.parquet", index=False)
    write_json(REPORT / "baseline-complete.json", {"status": "FRESH_CPU_BASELINE_COMPLETE_BEFORE_CANDIDATE", "backbone_fits": 10, "router_fits": 4, "rows": len(baseline), "oof_sha256": sha(OUT / "baseline_oof.parquet"), "old_gpu_score_inherited": False, "router_receipts": base_router, **zero_access()})
    for number, (fold, split) in enumerate(masks):
        progress(f"NUMERIC_CANDIDATE_{fold['id']}")
        single, receipt = fit_one("numeric_single", fold, number, split, features, anchors, columns, recipe, config, deadline)
        fits.append(receipt)
        numeric_parts.append(component_frame(anchors, split["validation"], fold["id"], single, multis[fold["id"]]))
    progress("BOTH_ARMS_COMPLETE_FIXED_ROUTER")
    numeric, candidate_router = fixed_policy(pd.concat(numeric_parts, ignore_index=True), features, anchors, contract, arm="candidate", fit=True)
    if not baseline[KEYS].equals(numeric[KEYS]) or not np.array_equal(baseline.target_hs, numeric.target_hs):
    ...
    wind_columns = [c for c in columns if c.startswith(("wspd_", "gust_", "wdir_sin_", "wdir_cos_")) and "_valid_" in c]
    final["wind_observed"] = features.set_index("anchor_id").loc[final.anchor_id, wind_columns].gt(0).any(axis=1).to_numpy()
    final.to_parquet(OUT / "paired_oof.parquet", index=False)
    stats = summary(final, contract)
    verify_pins(config, source, sealed=True)
    result = {"status": "MEAN_IMPROVEMENT_CANDIDATE_RETAINED" if stats["candidate_retained"] else "NO_OP_NUMERIC_LEAD_NOT_IMPROVED", "pid": os.getpid(), "comparison": stats, "fit_count": 15, "router_fit_count": 8, "fit_receipts": fits, "router_receipts": {"control": base_router, "candidate": candidate_router}, "execution_seconds": time.perf_counter() - begin, "preflight_seconds": pre["seconds"], "runner_sha256": sha(__file__), "config_sha256": sha(CONFIG), "preflight_sha256": sha(REPORT / "preflight.json"), "source_sha256": config["source_files"], "artifacts": {name: sha(OUT / name) for name in ("anchors.parquet", "baseline_oof.parquet", "candidate_oof.parquet", "paired_oof.parquet")}, "baseline_is_new_cpu_v5_not_old_gpu_score": True, "baseline_regeneration_check": "NOT_RUN_FULL_MATERIALIZATION_OUT_OF_SCOPE", "same_environment_saved_model_replay": "PENDING", **zero_access()}
    write_json(REPORT / "result.json", result)
    write_json(OUT / "TERMINAL.json", {"status": result["status"], "result_sha256": sha(REPORT / "result.json"), "fits": 15, "router_fits": 8})
    progress("TERMINAL")
    print(json.dumps({"status": result["status"], "seconds": result["execution_seconds"], "comparison": {k: v for k, v in stats.items() if k != "slices"}}), flush=True)
...
def replay(config, source):
    ...
    for fold, split in fold_masks(anchors, contract):
        ...
        for kind in ("multi", "categorical_single", "numeric_single"):
            ...
            if kind == "multi":
                ...
                prediction = np.clip(meta.current_hs.to_numpy() + model.predict(matrix(x, kind == "numeric_single"), thread_count=2), 0, 30)
                ordered = meta[["anchor_id", "lead_h"]].assign(prediction=prediction).sort_values(["anchor_id", "lead_h"])
                values[kind] = ordered.prediction.to_numpy().reshape(-1, 6)
            checks += 1
        for arm, kind in (("control", "categorical_single"), ("candidate", "numeric_single")):
            blocks[arm].append(component_frame(anchors, split["validation"], fold["id"], values[kind], values["multi"]))
        gc.collect()
    for arm, filename in (("control", "baseline_oof.parquet"), ("candidate", "candidate_oof.parquet")):
        predicted, _ = fixed_policy(pd.concat(blocks[arm], ignore_index=True), feature, anchors, contract, arm=arm, fit=False, expected_receipts=result["router_receipts"][arm])
    ...
    receipt = {"status": "PASS", "fresh_pid": os.getpid(), "training_pid": result["pid"], "backbone_models": checks, "router_models": 8, "paired_rows": 103602, "max_abs_prediction_error_m": max_error, "seconds": time.perf_counter() - start, "result_sha256": sha(REPORT / "result.json"), "same_environment_only": True, "full_regeneration_or_portable_package": False, **zero_access()}
...
def main():
    ...
    try:
        ...
        if OUT.exists() and not (OUT / "TERMINAL.json").exists() and not (OUT / "FAILURE.json").exists():
            write_json(OUT / "FAILURE.json", {"status": "RESOURCE_STOP_NO_RESTART" if isinstance(exc, TimeoutError) else "TECHNICAL_FAILURE_NO_RESTART", "error_type": type(exc).__name__, "message": str(exc), "pid": os.getpid(), "created_utc": now(), **zero_access()})
        raise
