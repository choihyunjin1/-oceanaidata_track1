"""User-authorized uncapped CPU execution with explicit verified-prefix reuse.

Old attempts remain immutable. A resumed run is not a new complete cold run.
"""
...
def no_watchdog(*args, **kwargs):
    """No timer and no background termination; elapsed time remains recorded."""
    return NoTimer()


def reuse_fits(cfg, c):
    manifest = D / "reuse-manifest.json"
    if not manifest.exists():
        return []
    receipt = b.read(manifest)
    parent_config = b.read(D / "parent-config.json")
    if b.sha(D / "parent-config.json") != receipt["parent_config_sha256"]:
        raise ValueError("parent config pin changed")
    ignored = {"id", "maximum_seconds_per_cold", "execution_amendment"}
    if {k: v for k, v in cfg.items() if k not in ignored} != {
        k: v for k, v in parent_config.items() if k not in ignored
    }:
        raise ValueError("only ID and execution-budget amendment may differ")
    if b.sha(D / "prepare.json") != receipt["prepare_sha256"]:
        raise ValueError("parent prepare receipt changed")
    for relative, expected in receipt["imported_files"].items():
        path = (P / relative).resolve()
        if not path.is_relative_to(P) or b.sha(path) != expected:
            raise ValueError("reused artifact pin mismatch")
    fits = receipt["fits"]
    if len(fits) != 29 or len({x["model"] for x in fits}) != len(fits):
        raise ValueError("exactly 29 unique completed parent fits required")
    actual = {x.relative_to(P).as_posix() for x in M.rglob("*") if x.is_file()}
    if actual != {x["model"] for x in fits}:
        raise ValueError("unrecorded/partial model present")
    if b.read(D / "prepare.json")["source_sha256"] != c["source_files"]:
        raise ValueError("reused data lineage mismatch")
    return fits


def settings():
    manifest = b.read(CODE / "unbounded-manifest.json")
    for name, expected in manifest.items():
        path = (P / name).resolve()
        if not path.is_relative_to(P) or b.sha(path) != expected:
            raise ValueError("source pin mismatch: " + name)
    cfg = b.read(CODE / "unbounded-config.json")
    if cfg["seeds"] != [20260817, 20260818, 20260819] or cfg["cpu_threads"] != 4:
        raise ValueError("fixed seed/resource contract")
    if cfg["maximum_seconds_per_cold"] is not None:
        raise ValueError("user-authorized uncapped contract required")
...
def train(cfg, c):
    ...
    inherited = reuse_fits(cfg, c)
    if (any(M.iterdir()) and not inherited) or len(columns) != 591:
        raise ValueError("fresh or explicitly hash-verified models and 591 features required")
    b.save(
        P / "CPU_TRAIN_LOCK.json", {"pid": os.getpid(), "backbone_budget": 36, "router_budget": 5}
    )
    timer = b.watchdog(c, p["started_unix"])
    deadline = float("inf")  # no elapsed-time early termination; full iterations retained
    fits = []
    ...
    for scope, ids in [
        ...
        for seed in cfg["seeds"]:
            for kind in ["single", "multi"]:
                relative = f"03_model/{scope}/{seed}_{kind}.cbm"
                if len(fits) < len(inherited):
                    item = inherited[len(fits)]
                    if item["model"] != relative or item["kind"] != kind or item["seed"] != seed:
                        raise ValueError("reused fits must be exact fixed-schedule prefix")
                    expected_rows = len(ids) * (6 if kind == "single" else 1)
                    if item["rows"] != expected_rows or item["parameters"] != cpu_parameters(
                        c["recipe"], kind, seed
                    ):
                        raise ValueError("reused population or model parameter mismatch")
                    model = CatBoostRegressor().load_model(P / relative)
                    if model.tree_count_ != item["iterations"]:
                        raise ValueError("reused model iteration mismatch")
                    del model
                    fits.append({**item, "reused_completed_fit": True})
                else:
                    fits.append(
                        fit_component(
                            kind,
    ...
    r = {
        ...
        "reused_backbone_fits": len(inherited),
        "new_backbone_fits": len(fits) - len(inherited),
        "run_kind": "VERIFIED_PREFIX_COMPLETION" if inherited else "FRESH_COLD",
        "fit_seconds_excluding_pause": float(sum(x["seconds"] for x in fits)),
        "hard_runtime_limit_seconds": None,
        "current_invocation_seconds": time.monotonic() - invocation_started,
        "router_fits": 5,
        ...
        "seconds": elapsed(),
        "official_rows": 0,
        "old_cache_model_prediction_inputs": len(inherited),
        "reused_inputs_scope": "explicit same-recipe failed-attempt completed models only"
        if inherited
        else "none",
        "hidden_rows": 0,
        "config_sha256": b.sha(CODE / "unbounded-config.json"),
...
def main():
    ...
    b.watchdog = no_watchdog  # authorized workflow time cap removal, not a data-access change
    ...
    try:
        with threadpool_limits(limits=2):
            if args.stage == "smoke":
                ...
                if (D / "reuse-manifest.json").exists():
                    reused = reuse_fits(cfg, c)
                    b.source_hashes(c, source)
                    b.prepared(c)
                    b.save(
                        D / "reuse-prepare-qa.json",
                        {
                            "status": "PASS",
                            "reused_fits": len(reused),
                            "pid": os.getpid(),
                            "new_fits": 0,
                            "official_rows": 0,
