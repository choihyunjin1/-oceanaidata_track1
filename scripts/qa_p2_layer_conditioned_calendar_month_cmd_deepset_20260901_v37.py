"""Independent QA for sealed P2 v37 layer-conditioned month CMD run."""
...
def main() -> None:
    ...
    penalty_points = float(config["evaluation"]["transport_penalty_points"])
    delta = float(recomputed["delta_rmse"])
    nominal = -delta * slope
    transport = nominal - penalty_points
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        environment_receipt = receipt["weight_receipt"]["cmd_environment_contract"]
        environment_contract_ok &= environment_receipt["encoding"] == (
            "100*target_layer+KST_calendar_month"
        )
        environment_contract_ok &= environment_receipt["environment_count"] == len(
            environment_receipt["environment_ids"]
        )
        environment_contract_ok &= sum(environment_receipt["environment_rows"]) == receipt[
            "training_rows"
        ]
        for fit in receipt["fit_receipts"]:
            finite_values = [
                fit["base_loss_first"],
                fit["base_loss_last"],
                fit["cmd_penalty_first"],
                fit["cmd_penalty_last"],
            ...
            fit_receipts_ok &= fit["cmd_penalty_steps"] == fit["optimizer_steps"]
            fit_receipts_ok &= fit["coefficient"] == 1.0
            fit_receipts_ok &= fit["orders"] == [1, 2, 3, 4, 5]
            fit_receipts_ok &= fit["bounded_latent_dimension"] == 32
            fit_receipts_ok &= fit["supported_environment_count_min"] >= 2
            fit_receipts_ok &= fit["supported_target_layer_count_min"] >= 1
            fit_receipts_ok &= fit["moment_term_count_min"] >= 10
            fit_receipts_ok &= fit["kernel_steps"] == 0
            fit_receipts_ok &= fit["adversary_steps"] == 0
            fit_receipts_ok &= fit["ema_steps"] == 0
            fit_receipts_ok &= fit["warmup_steps"] == 0
    ...
    cmd = runner._cmd_contract_receipt()
    forward = runner._forward_contract_receipt()
    ...
    checks = {
        ...
        "new_cmd_axis": semantic["classification"]
        == "NEW_P2_LAYER_CONDITIONED_CALENDAR_MONTH_LATENT_CMD_ORDERS_1_TO_5",
        "repo_exact_execution_hits_zero": semantic["repository_p2_exact_execution_hits"]
        == 0,
        "semantic_distinctions_all_true": all(
            semantic[name]
            for name in (
                "v20_covariance_only_distinguished",
                "v31_adversarial_distinguished",
                "v36_gradient_variance_distinguished",
            )
        ),
        ...
        is False,
        "cmd_contract": cmd["permutation_invariant"]
        and cmd["gradients_finite"]
        and cmd["penalty_finite"]
        and cmd["orders"] == [1, 2, 3, 4, 5],
        "forward_contract": forward["prediction_maximum_abs_error"] <= 1e-7
        and forward["latent_shape"] == [5, 32]
        and forward["bounded_latent_maximum_abs_value"] < 1.0,
    ...
    qa = {
        "schema_version": "p2.layer_conditioned_calendar_month_cmd_deepset.independent_qa.20260901.v37",
