"""Independent QA for the P2 v21 fixed-Student-t DeepSets run."""
...
import run_p2_fixed_student_t_robust_deepset_20260901_v21 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            ...
            fit_receipts_ok &= fit["student_t_degrees_of_freedom"] == 4.0
            fit_receipts_ok &= fit["student_t_normalized_scale"] == 1.0
            fit_receipts_ok &= fit["weight_decay"] == 0.0001
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"]
    ...
    for name, path in preserved_paths.items():
        ...
        comparison_ok &= close(comparison[f"{name}_delta_rmse"], preserved["delta_rmse"])
        comparison_ok &= close(
            comparison[f"v21_minus_{name}_delta_rmse"],
            delta - float(preserved["delta_rmse"]),
        )
        comparison_ok &= close(
            comparison[f"v21_minus_{name}_canonical_nominal_points"],
    ...
    checks = {
        ...
        "new_p2_student_t_axis": (
            semantic["classification"]
            == "NEW_P2_FIXED_STUDENT_T_ROBUST_REGRESSION_OBJECTIVE"
        ),
        "no_prior_p2_student_t_execution": (
            semantic["prior_p2_student_t_runners"] == []
            and semantic["prior_p2_student_t_artifacts"] == []
            and semantic["prior_p2_student_t_reports"] == []
        ),
        "v10_distinguished": semantic[
            "v10_zero_fit_public_difference_action_shrink_distinguished"
        ],
        "v13_v18_v20_distinguished": (
            semantic["v13_fixed_beta_huber_distinguished"]
            and semantic["v18_v19_environment_risk_objectives_distinguished"]
            and semantic["v20_latent_covariance_alignment_distinguished"]
        ),
        "fixed_objective": (
            result["training"]["student_t_degrees_of_freedom"] == 4.0
            and result["training"]["student_t_normalized_scale"] == 1.0
        ...
        "v20_runner_hash": (
            runner.v12.sha256_file(runner.v20.RUNNER) == result["hashes"]["v20_runner"]
        ),
        ...
        "comparison_v13_v18_v20": comparison_ok,
    ...
    qa = {
        "schema_version": "p2.fixed_student_t_robust_deepset.independent_qa.20260901.v21",
        ...
        "hashes": {
            ...
            "v20_runner": runner.v12.sha256_file(runner.v20.RUNNER),
        },
    }
