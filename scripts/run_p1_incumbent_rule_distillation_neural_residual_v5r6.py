"""Run the append-only P1 Gen5r6 portable blind-boundary correction once."""
...
EXPECTED_CONFIG_SHA256 = (
    "f2f1ccbb0b8ca3f7a1018cd7698d2a30313d29473b6fbe11becc5aec1161c5d8"
)
EXPECTED_CONFIG_DEEP_SHA256 = (
    "385741d8c6fc3d103f5220449b0de2d6b32e8d76bdcd4f561fbea2affa45dae0"
)
CANONICAL_CONFIG = (
    "configs/experiments/p1_incumbent_rule_distillation_neural_residual_v5r6.json"
)
CANONICAL_RUNNER = (
    "scripts/run_p1_incumbent_rule_distillation_neural_residual_v5r6.py"
)
CANONICAL_ARTIFACT = "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r6"
CANONICAL_CONTROL = "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r6_control"
...
R5_CONFIG_SHA256 = "a80bec2fa23d256d12c7575f6118cdcadffcb4deea9ef10d8aa19f890d799817"
R5_CONFIG_DEEP_SHA256 = "636975841e1805602b3f4404a1f500eae48adee17c13558fb899fef8a6e79083"
R5_RUNNER_SHA256 = "e734613f84264fe3f43337dc44c300ed322a0764d976faebf6d06fe265e48eea"
R5_TEST_SHA256 = "c35a3db40d48f3658e0531c873cf13c293ca87e5bedd72aa145d891cbffb9017"
R5_PREREGISTRATION_SHA256 = (
    "1faac14addb266ddd72d08e65675f194059f67ae50801dab40f2994b584f2021"
)
R5_PRESEAL_SHA256 = (
    "f0503c820f9e77bdf84a66881a61bb559549c15d9c7cd7a9269a78eb84cd916a"
)
R5_OWNER_NO_GO_SHA256 = (
    "ff3c67f8ee9750146216f4002c16c1d1a4ba4171af2eecab9e3b2bde29d7ed4c"
)
R5_TOMBSTONE_SHA256 = (
    "127084fce16bfd8c9b0416d84f4aba4a859289bc3b740eb1c7a8a0f53756dbfb"
)
R5_DATA_IDENTITY_OBSERVATION_1_SHA256 = (
    "7f0d568920fd5ecc82ce753cf872113accab9975297d934fe2f438973c1e223c"
)
R5_DATA_IDENTITY_OBSERVATION_2_SHA256 = (
...
_ALLOWED_DATA_IDENTITY_SET_IDS = ("r5_frozen", "reviewer_alternate")
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        ...
        / "scripts/run_p1_incumbent_rule_distillation_neural_residual_v5r5.py",
        "r5_tests": root
        / "tests/test_run_p1_incumbent_rule_distillation_neural_residual_v5r5.py",
        "r5_preregistration": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/preregistration.json",
        "r5_preseal": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/preseal_static_qa.json",
        "r5_owner_no_go": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/OWNER_STATIC_QA_NO_GO_20260823.json",
        "r5_tombstone": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/EXECUTION_TOMBSTONE.json",
        "r5_artifact": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5",
        "r5_qa": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/independent_static_qa.json",
        "r5_authorization": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/execution_authorization.json",
        "r5_lock": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/attempt.lock",
        "r5_identity_observation_1": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/data_identity_observation_1.json",
        "r5_identity_observation_2": root
        / "artifacts/p1_incumbent_rule_distillation_neural_residual_v5r5_control/data_identity_observation_2.json",
...
def _capture_anchor_snapshot(
    ...
    for role in _WORKSPACE_ANCHOR_ROLES:
        _, relative = _ANCHOR_FILES[role]
        files[role] = _file_identity(root / relative, role=role)
        if files[role] != expected_files[role]:
            raise PermissionError(f"registered anchor-file identity differs: {role}")
    workspace_identity = _directory_identity(root, role="workspace")
    if workspace_identity != overlay["anchor_directory_identity"]["workspace"]:
        raise PermissionError("registered workspace-directory identity differs")
    allowed_sets = _validate_allowed_data_identity_sets(overlay)
    _, live_data = _capture_live_data_identity(data_dir, allowed_sets=allowed_sets)
    common_data_files: dict[str, dict[str, Any]] = {}
    for role in _DATA_ANCHOR_ROLES:
        value = live_data["files"][role]
        common_data_files[role] = {
            key: value[key]
            for key in ("sha256", "bytes", "device", "nlink", "non_reparse")
        }
    files.update(common_data_files)
    directories = {
        "workspace": workspace_identity,
        "data": {
            "identity_policy": "EXACT_COMPLETE_SET_ALLOWLIST",
            "allowed_set_ids": list(_ALLOWED_DATA_IDENTITY_SET_IDS),
            "allowed_sets_sha256": _deep_sha(allowed_sets),
            "matched_exactly_one_allowed_set": True,
        },
    }
    ...
    return {
        ...
        "files": files,
        "allowed_data_identity_sets": allowed_sets,
        "data_identity_policy": {
            "accepted_set_count": 2,
            "atomic_complete_set_match": True,
            "mixed_or_third_set_rejected": True,
            "oscillation_between_known_sets_normalized": True,
        },
        "runner": runner,
        "config": config,
    }
...
def _validate_allowed_data_identity_sets(
    overlay: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    allowed = overlay.get("allowed_data_identity_sets")
    if not isinstance(allowed, dict) or tuple(allowed) != _ALLOWED_DATA_IDENTITY_SET_IDS:
        raise PermissionError("registered data-identity set allowlist differs")
    expected_frozen = {
        "directory": overlay["anchor_directory_identity"]["data"],
        "files": {
            role: overlay["anchor_file_identity"][role] for role in _DATA_ANCHOR_ROLES
        },
    }
    if allowed["r5_frozen"] != expected_frozen:
        raise PermissionError("r5 frozen data-identity lineage differs")
    common_by_role: dict[str, dict[str, Any]] | None = None
    for set_id, identity_set in allowed.items():
        if set(identity_set) != {"directory", "files"}:
            raise PermissionError(f"data-identity set shape differs: {set_id}")
        if set(identity_set["files"]) != set(_DATA_ANCHOR_ROLES):
            raise PermissionError(f"data-identity file roles differ: {set_id}")
        if identity_set["directory"].get("non_reparse") is not True:
            raise PermissionError(f"data directory reparse guard differs: {set_id}")
        common = {}
        for role, value in identity_set["files"].items():
            if value.get("nlink") != 1 or value.get("non_reparse") is not True:
                raise PermissionError(f"data file link guard differs: {set_id}:{role}")
            common[role] = {
                key: value[key]
                for key in ("sha256", "bytes", "device", "nlink", "non_reparse")
            }
        if common_by_role is None:
            common_by_role = common
        elif common != common_by_role:
            raise PermissionError("allowed data identities do not share exact source content")
    if allowed["r5_frozen"] == allowed["reviewer_alternate"]:
        raise PermissionError("allowed data-identity states unexpectedly collapse")
    return copy.deepcopy(allowed)


def _capture_live_data_identity(
    data_dir: Path,
    *,
    allowed_sets: Mapping[str, dict[str, Any]],
) -> tuple[str, dict[str, Any]]:
    observed = {
        "directory": _directory_identity(data_dir, role="data"),
        "files": {
            role: _file_identity(data_dir / _ANCHOR_FILES[role][1], role=role)
            for role in _DATA_ANCHOR_ROLES
        },
    }
    matches = [set_id for set_id, expected in allowed_sets.items() if observed == expected]
    if len(matches) != 1:
        raise PermissionError(
            "canonical data identity must match exactly one complete allowed set"
        )
    return matches[0], observed


def _verify_r5_tombstone(paths: dict[str, Path]) -> dict[str, Any]:
    expected = {
        "r5_config": R5_CONFIG_SHA256,
        "r5_runner": R5_RUNNER_SHA256,
        "r5_tests": R5_TEST_SHA256,
        "r5_preregistration": R5_PREREGISTRATION_SHA256,
        "r5_preseal": R5_PRESEAL_SHA256,
        "r5_owner_no_go": R5_OWNER_NO_GO_SHA256,
        "r5_tombstone": R5_TOMBSTONE_SHA256,
    }
    observed = {name: _sha(paths[name]) for name in expected}
    if observed != expected:
        raise PermissionError("Gen5r5 reviewed bytes, owner NO-GO, or tombstone differ")
    if _deep_sha(_strict_json(paths["r5_config"])) != R5_CONFIG_DEEP_SHA256:
        raise PermissionError("Gen5r5 reviewed config deep SHA differs")
    if any(
        paths[name].exists()
        for name in ("r5_artifact", "r5_qa", "r5_authorization", "r5_lock")
    ):
        raise PermissionError("invalidated Gen5r5 gained a run-control or result artifact")
    tombstone = _strict_json(paths["r5_tombstone"])
    no_go = _strict_json(paths["r5_owner_no_go"])
    if not (
        tombstone.get("generation")
        == "p1_incumbent_rule_distillation_neural_residual_v5r5"
        and tombstone.get("successor_generation")
        == "p1_incumbent_rule_distillation_neural_residual_v5r6"
        and tombstone.get("execution_prohibited") is True
        and tombstone.get("authorization_must_fail_before_attempt_lock") is True
        and tombstone.get("identity_return_to_either_known_set_does_not_clear_tombstone")
        is True
        ...
        raise PermissionError("Gen5r5 execution tombstone semantics differ")
    if not (
        no_go.get("verdict") == "P0=0_P1=1_NO_GO"
        and no_go.get("p0_count") == 0
        and no_go.get("p1_count") == 1
        and no_go.get("review_kind") == "fresh_independent_read_only_static_qa"
        and no_go.get("reviewer_live_identity_set", {}).get(
            "exactly_matches_r4_reviewer_alternate_set"
        ) is True
        and no_go.get("source_content_equivalence", {}).get(
            "all_four_sha256_and_bytes_exact"
        ) is True
        and no_go.get("successor_contract", {}).get("generation")
        == "p1_incumbent_rule_distillation_neural_residual_v5r6"
    ):
        raise PermissionError("Gen5r5 identity-oscillation provenance differs")
    return {**observed, "execution_prohibited": True}


def _verify_r5_source_content_equivalence(
    paths: dict[str, Path],
    overlay: dict[str, Any],
) -> dict[str, Any]:
    r5 = _strict_json(paths["r5_config"])
    role_names = {
    ...
    for role, filename in role_names.items():
        previous = r5["anchor_file_identity"][role]
        current = overlay["anchor_file_identity"][role]
        if (
            previous["sha256"] != current["sha256"]
            or previous["bytes"] != current["bytes"]
        ):
            raise PermissionError(f"Gen5r6 source content differs from r5: {filename}")
        rows[filename] = {
            "r5_sha256": previous["sha256"],
            "current_sha256": current["sha256"],
            "r5_bytes": int(previous["bytes"]),
            "current_bytes": int(current["bytes"]),
            "sha256_equal": True,
            "bytes_equal": True,
        }
    if set(rows) != {"README.md", "train.csv", "test.csv", "sample_submission.csv"}:
        raise AssertionError("Gen5r6 four-source equivalence set differs")
    return {
        "r5_config_sha256": R5_CONFIG_SHA256,
        "sources": rows,
        "all_four_sha256_and_bytes_equal": True,
    }


def _verify_identity_lineage(
    paths: dict[str, Path],
    overlay: dict[str, Any],
) -> dict[str, Any]:
    pins = (
        (1, paths["r5_identity_observation_1"], R5_DATA_IDENTITY_OBSERVATION_1_SHA256),
        (2, paths["r5_identity_observation_2"], R5_DATA_IDENTITY_OBSERVATION_2_SHA256),
    )
    source_roles = {
    ...
    for ordinal, path, expected_sha in pins:
        if _sha(path) != expected_sha:
            raise PermissionError(f"Gen5r6 identity observation {ordinal} SHA differs")
        value = _strict_json(path)
        expected_sources = {
            filename: {
                **overlay["allowed_data_identity_sets"]["r5_frozen"]["files"][role],
        ...
        if not (
            ...
            and value.get("data_directory_identity")
            == overlay["allowed_data_identity_sets"]["r5_frozen"]["directory"]
            and value.get("source_files") == expected_sources
            and value.get("r4_config_sha256")
            ...
            raise PermissionError(f"Gen5r6 identity observation {ordinal} differs")
        observed.append(
            {
    ...
    if not observed[0]["captured_at_kst"] < observed[1]["captured_at_kst"]:
        raise PermissionError("Gen5r6 identity observations are not time-separated")
    no_go = _strict_json(paths["r5_owner_no_go"])
    filename_roles = {
        "README.md_inode": "source_readme",
        "train.csv_inode": "train_source",
        "test.csv_inode": "test_source",
        "sample_submission.csv_inode": "sample_submission_source",
    }
    for receipt_key, set_id in (
        ("r5_frozen_identity_set", "r5_frozen"),
        ("reviewer_live_identity_set", "reviewer_alternate"),
    ):
        receipt_set = no_go[receipt_key]
        expected_set = overlay["allowed_data_identity_sets"][set_id]
        if not (
            receipt_set["device"] == expected_set["directory"]["device"]
            and receipt_set["data_directory_inode"]
            == expected_set["directory"]["inode"]
            and all(
                receipt_set[receipt_name] == expected_set["files"][role]["inode"]
                for receipt_name, role in filename_roles.items()
            )
        ):
            raise PermissionError(f"Gen5r6 {set_id} lineage differs from r5 NO-GO")
    expected_lineage = {
        "r5_observation_1": {
            "path": paths["r5_identity_observation_1"]
            .relative_to(paths["config"].parents[2])
            .as_posix(),
            "sha256": R5_DATA_IDENTITY_OBSERVATION_1_SHA256,
        },
        "r5_observation_2": {
            "path": paths["r5_identity_observation_2"]
            .relative_to(paths["config"].parents[2])
            .as_posix(),
            "sha256": R5_DATA_IDENTITY_OBSERVATION_2_SHA256,
        },
        "r5_owner_no_go": {
            "path": paths["r5_owner_no_go"]
            .relative_to(paths["config"].parents[2])
            .as_posix(),
            "sha256": R5_OWNER_NO_GO_SHA256,
        },
        "r5_execution_tombstone": {
            "path": paths["r5_tombstone"]
            .relative_to(paths["config"].parents[2])
            .as_posix(),
            "sha256": R5_TOMBSTONE_SHA256,
        },
    }
    if overlay.get("identity_lineage") != expected_lineage:
        raise PermissionError("Gen5r6 configured identity lineage differs")
    return {
        "observations": observed,
        "r5_frozen_observation_count": 2,
        "both_observations_match_r5_frozen_set": True,
        "reviewer_alternate_set_bound_by_r5_no_go": True,
        "accepted_identity_set_ids": list(_ALLOWED_DATA_IDENTITY_SET_IDS),
    }
...
def _verify_execution_closure(
    ...
    if not isinstance(expected, dict) or not expected:
        raise PermissionError("Gen5r6 executable closure is empty")
    names = set(expected)
    ...
    for relative, expected_sha in expected.items():
        ...
        if rel.is_absolute() or ".." in rel.parts:
            raise PermissionError("Gen5r6 executable closure path is non-portable")
        path = (root / rel).resolve(strict=True)
        if not path.is_relative_to(root):
            raise PermissionError("Gen5r6 executable closure escaped the workspace")
        observed[relative] = _sha(path)
        if observed[relative] != expected_sha:
            raise PermissionError(f"Gen5r6 executable closure SHA differs: {relative}")
    return observed
...
def _verify_owner_seals(
    ...
    if not paths["preregistration"].is_file() or not paths["preseal"].is_file():
        raise PermissionError("Gen5r6 owner preregistration or preseal is missing")
    runner_pin = {"path": CANONICAL_RUNNER, "sha256": _sha(paths["runner"])}
    config_pin = {"path": CANONICAL_CONFIG, "sha256": EXPECTED_CONFIG_SHA256}
    identity_lineage = _verify_identity_lineage(paths, overlay)
    content_equivalence = _verify_r5_source_content_equivalence(paths, overlay)
    allowed_sets_sha256 = _deep_sha(_validate_allowed_data_identity_sets(overlay))
    prereg = _strict_json(paths["preregistration"])
    if not (
        prereg.get("schema_version") == "p1_gen5r6_preregistration.v1"
        ...
        and prereg.get("data_identity_lineage") == identity_lineage
        and prereg.get("r5_source_content_equivalence_sha256")
        == _deep_sha(content_equivalence)
        and prereg.get("allowed_data_identity_sets_sha256") == allowed_sets_sha256
        ...
        raise PermissionError("Gen5r6 owner preregistration semantics differ")
    preseal = _strict_json(paths["preseal"])
    if not (
        preseal.get("schema_version") == "p1_gen5r6_owner_preseal_static_qa.v1"
        ...
        and preseal.get("data_identity_lineage") == identity_lineage
        and preseal.get("r5_source_content_equivalence_sha256")
        == _deep_sha(content_equivalence)
        and preseal.get("allowed_data_identity_sets_sha256") == allowed_sets_sha256
        ...
        raise PermissionError("Gen5r6 owner preseal semantics differ")
    return {
...
class _OperationalContext:
    ...
    def __init__(
        ...
        *,
        ...
        r5_content_equivalence: dict[str, Any],
        ...
        self.r5_content_equivalence = r5_content_equivalence
        self.identity_lineage = identity_lineage
        self.core_snapshot = {
            "schema_version": "p1_gen5r6_operational_core_snapshot.v1",
            "config": {
            ...
            "r5_tombstone": _verify_r5_tombstone(paths),
            "r5_source_content_equivalence": r5_content_equivalence,
...
def _load_operational_context(
    *,
    ...
    if hashlib.sha256(content).hexdigest() != EXPECTED_CONFIG_SHA256:
        raise PermissionError("canonical Gen5r6 config byte SHA differs")
    overlay = _strict_json(paths["config"])
    if _deep_sha(overlay) != EXPECTED_CONFIG_DEEP_SHA256:
        raise PermissionError("canonical Gen5r6 config deep JSON differs")
    if not (
        overlay.get("schema_version")
        == "p1_incumbent_rule_distillation_neural_residual.v5r6"
        ...
        is True
        and overlay["provenance_correction_contract"]["scientific_structure_changed"]
        is False
        and overlay["provenance_correction_contract"]["known_data_identity_set_count"]
        == 2
        and overlay["provenance_correction_contract"]["complete_atomic_set_match_only"]
        is True
        and tuple(overlay["allowed_data_identity_sets"])
        == _ALLOWED_DATA_IDENTITY_SET_IDS
    ):
        raise PermissionError("Gen5r6 identity, environment, comparator, or capability contract differs")
    _verify_r5_tombstone(paths)
    ...
    r5_content_equivalence = _verify_r5_source_content_equivalence(paths, overlay)
    identity_lineage = _verify_identity_lineage(paths, overlay)
    ...
    return _OperationalContext(
        ...
        r5_content_equivalence=r5_content_equivalence,
        identity_lineage=identity_lineage,
    )
...
def _reload_context(context: _OperationalContext) -> _OperationalContext:
    ...
    if current.core_snapshot != context.core_snapshot:
        raise PermissionError("Gen5r6 operational core snapshot changed")
    return current
...
def _verify_independent_qa(
    ...
    if not paths["qa_receipt"].is_file():
        raise PermissionError("canonical Gen5r6 independent-QA receipt is missing")
    ...
    if set(qa) != required:
        raise PermissionError("Gen5r6 independent-QA receipt keys differ")
    if not (
        qa["schema_version"] == "p1_gen5r6_independent_static_qa.v1"
        and qa["problem"] == "P1"
        ...
        raise PermissionError("Gen5r6 independent-QA receipt did not authorize GO")
...
def _verify_execution_authorization(
    ...
    *,
    ...
    if require_lock_absent and paths["lock"].exists():
        raise FileExistsError("Gen5r6 one-shot attempt lock already exists")
    if not paths["authorization"].is_file():
        raise PermissionError("separate Gen5r6 execution authorization is missing")
    ...
    if set(authorization) != required:
        raise PermissionError("Gen5r6 execution authorization keys differ")
    runner_pin = {
        "path": CANONICAL_RUNNER,
        "sha256": _sha(paths["runner"]),
    }
    phrase = (
        f"AUTHORIZE_P1_GEN5R6_EXECUTION:{EXPECTED_CONFIG_SHA256}:"
        f"{runner_pin['sha256']}"
    )
    if not (
        authorization["schema_version"]
        == "p1_gen5r6_execution_authorization.v1"
        ...
        raise PermissionError("Gen5r6 execution authorization failed")
...
class _PreLockCapability:
    ...
    def __init__(self, mint: object, token: object) -> None:
        if mint is not _PRELOCK_MINT:
            raise PermissionError("canonical Gen5r6 pre-lock capability mint is required")
        self._token = token
...
class _ExecutionCapability:
    ...
    def __init__(self, mint: object, token: object) -> None:
        if mint is not _EXECUTION_MINT:
            raise PermissionError("canonical Gen5r6 post-lock capability mint is required")
        self._token = token
...
def _mint_prelock_capability(
    ...
    *,
    ...
    if _LIVE_PRELOCK or _LIVE_EXECUTION:
        raise PermissionError("a canonical Gen5r6 capability is already live")
    token = object()
...
def authorize_entry(
    *,
    ...
    if context.paths["artifact"].exists() or context.paths["lock"].exists():
        raise FileExistsError("Gen5r6 artifact or attempt lock already exists")
    ...
    if (
        ...
        raise PermissionError("Gen5r6 QA or authorization changed before pre-lock mint")
...
def _require_prelock_capability(
    ...
    *,
    ...
    if not isinstance(capability, _PreLockCapability):
        raise PermissionError("canonical opaque Gen5r6 pre-lock capability is required")
    ...
    if (
        ...
        raise PermissionError("forged or stale Gen5r6 pre-lock capability")
    if state["stage"] != expected_stage:
        raise PermissionError(f"Gen5r6 pre-lock stage must be {expected_stage}")
    ...
    if not (
        ...
        raise PermissionError("Gen5r6 pre-lock operational snapshot changed")
    return state, current


def _canonical_lock_payload(state: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "p1_gen5r6_attempt_lock.v1",
        "status": "ATTEMPT_CONSUMED_ONE_SHOT",
        "experiment_id": "p1_incumbent_rule_distillation_neural_residual_v5r6",
        "config_sha256": EXPECTED_CONFIG_SHA256,
...
def _verify_consumed_lock(
    ...
    if state.get("mint") not in {_PRELOCK_STATE_MINT, _EXECUTION_STATE_MINT}:
        raise PermissionError("forged Gen5r6 capability state")
    _, qa_sha256 = _verify_independent_qa(context)
    _, authorization_sha256 = _verify_execution_authorization(
        context,
        qa_sha256=qa_sha256,
    ...
    if (
        ...
        raise PermissionError("Gen5r6 post-lock QA or authorization differs")
    lock_path = context.paths["lock"]
    if not lock_path.is_file():
        raise PermissionError("canonical consumed Gen5r6 attempt lock is missing")
    payload = _strict_json(lock_path)
    expected = _canonical_lock_payload(state)
    if payload != expected:
        raise PermissionError("canonical consumed Gen5r6 attempt lock payload differs")
    digest = _sha(lock_path)
    if state.get("lock_sha256") not in {None, digest}:
        raise PermissionError("canonical consumed Gen5r6 attempt lock SHA differs")
...
def _require_execution_capability(
    ...
    *,
    ...
    if not isinstance(capability, _ExecutionCapability):
        raise PermissionError("canonical live post-lock Gen5r6 capability is required")
    ...
    if (
        ...
        raise PermissionError("forged, stale, or replayed Gen5r6 execution capability")
    if state["stage"] != expected_stage:
        raise PermissionError(f"Gen5r6 execution stage must be {expected_stage}")
    current = _reload_context(state["context"])
    lock = _verify_consumed_lock(state, current)
    if lock["sha256"] != state["lock_sha256"]:
        raise PermissionError("Gen5r6 consumed lock changed")
...
class _BlindCommitmentLedger:
    ...
    def commit_cell(
        ...
        *,
        ...
        if self._expected[sequence - 1] != (fold, fraction):
            raise PermissionError("Gen5r6 cell commitment order is not fold-major monotone")
        if [row["seed"] for row in seed_blind_predictions] != list(SEEDS):
            raise PermissionError("Gen5r6 cell commitment seed order differs")
        ...
        payload = {
            "schema_version": "p1_gen5r6_blind_cell_commitment.v1",
            "sequence": sequence,
        ...
        if len(fold_cells) == len(FRACTIONS):
            ...
            fold_payload = {
                "schema_version": "p1_gen5r6_blind_fold_commitment.v1",
                "fold": fold,
    ...
    def finalize_ledger(self) -> dict[str, Any]:
        if len(self._cells) != 15 or tuple(self._folds) != FOLD_ORDER:
            raise PermissionError("Gen5r6 blind ledger is incomplete")
        relative = "blind_commitments/ledger_complete.json"
        path = self.artifact / relative
        payload = {
            "schema_version": "p1_gen5r6_blind_commitment_ledger.v1",
    ...
    def mark_predictions_complete(self, path: Path) -> None:
        ...
        if not (
            ...
            raise PermissionError("Gen5r6 predictions_complete did not bind every blind prediction")
...
def _run_curve(
    *,
    ...
    if not isinstance(runtime, dict) or runtime.get("sealed") is not True:
        raise PermissionError("Gen5r6 bound curve runtime is absent")
    ...
    if tuple(str(fold["name"]) for fold in folds) != FOLD_ORDER:
        raise PermissionError("Gen5r6 outer fold order differs")
    ...
    for fold in folds:
        ...
        if not np.array_equal(validation_ids, targets.validation_rows(fold_name)):
            raise PermissionError("Gen5r6 selective accessor validation IDs differ")
        for fraction in FRACTIONS:
            ...
            if np.intersect1d(train_ids, validation_ids).size:
                raise PermissionError("Gen5r6 active train/validation IDs overlap")
            ...
            if any(
                ...
                raise PermissionError("Gen5r6 planned and executed inner splits differ")
            ...
            if working_train.iloc[gate_ids]["label"].notna().any():
                raise PermissionError("Gen5r6 gate labels were decoded before blind prediction")
            ...
            for gate_seed in SEEDS:
                ...
                if not np.array_equal(gate_probability, reproduced_gate):
                    raise RuntimeError("saved Gen5r6 gate residual did not reproduce")
                gate_blind_relative = (
            ...
            if tuple(gate_stages) != SEEDS or any(
                ...
                raise PermissionError("Gen5r6 did not seal all three gate predictions")
            if working_train.iloc[gate_ids]["label"].notna().any():
                raise PermissionError("Gen5r6 gate labels changed before all blind seals")
            ...
            for seed in SEEDS:
                ...
                if not reload_exact:
                    raise RuntimeError("saved Gen5r6 refit did not reproduce blind probability")
                ...
                if gate_result["passed"]:
                    ...
                    if not (
                        ...
                        raise AssertionError("failed Gen5r6 gate changed incumbent seed")
                blind_relative = (
            ...
            if not any(cell_gate_passes):
                ...
                if not (
                    ...
                    raise AssertionError("all-failed Gen5r6 gates changed incumbent ensemble")
            else:
        ...
        if not commitment.is_fold_committed(fold_name):
            raise PermissionError("Gen5r6 fold did not reach its five-cell commitment")
    if not (
        ...
        raise AssertionError("Gen5r6 curve fit, part, or optimizer-step count differs")
    ledger_receipt = commitment.finalize_ledger()
    completion = {
        "schema_version": "p1_incumbent_residual_predictions_complete.v5r6",
...
def _full_fit_models(
    *,
    ...
    if not isinstance(runtime, dict) or runtime.get("labels_committed") is not True:
        raise PermissionError("Gen5r6 bound full-fit runtime is absent")
...
def _run_after_lock(
    *,
    ...
    if paths["artifact"].exists():
        raise FileExistsError("Gen5r6 artifact already exists")
    _patch_pure_engine_writer()
    if _verify_execution_closure(root, overlay) != closure_before:
        raise PermissionError("Gen5r6 closure changed after lock")
    if _capture_anchor_snapshot(root, data_dir, overlay) != anchors_before:
        raise PermissionError("Gen5r6 anchor snapshot changed after lock")
    ...
    if pins_before != input_pins_before:
        raise PermissionError("Gen5r6 immutable-input snapshot changed after lock")
    paths["artifact"].mkdir(parents=True, exist_ok=False)
    _json_new(
        paths["artifact"] / "preregistration.json",
        {
            "schema_version": "p1_gen5r6_runtime_preregistration.v1",
    ...
    if len(train) == 0 or {"label", "anomaly_type"}.intersection(train.columns):
        raise PermissionError("Gen5r6 input-only train load differs")
    ...
    if any(column not in feature_metadata["feature_columns"] for column in feature_columns):
        raise ValueError("registered Gen5r6 causal feature column is absent")
    ...
    if len(feature_frame) != len(train) or {"label", "anomaly_type"}.intersection(
        ...
        raise PermissionError("Gen5r6 feature cache row or target exclusion differs")
    ...
    if tuple(str(fold["name"]) for fold in folds) != FOLD_ORDER:
        raise PermissionError("Gen5r6 frozen outer fold order differs")
    ...
    if targets.decoded_target_scalars != 0:
        raise PermissionError("Gen5r6 target accessor decoded targets during indexing")
    _json_new(
    ...
    if targets.decoded_anomaly_rows != 0:
        raise PermissionError("Gen5r6 decoded anomaly_type despite binary-only structure")
    ...
    if closure_before != closure_after:
        raise RuntimeError("Gen5r6 executable closure changed during the run")
    ...
    if pins_before != pins_after:
        raise RuntimeError("Gen5r6 immutable inputs changed during the run")
    anchors_after = _capture_anchor_snapshot(root, data_dir, overlay)
    if anchors_before != anchors_after:
        raise RuntimeError("Gen5r6 anchor identities changed during the run")
    current = _reload_context(context)
    verified_lock = _verify_consumed_lock(state, current)
    if verified_lock["sha256"] != lock["sha256"]:
        raise RuntimeError("Gen5r6 consumed attempt lock changed during the run")
    result = {
        "schema_version": "p1_incumbent_residual_result.v5r6",
        "experiment_id": overlay["experiment_id"],
    ...
    registry = {
        "schema_version": "p1_incumbent_residual_registry.v5r6",
        "experiment_id": overlay["experiment_id"],
    ...
    manifest = {
        "schema_version": "p1_incumbent_residual_manifest.v5r6",
        "experiment_id": overlay["experiment_id"],
        "created_at": engine._now(),
        "config_sha256": EXPECTED_CONFIG_SHA256,
    ...
    engine._emit("gen5r6_generation_complete", **final)
...
def check_only(
    *,
    ...
    current_data_identity_set, _ = _capture_live_data_identity(
        data_dir,
        allowed_sets=_validate_allowed_data_identity_sets(overlay),
    )
    ...
    for fold in folds:
        ...
        for fraction in FRACTIONS:
            ...
            if (
                ...
                raise PermissionError("Gen5r6 static split firewall differs")
            split_cells.append(
                {
                    "fold": fold_name,
                    "fraction": fraction,
    ...
    if targets.decoded_target_scalars != 0 or targets.decoded_anomaly_rows != 0:
        raise PermissionError("Gen5r6 check-only decoded a target scalar")
    ...
    if not missing_qa_rejected:
        raise PermissionError("Gen5r6 authorize_entry did not reject missing QA/auth")
    ...
    for name, call in guarded_calls.items():
        try:
            ...
            raise PermissionError(f"Gen5r6 direct-call guard failed: {name}")
    if _LIVE_PRELOCK or _LIVE_EXECUTION:
        raise PermissionError("Gen5r6 check-only left a live capability")
    if paths["lock"].exists() or paths["artifact"].exists():
        raise PermissionError("Gen5r6 check-only created an execution output")
    ...
    if control_state != {
        ...
        raise PermissionError("Gen5r6 static-only control state differs")
    return {
        "status": "CANONICAL_GEN5R6_CHECK_ONLY_PASS",
        "experiment_id": overlay["experiment_id"],
        "config_sha256": EXPECTED_CONFIG_SHA256,
        "config_deep_json_sha256": EXPECTED_CONFIG_DEEP_SHA256,
        ...
        "current_data_identity_set": current_data_identity_set,
        "allowed_data_identity_set_ids": list(_ALLOWED_DATA_IDENTITY_SET_IDS),
        ...
        "owner_seals": owner_seals,
        "r5_tombstone": _verify_r5_tombstone(paths),
        "r5_source_content_equivalence": context.r5_content_equivalence,
        "identity_lineage": context.identity_lineage,
        "allowed_data_identity_sets_sha256": _deep_sha(
            _validate_allowed_data_identity_sets(overlay)
        ),
...
def main() -> int:
    ...
    if args.check_only:
        ...
        raise AssertionError("unreachable Gen5r6 mode")
    print(
        json.dumps(
            result,
            ensure_ascii=False,
