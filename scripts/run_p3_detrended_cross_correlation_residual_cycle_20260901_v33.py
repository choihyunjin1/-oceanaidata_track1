"""Sealed P3 v33 detrended cross-correlation residual experiment."""
...
EXPERIMENT_ID = "p3_detrended_cross_correlation_residual_cycle_20260901_v33"
...
SCALES = (12, 24, 48, 96)
PAIRED_CHANNELS = tuple(range(1, 12))
FEATURE_COUNT = 66
SPECS = (
    v26.Spec("P3_1_DCCA66_RIDGE512_ADD10", 512.0),
    v26.Spec("P3_2_DCCA66_RIDGE2048_ADD10", 2048.0),
)
BLEND = 0.10


class ContractError(RuntimeError):
    """Raised when the sealed v33 contract differs."""
...
def load_config() -> dict[str, Any]:
    ...
    checks = {
        ...
        == "p3.detrended_cross_correlation_residual.config.v33",
        ...
        "scales": tuple(encoder["fixed_scales"]) == SCALES,
        "pairs": tuple(encoder["paired_channels"]) == PAIRED_CHANNELS,
    ...
    if not all(checks.values()):
        raise ContractError(f"v33 config contract failed: {checks}")
...
def detrended_covariance(
    first_profile: np.ndarray, second_profile: np.ndarray, scale: int
) -> tuple[float, float, float]:
    n_rows = len(first_profile)
    count = n_rows // scale
    starts = [index * scale for index in range(count)] + [
        n_rows - (index + 1) * scale for index in range(count)
    ]
    axis = np.arange(scale, dtype=np.float64)
    first_power: list[float] = []
    second_power: list[float] = []
    cross_power: list[float] = []
    design = np.column_stack([axis, np.ones(scale, dtype=np.float64)])
    for start in starts:
        first = first_profile[start : start + scale]
        second = second_profile[start : start + scale]
        first_residual = first - design @ np.linalg.lstsq(design, first, rcond=None)[0]
        second_residual = second - design @ np.linalg.lstsq(
            design, second, rcond=None
        )[0]
        first_power.append(float(np.mean(np.square(first_residual))))
        second_power.append(float(np.mean(np.square(second_residual))))
        cross_power.append(float(np.mean(first_residual * second_residual)))
    return (
        float(np.mean(first_power)),
        float(np.mean(second_power)),
        float(np.mean(cross_power)),
    )


def dcca_pair_features(first: np.ndarray, second: np.ndarray) -> np.ndarray:
    first = np.asarray(first, dtype=np.float64)
    second = np.asarray(second, dtype=np.float64)
    if first.shape != second.shape or first.shape != (289,):
        raise ContractError("DCCA pair shape differs")
    first_profile = np.cumsum(first - first.mean())
    second_profile = np.cumsum(second - second.mean())
    correlations: list[float] = []
    cross_fluctuations: list[float] = []
    signs: list[float] = []
    for scale in SCALES:
        first_power, second_power, cross_power = detrended_covariance(
            first_profile, second_profile, scale
        )
        denominator = np.sqrt(max(first_power * second_power, 0.0))
        correlation = 0.0 if denominator <= 1e-15 else cross_power / denominator
        correlations.append(float(np.clip(correlation, -1.0, 1.0)))
        cross_fluctuations.append(float(np.sqrt(abs(cross_power)) + 1e-12))
        signs.append(float(np.sign(cross_power)))
    slope = float(
        np.polyfit(np.log(np.asarray(SCALES)), np.log(cross_fluctuations), 1)[0]
    )
    output = np.asarray([*correlations, slope, np.mean(signs)], dtype=np.float64)
    if output.shape != (6,) or not np.isfinite(output).all():
        raise ContractError("DCCA pair feature contract differs")
    return output


def dcca_features(sequence: np.ndarray) -> np.ndarray:
    path = v26.transformed_path(sequence)
    features = np.concatenate(
        [dcca_pair_features(path[:, 0], path[:, column]) for column in PAIRED_CHANNELS]
    )
    if features.shape != (FEATURE_COUNT,) or not np.isfinite(features).all():
        raise ContractError("DCCA feature contract differs")
    return features
...
def synthetic_receipt() -> dict[str, Any]:
    ...
    feature = dcca_features(sequence)
    return {
...
def preflight_payload() -> dict[str, Any]:
    ...
    if ARTIFACT.exists() or LOCK.exists():
        raise ContractError("v33 exactly-once namespace is consumed")
    payload = {
        "schema_version": "p3.detrended_cross_correlation_residual.preflight.v33",
...
def surface_features(cases: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    for position, row in enumerate(cases.itertuples(index=False)):
        ...
        features[position] = dcca_features(sequences[anchor_id])
    return features, {
        "rows": len(features),
...
def execute(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    result = {
        "schema_version": "p3.detrended_cross_correlation_residual.result.v33",
        "experiment_id": EXPERIMENT_ID,
        "created_at_utc": datetime.now(UTC).isoformat(),
        "status": "COMPLETE",
        "decision": "PASS_CANDIDATE_AVAILABLE"
        if passing
        else "NO_GO_ALL_DCCA_CANDIDATES",
...
def main() -> int:
    ...
    if ARTIFACT.exists() or REPORT.exists() or LOCK.exists():
        raise ContractError("v33 exactly-once namespace already exists")
    config = load_config()
    ...
    write_new(
        ...
        b"# Gap matrix\n\n| Audited axis | Verdict |\n|---|---|\n| univariate multifractal DFA | semantic proximity to closed wavelet/scattering and spectral families; zero fit |\n| detrended cross-correlation scaling | nonduplicate symmetric cross-scale covariance axis; executed as v33 |\n",
    )
    write_new(
        REPORT / "claim-source-ledger.md",
        b"# Claim-source ledger\n\n| Claim | Source | Use |\n|---|---|---|\n| DCCA measures power-law cross-correlations between nonstationary series through detrended covariance | Podobnik and Stanley, Physical Review Letters 100, 084102 (2008), DOI:10.1103/PhysRevLett.100.084102 | fixed cross-scale covariance representation |\n| No corresponding P3 implementation exists | repository semantic audit before sealing | novelty gate |\n",
    )
    write_new(
