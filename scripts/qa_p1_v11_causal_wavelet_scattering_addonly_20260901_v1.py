"""Lifecycle-safe independent post-terminal QA for P1 v11 scattering."""
...
def _module():
    spec = importlib.util.spec_from_file_location("p1_v11_qa_runner", RUNNER)
...
def verify(data_dir: Path) -> dict[str, object]:
    ...
    for fold in config["parts"]:
        ...
        metadata_parts.append(frame.iloc[positions].loc[:, ["station", "layer", "time"]].reset_index(drop=True))
    ...
    truth, incumbent, candidate, addition = (
        np.concatenate(values) for values in (truths, incumbents, candidates, additions)
    )
    ...
    long_event = runner.base._long_event_interior(truth, incumbent, candidate, metadata)
    checks = {
        "sealed_hashes": all(hashes),
        "nine_unique_models": complete["fits"] == 9 and len(set(models)) == 9,
        "outer_isolation": all(isolation) and complete["outer_target_reads_before_all_seals"] == 0,
        "add_only": np.array_equal(candidate, np.bitwise_or(incumbent, addition.astype(np.int8))) and not (incumbent > candidate).any(),
        "anchor_score": all(anchor_score[key] == result["pooled"]["incumbent"][key] for key in ("tp", "fp", "fn", "f1")),
        "candidate_score": all(candidate_score[key] == result["pooled"]["candidate"][key] for key in ("tp", "fp", "fn", "f1")),
        "delta": abs(candidate_score["f1"] - anchor_score["f1"] - result["pooled"]["delta_f1"]) < 1e-15,
        ...
        "schema": result["schema_version"] == config["result_schema_version"],
    ...
    return {
        ...
        "checks": checks,
        "recomputed": {"anchor": anchor_score, "candidate": candidate_score, "delta_f1": candidate_score["f1"] - anchor_score["f1"], "additions": int(addition.sum()), "addition_true_positive": int(truth[addition].sum()), "long_event_interior": long_event},
