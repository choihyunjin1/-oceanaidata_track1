...
RUN = "p1_clean_regeneration_20260905_v5"
...
def source_contract():
    ...
    paths = set(canonical.SOURCE_FILES) | set(cfg["source_pins"]) | {
        "scripts/verify_p1_clean_regeneration_20260905_v5.py"
    }
...
def fit_pair(training, evaluation, stage, frozen, receipt, started):
    ...
    eval_bundle = (
        screen.feature_pair(evaluation, stats, frozen)[0] if evaluation is not None else None
    )
    ...
    for name in order:
        ...
        eval_features = encoder.transform(eval_bundle) if eval_bundle is not None else None
...
def train():
    ...
    receipt = {
        ...
        "resource_amendment": "Restore canonical CPU4; inner B then O, full O then B; one diagnostic attempt",
        "source_hashes": hashes, "max_fits": MAX_FITS, "wall_cap_seconds": WALL_SECONDS,
        "versions": {k: canonical.importlib.metadata.version(k) for k in
                     ("numpy", "pandas", "scikit-learn", "lightgbm", "xgboost", "joblib")},
        "seed": frozen["seed"],
        "fit_order": ["inner_balanced", "inner_original", "full_original", "full_balanced"],
    ...
    try:
        ...
        gc.collect()
        fit_pair(frame.drop(columns="row_id"), None, "full", frozen, receipt, started)
...
def infer(verify=False):
    ...
    bits, _, _ = canonical.decoder.control_components(
        evaluation, probabilities, screen.rule_masks(evaluation, stats),
        recipe["feature_contract"], recipe["selection"], recipe["calibrations"], 1e-6
    )
