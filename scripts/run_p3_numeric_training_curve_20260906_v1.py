"""Held-inner schedule selection followed by immutable five-fold numeric P3 QA."""
...
NAME = "p3_numeric_training_curve_20260906_v1"
...
def validate_config(cfg):
    ...
    if cfg["budget"]["max_all_new_historical_fits"] != 20 or cfg["budget"]["wall_seconds"] != 3600:
...
def execute(cfg, previous):
    ...
    def progress(stage):
        ...
        if time.perf_counter() >= deadline:
            raise TimeoutError("60min fixed fit budget expired")

    timer = threading.Timer(3600, lambda: os._exit(124))
    timer.daemon = True
    timer.start()
    try:
        for arm in cfg["recipes"]:
            ...
            if arm["id"] == "baseline":
                ...
                core.save(
                    ...
                    {
                        "status": "CONTINUE" if estimate < 3600 else "RESOURCE_STOP",
                ...
                if estimate >= 3600:
                    raise TimeoutError("pilot forecast exceeds60min before outcome inspection")
...
def qa(cfg, previous):
    ...
    check("time/cap", result["execution_seconds"] < 3600)
    check(
        "replay/new_pid",
...
def main():
    ...
    core.guard(source, OUT, previous, e, official=False)
