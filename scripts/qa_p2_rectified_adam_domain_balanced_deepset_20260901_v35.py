"""Independent QA for sealed P2 v35 fixed-RAdam run."""
...
import run_p2_rectified_adam_domain_balanced_deepset_20260901_v35 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            ...
            observed_steps.append(fit["optimizer_steps"])
            ...
            fit_receipts_ok &= optimizer["class"] == "RAdam"
            fit_receipts_ok &= optimizer["learning_rate"] == 0.001
            fit_receipts_ok &= optimizer["betas"] == [0.9, 0.999]
            fit_receipts_ok &= optimizer["epsilon"] == 1e-8
            fit_receipts_ok &= optimizer["weight_decay"] == 0.0001
            fit_receipts_ok &= optimizer["decoupled_weight_decay"] is True
            fit_receipts_ok &= optimizer["foreach"] is False
            fit_receipts_ok &= optimizer["state_count"] == 10
            fit_receipts_ok &= optimizer["state_step_min"] == fit["optimizer_steps"]
            fit_receipts_ok &= optimizer["state_step_max"] == fit["optimizer_steps"]
            fit_receipts_ok &= optimizer["state_finite"] is True
            fit_receipts_ok &= fit["warmup_steps"] == 0
            fit_receipts_ok &= fit["scheduler_steps"] == 0
            fit_receipts_ok &= fit["lookahead_steps"] == 0
            fit_receipts_ok &= fit["gradient_projection_steps"] == 0
    ...
    radam = runner._radam_contract_receipt(config)
    ...
    checks = {
        ...
        is False,
        "radam_contract": radam["contract_exact"] and radam["deterministic"],
        ...
        "fit_receipts_valid": fit_receipts_ok and bool(observed_steps),
    ...
    qa = {
        "schema_version": "p2.rectified_adam_domain_balanced_deepset.independent_qa.20260901.v35",
