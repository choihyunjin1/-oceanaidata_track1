"""Exactly-once P3 directed transfer-entropy residual cycle."""
...
FEATURE_COUNT = 180
SPECS = (
    v26.Spec("P3_1_DTE180_RIDGE512_ADD10", 512.0),
    v26.Spec("P3_2_DTE180_RIDGE2048_ADD10", 2048.0),
)
BLEND = 0.10


class ContractError(RuntimeError):
    """Raised when the sealed v30 contract differs."""
...
def load_config() -> dict[str, Any]:
    ...
    checks = {
        ...
        == "p3.directed_transfer_entropy_residual.config.v30",
        ...
        "pairs": tuple(tuple(item) for item in encoder["fixed_directed_pairs"])
        == PAIRS,
        "lags": tuple(encoder["lags"]) == LAGS,
        "features": encoder["feature_count"] == FEATURE_COUNT,
        "bins": encoder["symbol_bins"] == 3,
        "specs": tuple(
        ...
        "no_posthoc": not config["duplication_audit"][
            "posthoc_v27_v28_v29_adjustment"
        ],
    }
    if not all(checks.values()):
        raise ContractError(f"v30 config contract failed: {checks}")
...
def symbols(values: np.ndarray) -> np.ndarray:
    """Stable empirical thirds, fixed per past-only case/channel."""
    values = np.asarray(values, dtype=np.float64)
    order = np.argsort(values, kind="stable")
    ranks = np.empty(len(values), dtype=np.int64)
    ranks[order] = np.arange(len(values), dtype=np.int64)
    return np.minimum((3 * ranks) // len(values), 2)


def transfer_entropy(source: np.ndarray, target: np.ndarray, lag: int) -> float:
    """Plug-in I(source[t-lag]; target[t] | target[t-lag]) in nats."""
    source = np.asarray(source, dtype=np.int64)
    target = np.asarray(target, dtype=np.int64)
    source_past = source[:-lag]
    target_past = target[:-lag]
    target_now = target[lag:]
    triple = np.zeros((3, 3, 3), dtype=np.float64)
    for x_value, y_past, y_now in zip(
        source_past, target_past, target_now, strict=True
    ):
        triple[x_value, y_past, y_now] += 1.0
    total = float(triple.sum())
    joint_xy = triple.sum(axis=2)
    joint_yy = triple.sum(axis=0)
    marginal_y = joint_yy.sum(axis=1)
    result = 0.0
    for x_value, y_past, y_now in np.argwhere(triple > 0):
        count = triple[x_value, y_past, y_now]
        numerator = count * marginal_y[y_past]
        denominator = joint_xy[x_value, y_past] * joint_yy[y_past, y_now]
        result += (count / total) * np.log(numerator / denominator)
    return float(result)


def directed_information_features(sequence: np.ndarray) -> np.ndarray:
    path = v26.transformed_path(sequence)
    encoded = np.column_stack([symbols(path[:, column]) for column in range(12)])
    output: list[float] = []
    for source, target in PAIRS:
        for lag in LAGS:
            forward = transfer_entropy(encoded[:, source], encoded[:, target], lag)
            reverse = transfer_entropy(encoded[:, target], encoded[:, source], lag)
            output.extend((forward, reverse, forward - reverse))
    features = np.asarray(output, dtype=np.float64)
    if features.shape != (FEATURE_COUNT,) or not np.isfinite(features).all():
        raise ContractError("directed-information feature contract differs")
    return features
...
def synthetic_receipt() -> dict[str, Any]:
    ...
    feature = directed_information_features(sequence)
    return {
...
def preflight_payload() -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "p3.directed_transfer_entropy_residual.preflight.v30",
...
def surface_features(cases: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    for position, row in enumerate(cases.itertuples(index=False)):
        ...
        features[position] = directed_information_features(sequences[anchor_id])
    return features, {
        "rows": len(features),
...
def execute(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    result = {
        "schema_version": "p3.directed_transfer_entropy_residual.result.v30",
        "experiment_id": EXPERIMENT_ID,
        "created_at_utc": datetime.now(UTC).isoformat(),
        "status": "COMPLETE",
        "decision": "PASS_CANDIDATE_AVAILABLE"
        if passing
        else "NO_GO_ALL_DIRECTED_INFORMATION_CANDIDATES",
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        "# P3 directed transfer-entropy residual cycle v30",
        "",
        "## 결론",
        "",
        f"- overall decision: **{result['decision']}**.",
        "- Energy-flux and source-decomposition proposals were rejected before fit as semantic duplicates.",
        "- v30 uses fixed directional conditional-information features; the 182-case surface remains EXPLORATORY_ONLY.",
    ]
...
def main() -> int:
    ...
    if ARTIFACT.exists() or REPORT.exists() or LOCK.exists():
        raise ContractError("v30 exactly-once namespace already exists")
    config = load_config()
    ...
    write_new(
        ...
        b"# Gap matrix\n\n| Audited axis | Verdict |\n|---|---|\n| energy-flux/tendency projection | semantic duplicate of prior energy/wind-work families |\n| source-model error decomposition | semantic duplicate of router/disagreement/stacking families |\n| directed transfer entropy | nonduplicate; executed as v30 |\n",
    )
    write_new(
        REPORT / "claim-source-ledger.md",
        b"# Claim-source ledger\n\n| Claim | Source | Use |\n|---|---|---|\n| Transfer entropy measures directional information transfer beyond target history | Schreiber, 2000, DOI:10.1103/PhysRevLett.85.461 | fixed directed-information feature basis |\n| No corresponding P3 implementation exists | repository semantic audit before sealing | novelty gate |\n",
    )
    write_new(
