"""Independent QA for sealed P2 v36 layer-month Fishr run."""
...
import run_p2_layer_month_fishr_gradient_variance_deepset_20260901_v36 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        environment_receipt = receipt["weight_receipt"]["fishr_environment_contract"]
        ...
        for fit in receipt["fit_receipts"]:
            finite_values = [
                fit["base_loss_first"],
                fit["base_loss_last"],
                fit["fishr_penalty_first"],
                fit["fishr_penalty_last"],
            ...
            fit_receipts_ok &= fit["fishr_penalty_steps"] == fit["optimizer_steps"]
            fit_receipts_ok &= fit["coefficient"] == 1.0
            fit_receipts_ok &= fit["supported_environment_count_min"] >= 2
            fit_receipts_ok &= fit["supported_environment_count_max"] >= 2
            fit_receipts_ok &= fit["final_head_gradient_dimension"] == 33
            fit_receipts_ok &= fit["variance_correction"] == 0
    ...
    fishr = runner._fishr_contract_receipt()
    ...
    checks = {
        ...
        "new_fishr_axis": semantic["classification"]
        == "NEW_P2_LAYER_MONTH_FINAL_HEAD_DIAGONAL_FISHR",
        ...
        is False,
        "fishr_contract": fishr["permutation_invariant"]
        and fishr["gradients_finite"]
        and fishr["penalty_finite"],
        "forward_contract": forward["prediction_maximum_abs_error"] <= 1e-7
        and forward["final_head_gradient_dimension"] == 33,
    ...
    qa = {
        "schema_version": "p2.layer_month_fishr_gradient_variance_deepset.independent_qa.20260901.v36",
