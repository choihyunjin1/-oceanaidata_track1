"""Crash-safe, sealed P2 NCR_LGBM Stage 1 v1r2 parent/worker runner.

The default mode is read-only strict preflight.  ``--execute-stage1`` first
creates a stable exclusive claim and durable append-only attempt journal, then
launches this same sealed file as a numerical worker under a 1,800-second
parent-enforced wall clock.  There is no retry or resume path.  Official test,
sample-submission, submission-candidate, CSV, and upload code paths do not
exist.
"""
...
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/experiments/p2_normalized_curvature_residual_lgbm_stage1_v1r2.json"
)
EXPECTED_CONFIG_SHA256 = "8880efb45f2ccdaf8864769106c84a0d3911733c25dca05965d4b29020cb88b2"
...
WORKER_TOKEN_ENV = "P2_NCR_V1R2_ATTEMPT_TOKEN"
WORKER_DEADLINE_ENV = "P2_NCR_V1R2_DEADLINE_EPOCH"
WORKER_PARENT_PID_ENV = "P2_NCR_V1R2_PARENT_PID"
    ...
    for name, pin in pins.items():
        ...
        if name == runner_name:
            if path != runner_path or mode != RUNNER_HASH_MODE:
                raise RuntimeError("v1r2 runner pin contract drift")
...
def _verify_static_bundle(config_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if config_sha256 != EXPECTED_CONFIG_SHA256:
        raise RuntimeError("NCR_LGBM v1r2 preregistration hash drift")
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if not isinstance(config, dict):
        raise TypeError("v1r2 preregistration must be a JSON object")
    if config.get("schema_version") != (
        "p2_normalized_curvature_residual_lgbm_stage1.prereg.v1r2"
    ):
        raise ValueError("unexpected v1r2 preregistration schema")
    if config.get("experiment_id") != (
        "p2_normalized_curvature_residual_lgbm_stage1_20260826_v1r2"
    ):
        raise ValueError("unexpected v1r2 experiment id")
    if config.get("status") != "PREREGISTERED_NOT_EXECUTED_SUPERSEDES_V1":
        raise ValueError("v1r2 is not in its sealed preregistered state")
...
def _assert_static_bundle_unchanged(
    ...
    if observed_config != expected_config or observed_bundle != expected_bundle:
        raise RuntimeError("v1r2 static bundle changed after preflight")
...
def _acquire_attempt(
    ...
    claim = {
        "schema_version": "p2_ncr_stage1_execution_claim.v1r2",
...
def _validate_worker_claim(
    ...
    if len(events) != 1 or events[0].get("event") != "ATTEMPT_CLAIMED":
        raise RuntimeError("internal worker requires a pristine claimed journal")
    return {
        "token_sha256": token_sha256,
        "deadline_epoch": deadline_epoch,
        "parent_pid": int(parent_pid_text),
...
def _run_internal_worker(
    ...
    if not data_dir_text:
        raise RuntimeError("P2_DATA_DIR is required for the internal worker")
    observations_path = Path(data_dir_text).resolve() / "observations.csv"
    _assert_safe_input_path(observations_path)
    if observations_path.name != "observations.csv" or not observations_path.is_file():
    ...
    with _held_verified_bytes(
        ...
        worker_result: dict[str, Any] = {
            "schema_version": "p2_ncr_stage1_worker.result.v1r2",
            "experiment_id": config["experiment_id"],
            "contract_sha256": authorization["contract_sha256"],
            "bundle_sha256": authorization["bundle_sha256"],
...
def _execute_parent(
    ...
    attempt = _acquire_attempt(config, bundle)
    ...
    result: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.result.v1r2",
        "experiment_id": config["experiment_id"],
        "status": (
    ...
    manifest: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.manifest.v1r2",
        "experiment_id": config["experiment_id"],
...
def main() -> int:
    ...
    if args.internal_worker:
        try:
            result = _run_internal_worker(config_path, config, bundle)
        except BaseException as error:
