"""Independent QA for sealed P2 v31 within-layer month adversary."""
...
import run_p2_within_layer_month_adversarial_deepset_20260901_v31 as runner  # noqa: E402
...
def main() -> None:
    ...
    blind = pd.DataFrame(
        {"time": pd.to_datetime(time_ns, unit="ns", utc=True), "layer": layer, "fold": fold}
    )
    ...
    independent_local_gate = runner.prospective_fold_layer_gate(recomputed, config)

    prefix_ok = True
    group_mass_ok = True
    nuisance_support_ok = True
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        masses = [
            value["raw_weight_sum"] for value in receipt["weight_receipt"]["groups"].values()
        ]
        group_mass_ok &= bool(np.max(masses) - np.min(masses) <= 1e-12)
        nuisance_support_ok &= receipt["weight_receipt"]["adversary_target_layers"] == [2, 3, 4]
        nuisance_support_ok &= all(
            1 <= int(month) <= 12
            for month in receipt["weight_receipt"]["adversary_calendar_months"]
        )
        for fit in receipt["fit_receipts"]:
            values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["domain_loss_first"],
                fit["domain_loss_last"],
                fit["domain_accuracy_first"],
                fit["domain_accuracy_last"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["task_parameters"] == 4865
            fit_receipts_ok &= fit["domain_head_parameters"] == 2340
            fit_receipts_ok &= fit["gradient_reversal_coefficient"] == 0.1
            fit_receipts_ok &= fit["domain_loss_multiplier"] == 1.0
            fit_receipts_ok &= fit["minimum_domain_heads_per_batch"] >= 1
            fit_receipts_ok &= fit["optimizer"] == "AdamW"
            fit_receipts_ok &= fit["task_context_adversarialized"] == 0
            fit_receipts_ok &= fit["schedule"] == 0
            fit_receipts_ok &= fit["coefficient_sweep"] == 0
            fit_receipts_ok &= fit["month_router"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            adversary_active &= fit["domain_loss_first"] > 0.0
            adversary_active &= fit["domain_loss_last"] > 0.0
    ...
    reversal = runner._gradient_reversal_contract_receipt()
    model_contract = runner._model_contract_receipt()
    isolation = runner._isolation_receipt()
    amendment_path = ROOT / config["authorization_evidence"]["prospective_gate_amendment"]
    checks = {
        "terminal": result["status"].startswith("EXPLORATORY_"),
        "exactly_nine_fits": result["fit_count"] == 9,
        "new_p2_domain_adversary_axis": semantic["classification"]
        == "NEW_P2_FIXED_WITHIN_LAYER_MONTH_GRADIENT_REVERSAL",
        "repo_p2_exact_execution_hits_zero": semantic["repository_p2_exact_execution_hits"] == 0,
        "p1_code_output_reused_zero": semantic["p1_code_or_output_reused"] is False,
        "semantic_distinctions_all_true": all(
            ...
            for name in (
                ...
                "v30_irmv1_distinguished",
            )
        ),
        "gradient_reversal_formula": reversal["formula_exact"],
        "exact_v13_task_initialization": (
            model_contract["v13_task_state_byte_equal_at_initialization"]
            and model_contract["v13_task_output_maximum_abs_error"] == 0.0
        ),
        "masked_future_isolation": isolation["masked_token_maximum_abs_error"] <= 1e-6,
        "permutation_invariance": (
            isolation["permutation_maximum_abs_error"] <= 1e-6
            and model_contract["permutation_maximum_abs_error"] <= 1e-6
        ),
        "fit_receipts_valid": fit_receipts_ok,
        "domain_adversary_actually_active": adversary_active,
        "nuisance_support": nuisance_support_ok,
        ...
        "canonical_nominal": close(nominal, record["canonical_nominal_pooled_points_delta"]),
        ...
        "bootstrap_ci": metric_tree_close(recomputed["bootstrap"], record["bootstrap"]),
        ...
        "prospective_fold_layer_gate": metric_tree_close(
            independent_local_gate, record["prospective_fold_layer_gate"]
        ),
        "effective_safety_pass": record["safety_pass_with_v26a_amendment"]
        == bool(
            record["legacy_safety_pass_without_v26a_amendment"]
            and independent_local_gate["pass"]
        ),
        ...
        "task_context_not_adversarialized": result["model_contract"]["task_context_adversarialized"] is False,
    ...
    qa = {
        "schema_version": "p2.within_layer_month_adversarial_deepset.independent_qa.20260901.v31",
        ...
        "recomputed": {
            ...
            "prospective_fold_layer_gate": independent_local_gate,
        },
        "hashes": {
