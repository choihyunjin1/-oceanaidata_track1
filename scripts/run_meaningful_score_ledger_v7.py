"""Canonical read/replay/append CLI for the policy-closed v7 score ledger."""
...
from ocean_goal.meaningful_score_ledger_v7 import (
    CONTRACT_RELATIVE,
    CONTRACT_SHA256,
    LEDGER_RELATIVE,
...
def check(root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "meaningful_score_ledger_v7.check.v1",
        ...
        "superseded_v6_implementation": contract["superseded_v6_implementation"],
        ...
        "p2_policy_closure": contract["p2_architecture_matched_policy"],
        ...
        "pre_init_qa_schema": "meaningful_score_ledger_v7.pre_init_qa.v1",
        "pre_init_qa_required_decision": "GO_INITIALIZE_V7_LEDGER",
...
def initialize(root: Path, *, append: bool, qa_receipt: Path | None) -> dict[str, Any]:
    ...
    if report["ledger_exists"]:
        if append:
            raise ContractError("v7 ledger is already initialized; second init forbidden")
    ...
    if qa_receipt is None:
        raise ContractError("canonical independent v7 pre-init QA receipt is required")
