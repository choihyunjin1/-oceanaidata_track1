"""Guarded exactly-once official materializer for immutable P3 v42 Ridge512."""
...
import run_p3_kramers_moyal_residual_cycle_20260901_v42 as science  # noqa: E402
...
def load_config() -> dict[str, Any]:
    ...
    checks = {
        "schema": config["schema_version"] == "p3.kramers_moyal.materializer.config.v42m1",
        "experiment": config["experiment_id"] == EXPERIMENT_ID,
        "status": config["status"] == "PREREGISTERED_BEFORE_OFFICIAL_READ",
        "candidate": config["candidate"]["name"] == "P3_1_KM80_RIDGE512_ADD10",
        "alpha": float(config["candidate"]["ridge_alpha"]) == 512.0,
...
def verify_internal(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if result["decision"] != contract["required_science_decision"] or candidate["decision"] != contract["required_candidate_decision"] or qa["decision"] != contract["required_qa_decision"]:
        raise ContractError("immutable v42 PASS contract absent")
    return result, qa
...
def resolve_inputs(config: dict[str, Any]) -> dict[str, Path]:
    ...
    data, source = Path(raw).resolve(), config["source_contract"]
...
def fit_full_model() -> tuple[Ridge, np.ndarray, np.ndarray, dict[str, Any]]:
    ...
    model = Ridge(alpha=512.0, fit_intercept=True, solver="cholesky").fit(train_z, target)
    receipt = {"historical_cases": int(len(cases)), "historical_target_rows": int(targets.size), "feature_receipt": receipt, "ridge_alpha": 512.0, "additive_residual_weight": 0.10, "target_winsor": [0.025, 0.975], "coefficient_l2": float(np.linalg.norm(model.coef_)), "row_deletion": 0, "full_deployment_fits": 1}
    return model, center, scale, receipt
...
def official_features(context: pd.DataFrame, index: pd.DataFrame) -> tuple[np.ndarray, pd.DataFrame, dict[str, Any]]:
    ...
    for position, row in enumerate(pairs.itertuples(index=False)):
        ...
        features[position] = science.kramers_moyal_features(group[VALUE_COLUMNS].to_numpy(np.float64))
    if not np.isfinite(features).all():
        raise ContractError("official Kramers-Moyal features are non-finite")
    receipt = {"rows": 200, "columns": science.FEATURE_COUNT, "matrix_sha256": hashlib.sha256(features.astype("<f8").tobytes()).hexdigest()}
    return features, pairs, receipt
...
def geometry(frame: pd.DataFrame, candidate: np.ndarray, champion: np.ndarray, config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, bool]]:
    action, absolute = candidate - champion, np.abs(candidate - champion)
    work = frame.assign(action=action)
    station = {str(key): {"rows": int(len(group)), "action_p99_m": float(np.quantile(np.abs(group["action"]), 0.99)), "action_max_m": float(np.max(np.abs(group["action"]))), "action_rms_m": float(np.sqrt(np.mean(np.square(group["action"]))))} for key, group in work.groupby("station", observed=True)}
    lead = {str(int(key)): {"rows": int(len(group)), "action_p99_m": float(np.quantile(np.abs(group["action"]), 0.99)), "action_max_m": float(np.max(np.abs(group["action"]))), "action_rms_m": float(np.sqrt(np.mean(np.square(group["action"]))))} for key, group in work.groupby("lead_h", observed=True)}
...
def run() -> dict[str, Any]:
    ...
    paths, source = resolve_inputs(config), config["source_contract"]
    ...
    residual = np.asarray(model.predict(np.clip((features - center) / scale, -8.0, 8.0)), dtype=np.float64)
    ...
    checks, passed = {**structure, **geometry_checks}, bool(all({**structure, **geometry_checks}.values()))
    ...
    if passed:
        output = ARTIFACT / "submission/P3_V42M1_KM80_RIDGE512_ADD10/P3_submission.csv"
        ...
        write_new(output.parent / "submission-info.txt", b"title: P3 v42 fixed Kramers-Moyal residual\nsummary: Exact uniform KMA 0.425 champion plus frozen KM80 Ridge512 residual at weight 0.10; full historical fit, no routing or retuning.\nstatus: READY_NOT_UPLOADED\n")
...
def main() -> None:
    ...
    if args.preflight:
        print(canonical(preflight()).decode(), end="")
    else:
        run()
