"""Independent QA for the P2 v20 layer-conditioned month-CORAL run."""
...
import run_p2_layer_conditioned_month_covariance_alignment_20260901_v20 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold, receipt in result["training"]["folds"].items():
        ...
        masses = [
            ...
            for value in receipt["environment_receipt"]["groups"].values()
        ]
        ...
        for fit in receipt["fit_receipts"]:
            ...
            fit_receipts_ok &= fit["coral_coefficient"] == 1.0
            fit_receipts_ok &= fit["weight_decay"] == 0.0001
            fit_receipts_ok &= fit["minimum_within_layer_month_pairs_per_batch"] >= 3
            fit_receipts_ok &= fit["minimum_rows_per_covariance"] >= 2
            covariance_ok &= fit["latent_covariance_finite"]
            covariance_ok &= not fit["alignment_crosses_target_layers"]
            covariance_ok &= bool(
                np.isfinite(
                    [
                        fit["coral_loss_first"],
                        fit["coral_loss_last"],
                        fit["objective_first"],
                        fit["objective_last"],
                    ]
    ...
    comparison_ok = comparison["use"] == "ledger_only_no_posthoc_selection_or_ensemble"
    for name, path in preserved_paths.items():
        preserved = json.loads(path.read_text(encoding="utf-8"))["candidate"]
        comparison_ok &= close(
            comparison[f"{name}_delta_rmse"],
            preserved["delta_rmse"],
        )
        comparison_ok &= close(
            comparison[f"v20_minus_{name}_delta_rmse"],
            delta - float(preserved["delta_rmse"]),
        )
        comparison_ok &= close(
            comparison[f"v20_minus_{name}_canonical_nominal_points"],
            -delta * slope - float(preserved["canonical_nominal_pooled_points_delta"]),
        )
    official_names = (
    ...
    checks = {
        ...
        "new_p2_coral_axis": (
            ...
            == "NEW_P2_LAYER_CONDITIONED_MONTH_LATENT_COVARIANCE_ALIGNMENT"
        ),
        "no_prior_p2_coral_execution": (
            semantic["prior_p2_coral_runners"] == []
            and semantic["prior_p2_coral_artifacts"] == []
            and semantic["prior_p2_coral_reports"] == []
        ),
        "v18_v19_distinguished": (
            semantic["v18_minimax_loss_distinguished"]
            and semantic["v19_label_risk_variance_distinguished"]
        ),
        "within_layer_alignment_only": (
            semantic["alignment_is_within_target_layer_only"]
            and result["training"]["alignment"]
            == "same_target_layer_calendar_month_covariance_only"
        ),
        "fixed_objective": (
            result["training"]["coral_coefficient"] == 1.0
            and result["training"]["weight_decay"] == 0.0001
        ),
        "fit_receipts_valid": fit_receipts_ok,
        "latent_covariance_checks": covariance_ok,
        ...
        "v13_runner_hash": (
            runner.v12.sha256_file(runner.v13.RUNNER) == result["hashes"]["v13_runner"]
        ),
        "v18_runner_hash": (
            runner.v12.sha256_file(runner.v18.RUNNER) == result["hashes"]["v18_runner"]
        ),
        "v19_runner_hash": (
            runner.v12.sha256_file(runner.v19.RUNNER) == result["hashes"]["v19_runner"]
        ),
        ...
        "comparison_v13_v18_v19": comparison_ok,
    ...
    qa = {
        "schema_version": "p2.layer_conditioned_month_covariance_alignment.independent_qa.20260901.v20",
        ...
        "hashes": {
            ...
            "v19_runner": runner.v12.sha256_file(runner.v19.RUNNER),
        },
    }
