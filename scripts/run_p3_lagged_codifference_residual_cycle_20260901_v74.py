"""Sealed P3 v74 fixed lagged-codifference experiment."""
...
import run_p3_pairwise_predictive_causality_residual_cycle_20260901_v73 as v73  # noqa: E402

EXPERIMENT_ID = "p3_lagged_codifference_residual_cycle_20260901_v74"
...
WINDOWS, LAGS = ((0, 145), (72, 145)), (1, 3)
FEATURE_COUNT = 48
BASE = v73.BASE
SPEC_CLASS = v73.SPECS[0].__class__
SPECS = (SPEC_CLASS("P3_1_CODIFF48_RIDGE512_ADD10", 512.0), SPEC_CLASS("P3_2_CODIFF48_RIDGE2048_ADD10", 2048.0))
BLEND, MAD_SCALE, EPSILON = 0.10, 1.4826, 1e-12
sha256, canonical, write_new = v73.sha256, v73.canonical, v73.write_new


class ContractError(RuntimeError):
    """Raised when the sealed v74 contract differs."""
...
def load_config() -> dict[str, Any]:
    ...
    checks = {
        "schema": config["schema_version"] == "p3.lagged_codifference_residual.config.v74",
        "experiment": config["experiment_id"] == EXPERIMENT_ID,
        "novel": config["duplication_audit"]["semantic_verdict"] == "NON_DUPLICATE_P3_LAGGED_CODIFFERENCE_AXIS",
        ...
        "lags": tuple(encoder["lags"]) == LAGS,
        "frequency": float(encoder["characteristic_frequency"]) == 1.0,
    ...
    if not all(checks.values()):
        raise ContractError(f"v74 config contract failed: {checks}")
...
def robust_normalize(values: np.ndarray) -> np.ndarray:
    ...
    if len(path) < 16 or not np.isfinite(path).all():
        raise ContractError("codifference path support differs")
...
def lagged_codifference(source: np.ndarray, target: np.ndarray, lag: int) -> float:
    x, y = robust_normalize(source), robust_normalize(target)
    source_aligned, target_aligned = x[:-lag], y[lag:]
    joint = np.mean(np.exp(1j * (source_aligned - target_aligned)))
    source_cf = np.mean(np.exp(1j * source_aligned))
    target_cf = np.mean(np.exp(-1j * target_aligned))
    value = float(np.log(max(abs(joint), EPSILON)) - np.log(max(abs(source_cf), EPSILON)) - np.log(max(abs(target_cf), EPSILON)))
    if not np.isfinite(value):
        raise ContractError("codifference is nonfinite")
    return value


def transformed_path(sequence: np.ndarray) -> np.ndarray:
    return v73.transformed_path(np.asarray(sequence)[:289])


def codifference_features(sequence: np.ndarray) -> np.ndarray:
    path = transformed_path(sequence)[::2]
    if path.shape != (145, 12):
        raise ContractError("fixed 20-minute path differs")
    features = np.asarray([
        lagged_codifference(path[start:stop, source], path[start:stop, target], lag)
        for source, target in PAIRS for start, stop in WINDOWS for lag in LAGS
    ], dtype=np.float64)
    if features.shape != (FEATURE_COUNT,) or not np.isfinite(features).all():
        raise ContractError("codifference surface feature contract differs")
    return features
...
def synthetic_receipt() -> dict[str, Any]:
    ...
    length = 8192
    source = rng.standard_t(2.5, size=length)
    target = np.zeros(length, dtype=np.float64)
    target[1:] = source[:-1] + 0.2 * rng.standard_t(2.5, size=length - 1)
    independent = rng.standard_t(2.5, size=length)
    forward = lagged_codifference(source, target, 1)
    reverse = lagged_codifference(target, source, 1)
    unrelated = lagged_codifference(source, independent, 1)
    if not forward > 0.50 or not forward > reverse + 0.50 or not abs(unrelated) < 0.10:
        raise ContractError("heavy-tail directed dependence guard failed")
    affine = lagged_codifference(5.0 * source + 2.0, 7.0 * target - 3.0, 1)
    signed = lagged_codifference(-source, -target, 1)
    ...
    if abs(lagged_codifference(np.ones(80), np.ones(80), 1)) > 1e-12:
    ...
    direct = codifference_features(sequence)
    extended = np.vstack([sequence, np.full((12, 10), 1e9)])
    if not np.array_equal(direct, codifference_features(extended)):
        raise ContractError("future isolation guard failed")
    return {"feature_count": len(direct), "feature_sha256": hashlib.sha256(direct.astype("<f8").tobytes()).hexdigest(), "finite": bool(np.isfinite(direct).all()), "forward_codifference": forward, "reverse_codifference": reverse, "independent_codifference": unrelated, "directed_margin": forward - reverse, "positive_affine_invariant": True, "sign_invariant": True, "constant_zero": True, "future_isolated": True}
...
def surface_features(cases: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    for position, row in enumerate(cases.itertuples(index=False)):
        ...
        features[position] = codifference_features(sequences[anchor_id])
...
def support_receipt(config: dict[str, Any]) -> dict[str, Any]:
    cases, _, _, _ = v73.v72.v71.v70.v69.v68.v67.v66.v65.v64.v63.v62.case_surface()
...
def preflight_payload() -> dict[str, Any]:
    ...
    if ARTIFACT.exists() or LOCK.exists():
        raise ContractError("v74 exactly-once namespace is consumed")
    support = support_receipt(config)
    payload = {"schema_version": "p3.lagged_codifference_residual.preflight.v74", "experiment_id": EXPERIMENT_ID, "status": "READY_EXACTLY_ONCE" if support["passed"] else "STOP_SUPPORT_GATE", "config_sha256": sha256(CONFIG), "runner_sha256": sha256(Path(__file__)), "candidate_count": 2, "maximum_model_fits": 12 if support["passed"] else 0, "synthetic": synthetic_receipt(), "historical_support": support, "prior_outputs_used": False, "official_used_for_features_gates_selection": False, "official_access": 0, "csv_materializations": 0, "uploads": 0, "config_status": config["status"]}
...
def execute(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    result.update({"schema_version": "p3.lagged_codifference_residual.result.v74", "experiment_id": EXPERIMENT_ID, "decision": "PASS_CANDIDATE_AVAILABLE" if any(item["decision"] != "NO_GO" for item in result["candidates"]) else "NO_GO_ALL_LAGGED_CODIFFERENCE_CANDIDATES", "duplication_audit": config["duplication_audit"], "primary_sources": config["primary_sources"]})
    return result, arrays


def render_report(result: dict[str, Any]) -> str:
    lines = ["# P3 lagged codifference residual cycle v74", "", "## 결론", "", f"- overall decision: **{result['decision']}**.", "- v74 measures ordered joint-versus-marginal characteristic-function dependence, not v43 marginal increment ECF, v44 distance correlation, or v51 covariance geometry.", "- Prior and official outputs were excluded; the repeatedly exposed 182-case surface is EXPLORATORY_ONLY."]
...
def main() -> int:
    ...
    if ARTIFACT.exists() or REPORT.exists() or LOCK.exists():
        raise ContractError("v74 exactly-once namespace already exists")
    config, preflight = load_config(), preflight_payload()
    if preflight["status"] != "READY_EXACTLY_ONCE":
        raise ContractError("v74 support gate failed; zero-fit closure required")
    ...
    write_new(REPORT / "claim-source-ledger.md", b"# Claim-source ledger\n\n| Claim | Source | Use |\n|---|---|---|\n| Codifference uses characteristic functions as a heavy-tail-compatible dependence measure | Wylomanska et al. 2015, DOI:10.1016/j.physa.2014.11.049 | mechanism only |\n| No executed P3 joint-versus-marginal characteristic-function codifference axis exists | repository semantic audit | novelty boundary |\n| Prior/official outputs were excluded | sealed v74 contract | reuse boundary |\n")
