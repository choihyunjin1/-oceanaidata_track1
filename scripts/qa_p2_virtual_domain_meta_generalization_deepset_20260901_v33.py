"""Independent QA for sealed P2 v33 virtual-domain MLDG run."""
...
import run_p2_virtual_domain_meta_generalization_deepset_20260901_v33 as runner  # noqa: E402
...
def main() -> None:
    ...
    prefix_ok = True
    group_mass_ok = True
    environment_contract_ok = True
    fit_receipts_ok = True
    all_environment_coverage = True
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        environment = receipt["weight_receipt"]["mldg_environment"]
        environment_contract_ok &= environment["definition"] == "target_layer_x_calendar_month"
        environment_contract_ok &= environment["count"] == len(environment["labels"])
        environment_contract_ok &= sum(environment["rows"].values()) == receipt["training_rows"]
        for fit in receipt["fit_receipts"]:
            finite_values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["meta_train_loss_first"],
                fit["meta_train_loss_last"],
                fit["meta_test_loss_first"],
                fit["meta_test_loss_last"],
                fit["maximum_inner_gradient_norm"],
                fit["maximum_outer_gradient_norm"],
            ]
            fit_receipts_ok &= bool(np.isfinite(finite_values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["parameters"] == 4865
            fit_receipts_ok &= fit["loss"] == "weighted_SmoothL1_beta_1.0_MLDG"
            fit_receipts_ok &= fit["second_order"] is True
            fit_receipts_ok &= fit["second_order_steps"] == fit["optimizer_steps"]
            fit_receipts_ok &= fit["inner_steps_per_outer_step"] == 1
            fit_receipts_ok &= fit["inner_learning_rate"] == 0.001
            fit_receipts_ok &= fit["meta_test_coefficient"] == 1.0
            fit_receipts_ok &= fit["minimum_present_domains_per_batch"] >= 2
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            all_environment_coverage &= set(fit["held_domain_steps"]) == set(
                environment["rows"]
            )
            all_environment_coverage &= min(fit["held_domain_steps"].values()) >= 1
    ...
    scalar = runner._mldg_scalar_contract_receipt()
    cycle = runner._virtual_domain_cycle_receipt()
    ...
    checks = {
        ...
        "new_virtual_domain_mldg_axis": semantic["classification"]
        == "NEW_P2_FIXED_VIRTUAL_DOMAIN_MLDG_DEEPSET",
        "repo_exact_execution_hits_zero": semantic["repository_p2_exact_execution_hits"] == 0,
        "semantic_distinctions_all_true": all(
            ...
            for name in (
                ...
                "v32_weighted_mse_distinguished",
            )
        ),
        "official_v23_feedback_selection_zero": semantic[
            "official_v23_feedback_used_for_selection"
        ]
        is False,
        "scalar_second_order_contract": scalar["second_order_graph"]
        and scalar["all_finite"],
        "virtual_domain_cycle": cycle["deterministic"],
        ...
        "environment_contract": environment_contract_ok,
        "environment_coverage": all_environment_coverage,
    ...
    qa = {
        "schema_version": "p2.virtual_domain_meta_generalization_deepset.independent_qa.20260901.v33",
