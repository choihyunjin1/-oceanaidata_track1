"""Crash-safe, literal-path-sealed P2 NCR_LGBM Stage 1 v1r5 runner.
...
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/experiments/p2_normalized_curvature_residual_lgbm_stage1_v1r5.json"
)
EXPECTED_CONFIG_SHA256 = "4c2da1ae3dc82e7a0f205cf263ee8524f28912b6a0e5419299778aa856103f48"
...
WORKER_TOKEN_ENV = "P2_NCR_V1R5_ATTEMPT_TOKEN"
WORKER_DEADLINE_ENV = "P2_NCR_V1R5_DEADLINE_EPOCH"
WORKER_PARENT_PID_ENV = "P2_NCR_V1R5_PARENT_PID"
PRELAUNCH_JOURNAL_SCHEMA = "p2_ncr_prelaunch_journal.v1r5"
    ...
    for name, pin in pins.items():
        ...
        if name == runner_name:
            if path != runner_path or mode != RUNNER_HASH_MODE:
                raise RuntimeError("v1r5 runner pin contract drift")
...
def _verify_static_bundle(config_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if config_sha256 != EXPECTED_CONFIG_SHA256:
        raise RuntimeError("NCR_LGBM v1r5 preregistration hash drift")
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if not isinstance(config, dict):
        raise TypeError("v1r5 preregistration must be a JSON object")
    if config.get("schema_version") != (
        "p2_normalized_curvature_residual_lgbm_stage1.prereg.v1r5"
    ):
        raise ValueError("unexpected v1r5 preregistration schema")
    if config.get("experiment_id") != (
        "p2_normalized_curvature_residual_lgbm_stage1_20260826_v1r5"
    ):
        raise ValueError("unexpected v1r5 experiment id")
    if config.get("status") != (
        "PREREGISTERED_NOT_EXECUTED_SUPERSEDES_V1_V1R2_V1R3_V1R4"
    ):
        raise ValueError("v1r5 is not in its sealed preregistered state")
    ...
    if execution.get("execution_enabled_after_final_qa") is not True:
        raise ValueError("v1r5 final-QA execution seal is not enabled")
    command = str(execution.get("command", ""))
    if "run_p2_normalized_curvature_residual_stage1_v1r5.py" not in command:
        raise ValueError("execution command is not the v1r5 sealed runner")
    if "run_p2_normalized_curvature_residual_stage1.py" in command or (
        ...
        raise ValueError("superseded v1/v1r2/v1r3/v1r4 runner execution is forbidden")
    ...
    if journal_contract != {
        ...
        raise ValueError("v1r5 worker prelaunch journal contract drift")
...
def _assert_static_bundle_unchanged(
    ...
    if observed_config != expected_config or observed_bundle != expected_bundle:
        raise RuntimeError("v1r5 static bundle changed after preflight")
...
def _literal_source_readiness(
    ...
    *,
    ...
    if require_environment and not environment["present"]:
        raise RuntimeError("P2_DATA_DIR is required before any v1r5 claim")
    ...
    with _held_verified_bytes(
        ...
        receipt: dict[str, Any] = {
            "schema_version": "p2_ncr_literal_data_readiness.v1r5",
...
def _acquire_attempt(
    ...
    bundle_sha256 = str(bundle["bundle_sha256"])
    claim = {
        "schema_version": "p2_ncr_stage1_execution_claim.v1r5",
        "experiment_id": config["experiment_id"],
        "contract_sha256": contract_sha256,
        "bundle_sha256": bundle_sha256,
...
def _validate_worker_claim(
    ...
    if claim.get("schema_version") != "p2_ncr_stage1_execution_claim.v1r5":
...
def _run_internal_worker(
    ...
    with _held_verified_bytes(
        ...
        worker_result: dict[str, Any] = {
            "schema_version": "p2_ncr_stage1_worker.result.v1r5",
            "experiment_id": config["experiment_id"],
            "contract_sha256": authorization["contract_sha256"],
            "bundle_sha256": authorization["bundle_sha256"],
...
def _execute_parent_impl(
    ...
    result: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.result.v1r5",
        "experiment_id": config["experiment_id"],
        "status": (
    ...
    manifest: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.manifest.v1r5",
        "experiment_id": config["experiment_id"],
...
def _record_parent_terminal_failure_best_effort(
    ...
    """Append one terminal failure unless success/final publication already exists."""
    ...
    try:
        ...
        if terminal_events or final_exists:
            diagnostic = {
                "status": "PARENT_TERMINAL_FAILURE_NOT_APPENDED_FAIL_CLOSED",
                "reason": (
                    "EXISTING_TERMINAL_EVENT"
                    if terminal_events
                    else "FINAL_ALREADY_PUBLISHED"
                ),
...
def main() -> int:
    ...
    if args.execute_stage1 and not bool(
        ...
        raise RuntimeError("v1r5 draft execution is disabled pending final QA seal")
            ...
            json.dumps(
                {
                    ...
                    "superseded_v1_v1r2_v1r3_v1r4_execution_forbidden": True,
                    "maximum_physical_fit_calls_lifetime": 3,
                    "worker_restart_count": 0,
                    "hard_wall_seconds": 1800,
