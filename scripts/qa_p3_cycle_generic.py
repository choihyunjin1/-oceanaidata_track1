"""Reusable independent recomputation for sealed P3 two-candidate residual cycles."""
...
def run_qa(
    ...
    arrays_path = artifact / "evaluation-arrays.npz"
    lock_path = artifact.parent / f"{experiment_id}.ATTEMPT_LOCK.json"
    ...
    checks: dict[str, bool] = {
        ...
        "sealed_feature_shape": result["feature_receipt"]["columns"] == feature_count
        ...
        "config_hash": result["provenance"]["config_sha256"] == qa.sha256(config_path),
        "arrays_hash": result["provenance"]["evaluation_arrays_sha256"]
        == qa.sha256(arrays_path),
    ...
    payload = {
        ...
        "hashes": {
            "result": qa.sha256(result_path),
            "arrays": qa.sha256(arrays_path),
            "runner": qa.sha256(runner),
            "config": qa.sha256(config_path),
        },
