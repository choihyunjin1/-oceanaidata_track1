"""Independent QA for the v3 input-superset to exact-output completion."""
...
from scripts.run_p2_gaussian_copula_v2_exact_frozen_submission_pack_20260830_v3 import (  # noqa: E402
    DEFAULT_BASE_CONFIG,
    DEFAULT_CONFIG,
    DEFAULT_REPORT_DIR,
...
def main() -> int:
    ...
    test, input_receipt = read_test_index_superset(p2_dir / "test_index.csv")
    ...
    partial = (
        test.groupby("time", sort=False)["layer"].transform("nunique").lt(3).to_numpy()
    )
    ...
    stored_query = result["materialization"]["query_contract"]
    checks = {
        ...
        "input_superset_exact": list(test.columns) == INPUT_COLUMNS
        and input_receipt == stored_query["input_superset"],
        "nominal_depth_preserved_qa_only": input_receipt[
            "nominal_depth_preserved"
        ]
        and input_receipt["nominal_depth_finite"]
        and not input_receipt["nominal_depth_used_as_model_feature"],
        "sample_and_candidate_exact_output_schema": list(sample.columns)
        == OUTPUT_COLUMNS
        and list(candidate.columns) == OUTPUT_COLUMNS
        and stored_query["output_columns_exact_order"] == OUTPUT_COLUMNS,
        ...
        "query_contract": query_contract["rows"] == stored_query["rows"]
        and query_contract["layer_rows"] == stored_query["layer_rows"],
    ...
    qa = {
        "schema_version": (
            "p2.gaussian_copula_v2.exact_frozen_submission_pack.qa.20260830.v3"
        ),
