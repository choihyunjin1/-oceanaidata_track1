"""Independent post-run QA for P3 v28 cross-wavelet phase residuals."""
...
def grouped_delta(frame: pd.DataFrame, prediction: np.ndarray, columns: list[str]) -> dict[str, float]:
...
def main() -> int:
    ...
    checks: dict[str, bool] = {
        ...
        "terminal_no_go": result["decision"] == "NO_GO_ALL_PHASE_CANDIDATES",
        ...
        "sealed_phase_features": result["feature_receipt"]["columns"] == 330,
        ...
        "config_official_zero": all(value == 0 for value in config["official_policy"].values()),
    ...
    for index, (candidate, prediction) in enumerate(
        ...
        ci_match &= np.allclose(
            group_ci["ci90_m"], candidate["block_station_bootstrap"]["ci90_m"], atol=1e-12
        )
    ...
    payload = {
        "schema_version": "p3.cross_wavelet_phase_residual.independent_qa.v28",
