"""Independent QA for sealed P2 v34 gradient-centralized run."""
...
import run_p2_gradient_centralized_domain_balanced_deepset_20260901_v34 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            finite_values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["maximum_abs_mean_before"],
                fit["maximum_abs_mean_after"],
            ]
            fit_receipts_ok &= bool(np.isfinite(finite_values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["parameters"] == 4865
            fit_receipts_ok &= fit["loss"] == "weighted_SmoothL1_beta_1.0_with_gradient_centralization"
            fit_receipts_ok &= fit["centralization_steps"] == fit["optimizer_steps"]
            fit_receipts_ok &= fit["eligible_count"] == 5
            fit_receipts_ok &= fit["maximum_abs_mean_after"] <= 1e-6
            fit_receipts_ok &= fit["coefficient"] is None
            fit_receipts_ok &= fit["second_loss"] == 0
            fit_receipts_ok &= fit["task_split"] == 0
            fit_receipts_ok &= fit["bias_or_1d_change"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            if eligible_names is None:
                eligible_names = fit["eligible_names"]
            else:
                fit_receipts_ok &= eligible_names == fit["eligible_names"]
    ...
    central = runner._gradient_centralization_contract_receipt()
    ...
    checks = {
        ...
        "new_gradient_centralization_axis": semantic["classification"]
        == "NEW_P2_FIXED_LINEAR_WEIGHT_GRADIENT_CENTRALIZATION_DEEPSET",
        ...
        is False,
        "centralization_formula": central["formula_exact"]
        and central["row_means_zero"],
        "bias_unchanged": central["bias_unchanged"],
        ...
        "exact_five_linear_weights": eligible_names is not None and len(eligible_names) == 5,
    ...
    qa = {
        "schema_version": "p2.gradient_centralized_domain_balanced_deepset.independent_qa.20260901.v34",
