# P3 interrupted-run recovery — not a cold-start package

This directory is a separately authorized recovery of v2 run_a, not a pristine
submission training package or a second uninterrupted cold-start proof.
It reuses exactly seven same-cycle freshly trained model files and their own
source-generated feature/OOF files after hashes and native model schemas pass.
It performs only one missing full multi CatBoost fit and one full loss-router fit.
The two earlier prequential routers are not refit. The original interrupted
attempt and its locks remain unchanged.

Accounting: 7 successful + 1 interrupted backbone attempt in the original run;
1 new full multi fit here gives 8 successful models but 9 started backbone
attempts across the interrupted lineage. The pristine v2 run_b is the separate
uninterrupted source-to-empty-model proof.

Use this recovery only with RECOVERY_INPUTS.json and its pinned own files.
Do not use --prepare, delete a lock, rerun completed fits, or label it empty-model
training. RUN_TRAINING completes the missing fits; the following replay/QA/
inference commands preserve the fixed v2 prediction policy.

The six-hour elapsed wall is conservatively measured from the original run_a
prepare start and includes the user interruption and inter-process gaps.
Original full single fit seconds were not saved before interruption and remain
unknown; they are not reconstructed as a precise duration.

The general fixed model description and data restrictions follow below. Any
generic pristine-package wording below applies to v2, not to this recovery.
...
The v1 package completed source feature preparation but stopped before any model fit because its packaging adapter mistakenly required previous OOF targets before the current 48h context start. This v2 restores the original canonical requirement: all previous OOF targets must be available before the fixed fold-start model/router fit time. Context-start gap is only a diagnostic. The earlier failed package/receipt are preserved. Model algorithms, rows, splits, seeds, coefficients and replay tolerance are unchanged; no outputs were evaluated or used to tune this correction.
