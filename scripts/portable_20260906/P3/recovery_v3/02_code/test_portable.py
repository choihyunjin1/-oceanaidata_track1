...
def test_recovery_path_boundary():
    assert r.owned_recovery_path("03_model/full/single.cbm").is_relative_to(r.ROOT)
    with pytest.raises(ValueError):
        r.owned_recovery_path("../outside-model.cbm")
    with pytest.raises(ValueError):
        r.owned_recovery_path(str(r.ROOT.parent / "outside-model.cbm"))


def test_recovery_cannot_claim_empty_prepare(tmp_path):
    with pytest.raises(RuntimeError, match="does not rebuild"):
        r.prepare({"recovery_id": "synthetic"}, tmp_path)


def test_recovery_has_only_two_fit_calls_and_no_historical_refit():
    tree = ast.parse(inspect.getsource(r.train))
    fits = [
        node
    ...
    assert len(fits) == 2
    text = inspect.getsource(r.train)
    assert "_fit_fold_components" not in text
    assert "_evaluate_fixed_structure" not in text
    assert "fixed_prequential_lead_router" not in text
    assert "_single_model" not in text
...
def test_canonical_timeline_accepts_available_fold_targets_not_new_context_gate():
    selected = pd.DataFrame(
        {
            "fold": ["past", "next"],
            "anchor_time": pd.to_datetime(["2024-10-30T12:00:00Z", "2024-11-01T00:00:00Z"]),
        }
    )
    result = r.canonical_timeline(
        selected,
        [
            ("past", "2024-07-01", "2024-11-01"),
            ("next", "2024-11-01", "2025-03-01"),
        ],
    )
    assert result[0]["target_ready_fold_start_gap_hours"] == 12
    assert result[0]["target_ready_context_gap_hours_diagnostic_only"] == -36


def test_canonical_timeline_rejects_unavailable_fold_targets():
    selected = pd.DataFrame(
        {
            "fold": ["past", "next"],
            "anchor_time": pd.to_datetime(["2024-10-31T01:00:00Z", "2024-11-01T00:00:00Z"]),
        }
    )
    with pytest.raises(ValueError, match="before fold start"):
        r.canonical_timeline(
            selected,
            [
                ("past", "2024-07-01", "2024-11-01"),
                ("next", "2024-11-01", "2025-03-01"),
