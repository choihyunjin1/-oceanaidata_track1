...
def prepare(config, source):
    if "recovery_id" in config:
        raise RuntimeError("research recovery does not rebuild or claim cold-start preparation")
...
def owned_recovery_path(name):
    path = (OUT / name).resolve()
    if OUT not in path.parents:
        raise ValueError("recovery input path escapes owned package")
    return path


def verify_reused(config, source):
    """Validate only files newly produced by the interrupted v2 run, without fitting."""
    verify(config, source, prepared=True)
    contract = json.loads((OUT / "RECOVERY_INPUTS.json").read_text(encoding="utf-8"))
    for name, expected in contract["sha256"].items():
        if sha(owned_recovery_path(name)) != expected:
            raise ValueError("recovery input hash differs: " + name)
    original = json.loads((CODE / "original_config.json").read_text(encoding="utf-8"))
    if {k: v for k, v in config.items() if k != "recovery_id"} != original:
        raise ValueError("recovery changes original config beyond identity")
    inherited_seal = json.loads((OUT / "inherited_seal.json").read_text(encoding="utf-8"))
    prepared = json.loads((REPORT / "prepare.json").read_text(encoding="utf-8"))
    if prepared["seal_sha256"] != sha(OUT / "inherited_seal.json"):
        raise ValueError("original prepare/seal link differs")
    if inherited_seal["config_sha256"] != sha(CODE / "original_config.json"):
        raise ValueError("original frozen config hash differs")
    if inherited_seal["runner_sha256"] != contract["original_runner_sha256"]:
        raise ValueError("original runner hash differs")
    for name, expected in prepared["files"].items():
        if sha(WORK / name) != expected:
            raise ValueError("original source-generated feature changed")
    original_fit = json.loads((OUT / "inherited_fit_receipts.json").read_text(encoding="utf-8"))
    if original_fit["completed_backbone_fits"] != 6:
        raise ValueError("six completed historical models required")
    for fold in original_fit["historical"]:
        for name, expected in fold["model_sha256"].items():
            if sha(MODELS / "historical" / fold["fold"] / name) != expected:
                raise ValueError("historical save-time model hash differs")
    oof_receipt = json.loads((REPORT / "regenerated-oof.json").read_text(encoding="utf-8"))
    if sha(WORK / "oof.parquet") != oof_receipt["oof_sha256"]:
        raise ValueError("original OOF save-time hash differs")
    router_receipts = oof_receipt["detail"]["router_receipts"]
    if [x["past_fit_rows"] for x in router_receipts] != [0, 294, 768]:
        raise ValueError("original two prequential router receipts missing")
    if [x["action"] for x in router_receipts] != [
        "exact_equal_component_no_op",
        "fixed_config_refit_on_completed_corrected_oof_only",
        "fixed_config_refit_on_completed_corrected_oof_only",
    ]:
        raise ValueError("original prequential router action differs")
    if any(x["current_fold_target_used_for_router"] for x in router_receipts):
        raise ValueError("original router used current targets")
    recipe = json.loads((CODE / "recipe.json").read_text(encoding="utf-8"))
    columns = json.loads((WORK / "feature_columns.json").read_text(encoding="utf-8"))["columns"]
    for path in MODELS.rglob("*.cbm"):
        model = CatBoostRegressor().load_model(path)
        kind = path.stem
        if model.tree_count_ != recipe["model"][kind]["iterations"]:
            raise ValueError("reused model tree count differs")
        expected_names = (
            ["station", "lead_h", "current_hs_for_residual", *columns]
            if kind == "single"
            else ["station", *columns]
        )
        if model.feature_names_ != expected_names:
            raise ValueError("reused model feature schema differs")
    return contract


def full_router_material(oof, features, anchors):
    """Create full-router training arrays from existing OOF, with zero meta fitting."""
    if len(oof) != 1086 or oof.anchor_id.nunique() != 181:
        raise ValueError("recovery OOF population differs")
    ...
    truth_rows = np.asarray(
        [
            lookup.loc[aid, f"target_{int(lead)}"]
            for aid, lead in oof[["anchor_id", "lead_h"]].itertuples(index=False, name=None)
        ]
    )
    if not np.array_equal(truth_rows, oof.target_hs.to_numpy()):
        raise ValueError("recovery OOF targets differ from own prepared source")
    case_x, case_meta, components, _ = base.build_case_router_data(oof, features, anchors)
    truth = np.column_stack(
        [lookup.loc[case_meta.anchor_id, f"target_{lead}"].to_numpy(dtype=float) for lead in LEADS]
    )
    row_x, _, _, row_losses = base.expand_case_router_rows(case_x, case_meta, components, truth)
    return row_x, row_losses


def train(config, source):
    """One new full multi and one full router; never repeat the seven prior fits."""
    if (OUT / "TRAIN_LOCK.json").exists():
        raise RuntimeError("recovery attempt consumed; preserve outputs")
    if (MODELS / "full" / "multi.cbm").exists() or (MODELS / "full" / "router.joblib").exists():
        raise ValueError("missing models unexpectedly present")
    if len(list(MODELS.rglob("*.cbm"))) != 7:
        raise ValueError("exactly seven pinned fresh models required")
    contract = verify_reused(config, source)
    started = time.perf_counter()
    save(
        OUT / "TRAIN_LOCK.json",
        {
            "pid": os.getpid(),
            "created_utc": stamp(),
            "empty_03_model": False,
            "gpu_authorized": True,
            "max_new_backbone_fits": 1,
            "max_new_router_fits": 1,
    ...
    if set(oof.anchor_id.astype(int)) != set(keys.anchor_id.astype(int)):
        raise ValueError("recovery validation anchor keys differ")
    row_x, row_losses = full_router_material(oof, features, anchors)
    save(
        REPORT / "recovery-preflight.json",
        {
            "status": "PASS",
            "pid": os.getpid(),
            "input_hash_checks": len(contract["sha256"]),
            "model_native_load_schema_checks": 7,
            "prior_historical_backbone_fits": 6,
            "prior_full_single_fits": 1,
            "prior_prequential_router_fits": 2,
            "prequential_router_saved_models": 0,
            "repeated_prequential_router_fits": 0,
            "source_config_key_oof_model_provenance_pass": True,
            "official_rows": 0,
            "csv_rows": 0,
            "uploads": 0,
        },
    ...
    save(
        OUT / "progress.json",
        {
            "stage": "RECOVERY_FULL_MULTI",
            "pid": os.getpid(),
            "inherited_successful_backbone_fits": 7,
            "new_completed_backbone_fits": 0,
            "new_max_backbone_fits": 1,
            "cpu_threads": 2,
            "gpu_owner": "P3",
        },
        progress=True,
    ...
    conservative_wall = (datetime.now(UTC) - datetime.fromisoformat(began)).total_seconds()
    prepared = json.loads((REPORT / "prepare.json").read_text(encoding="utf-8"))
    result = {
        "status": "TRAINING_COMPLETE_RECOVERED_FRESH_MODELS",
        "pid": os.getpid(),
        "empty_03_model_verified": False,
        "uninterrupted_cold_start_proof": False,
        "recovery_id": config["recovery_id"],
        "backbone_fits": 8,
        ...
        "inherited_successful_backbone_fits": 7,
        "inherited_interrupted_backbone_attempts": 1,
        "cumulative_backbone_attempts_including_interruption": 9,
        "full_single_rows": len(ids) * 6,
        "full_multi_cases": len(ids),
        "full_single_seconds": None,
        "full_multi_seconds": multi_seconds,
        "train_seconds": elapsed,
        "prepare_seconds": prepared["seconds"],
        "prepare_train_seconds": conservative_wall,
        "prepare_train_seconds_basis": "conservative wall from original prepare including pause",
        "regenerated_oof_rmse_m": float(
            np.sqrt(np.square(oof.target_hs - oof.final_prediction).mean())
        ),
        ...
        "source_sha256": config["source_files"],
        "old_model_cache_oof_answer_reads": 0,
        "same_cycle_fresh_models_reused": 7,
        "same_cycle_fresh_oof_reused": True,
        "pretrained_weights": 0,
        "official_rows": 0,
        "csv_rows": 0,
        "uploads": 0,
        "organizer_hardware_verified": False,
        "recovery_inputs_sha256": sha(OUT / "RECOVERY_INPUTS.json"),
    }
    if conservative_wall > 21600:
        raise TimeoutError("six-hour original-source-to-recovery wall exceeded")
        ...
        json.dumps(
            {
                ...
                "new_backbone_fits": 1,
                "new_router_fits": 1,
                "new_train_seconds": elapsed,
                "conservative_total_wall_seconds": conservative_wall,
            }
        ),
        flush=True,
    )
...
def inference(config, source, *, verify_answer=False):
    ...
    if verify_answer:
        ...
        receipt = {
            ...
            "empty_model_to_answer_regeneration_completed": False,
            "recovered_source_only_training_to_answer": True,
