...
def prepare(config, source):
    ...
    # Verify actual chronology without changing the canonical fixed selection/router.
    timeline = []
    completed = []
    for name, _, _ in recipe["validation"]["windows"]:
        current = selected.loc[selected.fold.eq(name)]
        if completed:
            past = selected.loc[selected.fold.isin(completed)]
            separation = (current.anchor_time.min() - pd.Timedelta(hours=48)) - (
                past.anchor_time.max() + pd.Timedelta(hours=24)
            )
            if separation.total_seconds() < 0:
                raise ValueError("canonical prior OOF targets unavailable before current context")
            timeline.append(
                {
                    "fold": name,
                    "past_cases": len(past),
                    "target_ready_context_gap_hours": separation.total_seconds() / 3600,
                }
            )
