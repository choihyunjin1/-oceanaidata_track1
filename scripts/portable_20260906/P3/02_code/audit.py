...
def audit(training_only):
    ...
    checks = {}
    checks["empty_model_training_complete"] = (
        train["status"] == "TRAINING_COMPLETE_FROM_EMPTY_MODEL" and train["empty_03_model_verified"]
    )
    ...
    for row in pre["prior_oof_actual_time_audit"]:
        checks["earlier_targets_available:" + row["fold"]] = (
            row["target_ready_context_gap_hours"] >= 0
        )
