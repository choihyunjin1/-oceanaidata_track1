"""Independent post-run QA for P3 v24 station-graph cycle."""
...
def main() -> int:
    ...
    checks["terminal_no_go"] = result["decision"] == "NO_GO_ALL_STATION_GRAPH_CANDIDATES"
    ...
    checks["six_blocks_each"] = all(sum(receipt["candidate"] == candidate["name"] for receipt in result["fit_receipts"]) == 6 for candidate in result["candidates"])
    checks["zero_row_deletion"] = all(receipt["row_deletion"] == 0 for receipt in result["fit_receipts"])
    ...
    payload = {
        "schema_version": "p3.station_graph_laplacian_residual.independent_qa.v24",
        ...
        "uploads": 0,
        "hashes": {"result": sha256(RESULT), "arrays": sha256(ARRAYS), "runner": sha256(RUNNER), "config": sha256(CONFIG)},
    }
    if OUTPUT.exists():
