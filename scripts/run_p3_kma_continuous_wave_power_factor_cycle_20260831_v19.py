"""Run the sealed train-prefix wave-power ECDF KMA factor for P3."""
...
EXPERIMENT_ID = "p3_kma_continuous_wave_power_factor_cycle_20260831_v19"
...
CONFIG = ROOT / "configs/experiments/p3_kma_continuous_wave_power_factor_cycle_20260831_v19.json"
...
DELIVERY = Path(
    r"C:\Users\cedis\Downloads\해양 해커톤 제출용\20260831_P3_KMA_CONTINUOUS_WAVE_POWER_V19"
)

CANDIDATE = "P3_1_KMA_CONTINUOUS_WAVE_POWER_FACTOR"
...
class ContractError(RuntimeError):
    """Raised when the sealed wave-power contract is violated."""


def compute_wave_power(hs: np.ndarray, tp: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    value = np.full(len(hs_array), np.nan, dtype=np.float64)
    value[valid] = np.square(hs_array[valid]) * tp_array[valid]
    return value, valid


def load_wave_power_history() -> pd.DataFrame:
    ...
    history["wave_power"], history["wave_power_valid"] = compute_wave_power(
    ...
    if len(history) != len(features) or history.duplicated(["anchor_id", "station"]).any():
        raise ContractError("wave-power history key contract failed")
    if not np.isfinite(history.loc[history["wave_power_valid"], "wave_power"]).all():
        raise ContractError("valid wave power contains non-finite values")
    return history


def attach_wave_power(frame: pd.DataFrame, history: pd.DataFrame) -> pd.DataFrame:
    columns = ["anchor_id", "station", "wave_power", "wave_power_valid"]
    merged = frame.merge(history[columns], on=["anchor_id", "station"], how="left", validate="many_to_one")
    if merged["wave_power_valid"].isna().any():
        raise ContractError("historical wave-power mapping missing")
    return merged


def ranks_from_prefix(prefix: np.ndarray, value: np.ndarray, valid: np.ndarray) -> np.ndarray:
    rank = np.full(len(value), NEUTRAL_ECDF, dtype=np.float64)
    if np.any(valid):
        rank[valid] = ecdf(prefix, value[valid])
    return rank
...
def predict_policy(frame: pd.DataFrame, rank: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    valid = frame["wave_power_valid"].to_numpy(dtype=bool)
    lead18 = frame["lead_h"].eq(18).to_numpy()
    lead24 = frame["lead_h"].eq(24).to_numpy()
    alpha[lead18 & valid] = ALPHA_18
    alpha[lead24 & valid] = 0.20 + 0.40 * rank[lead24 & valid]
    alpha[(lead18 | lead24) & ~valid] = REFERENCE_ALPHA
    for mask in (lead18 & valid, lead24 & valid):
        prediction[mask] = (
...
def key_values_and_order_equal(left: pd.DataFrame, right: pd.DataFrame) -> bool:
    return (
        ...
        and bool(np.array_equal(left.to_numpy(), right.to_numpy()))
    )
...
def evaluate(frame: pd.DataFrame, history: pd.DataFrame) -> dict[str, Any]:
    ...
    if not np.isfinite(prediction).all():
        raise ContractError("wave-power OOF prediction incomplete")
    ...
    station_lead = {
        ...
        for (station, lead), part in scored.groupby(["station", "lead_h"], observed=True, sort=True)
    }
    ...
    return {
        "candidate": {
            ...
            "proxy_formula": "hs_current^2*tp_current",
...
def materialize(passed: bool, history: pd.DataFrame) -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    value, valid = compute_wave_power(features["hs_current"].to_numpy(), features["tp_current"].to_numpy())
    features = features.assign(wave_power=value, wave_power_valid=valid)
    official = official.merge(
        features[["case_id", "station", "wave_power", "wave_power_valid"]],
        on=["case_id", "station"],
        how="left",
        validate="many_to_one",
    )
    if not key_values_and_order_equal(champion[KEYS], official[KEYS]) or official["wave_power_valid"].isna().any():
        raise ContractError("official key/order/wave-power contract failed")
    prefix = np.sort(
        history.loc[history["wave_power_valid"], "wave_power"].to_numpy(dtype=np.float64)
    )
    rank = ranks_from_prefix(
        prefix,
        official["wave_power"].to_numpy(dtype=np.float64),
        official["wave_power_valid"].to_numpy(dtype=bool),
    )
...
def main() -> int:
    ...
    if ARTIFACT_DIR.exists() or LOCK.exists() or DELIVERY.exists():
        raise ContractError("exactly-once artifact, lock, or delivery exists")
