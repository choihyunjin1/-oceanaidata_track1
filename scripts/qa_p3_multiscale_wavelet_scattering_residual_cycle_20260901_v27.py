"""Independent post-run QA for P3 v27 fixed wavelet-scattering residuals."""
...
def main() -> int:
    ...
    checks["terminal_pass_available"] = result["decision"] == "PASS_CANDIDATE_AVAILABLE"
    ...
    checks["fit_count_12"] = result["fit_count"] == 12 and len(result["fit_receipts"]) == 12
    checks["six_outer_blocks_per_candidate"] = all(sum(item["candidate"] == candidate["name"] for item in result["fit_receipts"]) == 6 for candidate in result["candidates"])
    checks["sealed_scattering_features"] = result["feature_receipt"]["columns"] == 336
    ...
    payload = {"schema_version": "p3.multiscale_wavelet_scattering_residual.independent_qa.v27", "experiment_id": EXPERIMENT_ID, "decision": "PASS" if all(checks.values()) else "FAIL", "checks": checks, "check_count": len(checks), "passed": sum(checks.values()), "failed": sum(not value for value in checks.values()), "model_fits": result["fit_count"], "official_rows": 0, "csv_materializations": 0, "uploads": 0, "hashes": {"result": sha256(RESULT), "arrays": sha256(ARRAYS), "runner": sha256(RUNNER), "config": sha256(CONFIG)}}
    if OUTPUT.exists():
