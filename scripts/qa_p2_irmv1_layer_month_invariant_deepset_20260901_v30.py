"""Independent QA for sealed P2 v30 fixed IRMv1 run."""
...
import run_p2_irmv1_layer_month_invariant_deepset_20260901_v30 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        environment_support_ok &= (
            receipt["weight_receipt"]["irmv1_environment_count"]
            == len(receipt["weight_receipt"]["irmv1_environment_ids"])
        )
        environment_support_ok &= (
            receipt["weight_receipt"]["irmv1_environment_count"] >= 3
        )
        for fit in receipt["fit_receipts"]:
            values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["irmv1_penalty_first"],
                fit["irmv1_penalty_last"],
                fit["irmv1_penalty_min"],
                fit["irmv1_penalty_max"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["irmv1_coefficient"] == 1.0
            fit_receipts_ok &= fit["dummy_classifier_scale"] == 1.0
            fit_receipts_ok &= fit["minimum_batch_environment_count"] >= 1
            fit_receipts_ok &= fit["optimizer"] == "AdamW"
            fit_receipts_ok &= fit["annealing"] == 0
            fit_receipts_ok &= fit["coefficient_sweep"] == 0
            fit_receipts_ok &= fit["environment_router"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            irmv1_active &= fit["irmv1_penalty_max"] > 0.0
    ...
    irm = runner._irmv1_contract_receipt()
    ...
    checks = {
        ...
        "new_irmv1_axis": semantic["classification"]
        == "NEW_P2_FIXED_IRMV1_LAYER_MONTH_OPTIMALITY_PENALTY",
        ...
        "irmv1_formula": (
            irm["formula_exact"]
            and irm["byte_identical_replay"]
            and not irm["annealing"]
            and not irm["coefficient_sweep"]
        ),
        ...
        "irmv1_actually_active": irmv1_active,
        "environment_support": environment_support_ok,
    ...
    qa = {
        "schema_version": "p2.irmv1_layer_month_invariant_deepset.independent_qa.20260901.v30",
