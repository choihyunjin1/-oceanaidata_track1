"""CLI body executed only from the authenticated P2 v4 bootstrap buffer."""
...
try:
    _CONTEXT = _P2_V4_BOOTSTRAP_CONTEXT  # type: ignore[name-defined]  # noqa: F821
    _HELPER = _P2_V4_AUTHENTICATED_HELPER  # type: ignore[name-defined]  # noqa: F821
except NameError as exc:  # pragma: no cover - subprocess contract test
    raise RuntimeError("P2 v4 CLI requires the authenticated bootstrap") from exc
...
def run_authenticated() -> dict[str, Any]:
    ...
    if root != _CONTEXT["workspace"]:
        raise RuntimeError("P2 v4 CLI workspace changed")
    if arguments["mode"] != "check-only":
        raise RuntimeError("P2 v4 CLI is check-only")
    ...
    if requested is not None:
        ...
        if requested_path.resolve(strict=True) != canonical.resolve(strict=True):
            raise RuntimeError("alternate P2 v4 config is forbidden")
    ...
    report["authenticated_cli"] = {
        ...
        "canonical_absent_pycache_prefix": _CONTEXT["pycache_prefix_relative"],
        "pycache_prefix_absent": _CONTEXT["pycache_prefix_absent"](),
