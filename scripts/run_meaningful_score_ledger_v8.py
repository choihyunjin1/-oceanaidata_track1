"""Canonical CLI for the v8 completion-lineage-closed score ledger."""
...
from ocean_goal.meaningful_score_ledger_v8 import (
    CONTRACT_RELATIVE,
    CONTRACT_SHA256,
    LEDGER_RELATIVE,
...
def check(root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "meaningful_score_ledger_v8.check.v1",
        ...
        "superseded_v7_implementation": contract["superseded_v7_implementation"],
        ...
        "pre_init_qa_schema": "meaningful_score_ledger_v8.pre_init_qa.v1",
        "pre_init_qa_required_decision": "GO_INITIALIZE_V8_LEDGER",
...
def initialize(root: Path, *, append: bool, qa_receipt: Path | None) -> dict[str, Any]:
    ...
    if report["ledger_exists"]:
        if append:
            raise ContractError("v8 ledger is already initialized; second init forbidden")
    ...
    if qa_receipt is None:
        raise ContractError("canonical independent v8 pre-init QA receipt is required")
