"""Prepared exact official materializer for v33a; execution is separately gated."""
...
INTERNAL_RESULT = ROOT / "artifacts" / EXPERIMENT_ID / "result.json"
...
class ContractError(RuntimeError):
    """Raised when official candidate inputs or structure drift."""
...
def validate_frame(frame: pd.DataFrame, rows: int) -> dict[str, bool]:
    checks = {
...
def build_candidate(champion: pd.DataFrame, e150: pd.DataFrame, anchor: pd.DataFrame) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    removal = (
        champion["station"].astype(str).eq("I-ORS").to_numpy()
        & (anchor_label == 0)
        & (e150_label == 1)
    )
    ...
    if int(gi2.sum()) != 2 or np.any(gi2 & (output == 0)):
...
def preflight() -> dict[str, Any]:
    ...
    checks = {
        ...
        "internal_terminal": result["status"] == "INFORMATION_VALUE_POSITIVE",
        "fit0": result["fit_count"] == 0,
        "official_zero": result["operations"]["official_reads"] == 0,
        "csv_zero": result["operations"]["submission_csv_created"] == 0,
        "output_absent": not resolve(config["official_materializer"]["output"]).exists()
    }
    if not all(checks.values()):
        raise ContractError(f"materializer preflight failed: {checks}")
    return {"status": "READY_NO_OFFICIAL_READS", "checks": checks, "official_reads": 0, "submission_csv_created": 0, "uploads": 0}
...
def execute() -> dict[str, Any]:
    ...
    candidate, removal = build_candidate(frames["champion"], frames["e150"], frames["anchor"])
    checks = validate_frame(candidate, int(spec["expected_rows"]))
    if int(removal.sum()) != int(spec["expected_removed_i_ors_e150_additions"]):
        raise ContractError("official I-ORS E150 removal count differs")
    if int(candidate["label"].sum()) != int(spec["expected_positive_rows"]):
        raise ContractError("official candidate positive count differs")
    output = resolve(spec["output"])
    output.parent.mkdir(parents=True, exist_ok=False)
    candidate.to_csv(output, index=False, lineterminator="\n")
    receipt = {
        "schema_version": "p1.remove_i_ors_e150.materialization.v33a",
        "experiment_id": EXPERIMENT_ID,
        "status": "MATERIALIZED_NOT_UPLOADED",
        "path": str(output),
        "rows": len(candidate),
        "positive_rows": int(candidate["label"].sum()),
        "removed_i_ors_e150_additions": int(removal.sum()),
        "checks": checks,
