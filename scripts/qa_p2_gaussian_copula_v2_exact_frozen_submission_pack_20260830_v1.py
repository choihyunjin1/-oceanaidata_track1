"""Independent official-query QA for the frozen P2 Gaussian-copula v2 pack."""
...
def main() -> int:
    ...
    duplicate = duplicate_receipt(candidate_sha, config)
    ...
    checks = {
        "config_hash": config_sha == EXPECTED_CONFIG_SHA256
        and result["config_sha256"] == config_sha,
        "attempt_one_of_one": lock["attempt_number"] == 1
        and lock["maximum_executions"] == 1,
        "attempt_config_hash": lock["config_sha256"] == config_sha,
        ...
        == config["frozen_recipe"]["expected_refit_model_receipt_sha256"],
        ...
        "note_title_summary_hash": config["title"] in note
        and config["one_line_summary"] in note
    ...
    qa = {
        "schema_version": "p2.gaussian_copula_v2.exact_frozen_submission_pack.qa.20260830.v1",
