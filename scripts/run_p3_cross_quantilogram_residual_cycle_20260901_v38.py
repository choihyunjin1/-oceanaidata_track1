"""Sealed P3 v38 directional cross-quantilogram residual experiment."""
...
EXPERIMENT_ID = "p3_cross_quantilogram_residual_cycle_20260901_v38"
...
PAIRS = ((0, 1), (0, 2), (0, 5))
PAIR_NAMES = (("hs", "tp"), ("hs", "hmax"), ("hs", "wspd"))
TAILS = (("upper", 0.8), ("upper", 0.9), ("lower", 0.2), ("lower", 0.1))
LAGS = (0, 6, 18)
FEATURE_COUNT = 72
SPECS = (
    v36.v26.Spec("P3_1_XQ72_RIDGE512_ADD10", 512.0),
    v36.v26.Spec("P3_2_XQ72_RIDGE2048_ADD10", 2048.0),
)
BLEND = 0.10


class ContractError(RuntimeError):
    """Raised when the sealed v38 contract differs."""
...
def load_config() -> dict[str, Any]:
    ...
    checks = {
        "schema": config["schema_version"] == "p3.cross_quantilogram_residual.config.v38",
        ...
        "pairs": tuple(tuple(item) for item in encoder["pairs"]) == PAIR_NAMES,
        "upper": tuple(encoder["upper_quantiles"]) == (0.8, 0.9),
        "lower": tuple(encoder["lower_quantiles"]) == (0.2, 0.1),
    ...
    if not all(checks.values()):
        raise ContractError(f"v38 config contract failed: {checks}")
...
def quantile_hits(values: np.ndarray, tail: str, tau: float) -> np.ndarray:
    path = np.asarray(values, dtype=np.float64)
    threshold = float(np.quantile(path, tau))
    if tail == "upper":
        hits = (path > threshold).astype(np.float64) - (1.0 - tau)
    elif tail == "lower":
        hits = (path <= threshold).astype(np.float64) - tau
    else:
        raise ContractError("unknown tail contract")
    if not np.isfinite(hits).all():
        raise ContractError("quantile-hit process is nonfinite")
    return hits


def cross_quantilogram(
    source: np.ndarray,
    target: np.ndarray,
    tail: str,
    tau: float,
    ...
    source_hit = quantile_hits(source, tail, tau)
    target_hit = quantile_hits(target, tail, tau)
    if lag:
        source_hit = source_hit[:-lag]
        target_hit = target_hit[lag:]
    numerator = float(np.mean(source_hit * target_hit))
    denominator = float(
        np.sqrt(np.mean(np.square(source_hit)) * np.mean(np.square(target_hit)))
    )
    if denominator <= 1e-15:
        return 0.0
    value = numerator / denominator
    if not np.isfinite(value):
        raise ContractError("cross-quantilogram is nonfinite")
    return float(np.clip(value, -1.0, 1.0))


def cross_quantilogram_features(sequence: np.ndarray) -> np.ndarray:
    ...
    for first, second in PAIRS:
        for tail, tau in TAILS:
            for lag in LAGS:
                output.append(
                    cross_quantilogram(native[:, first], native[:, second], tail, tau, lag)
                )
                output.append(
                    cross_quantilogram(native[:, second], native[:, first], tail, tau, lag)
                )
    features = np.asarray(output, dtype=np.float64)
    if features.shape != (FEATURE_COUNT,) or not np.isfinite(features).all():
        raise ContractError("cross-quantilogram feature contract differs")
    return features


def lead_lag_receipt() -> dict[str, Any]:
    rng = np.random.default_rng(20260901)
    source = rng.normal(size=145)
    target = rng.normal(scale=0.15, size=145)
    target[6:] += source[:-6]
    forward = cross_quantilogram(source, target, "upper", 0.8, 6)
    reverse = cross_quantilogram(target, source, "upper", 0.8, 6)
    if forward <= reverse + 0.25 or forward <= 0.50:
        raise ContractError("synthetic lead-lag tail-hit recovery failed")
    return {
        "tail": "upper",
        "tau": 0.8,
        "lag_rows": 6,
        ...
        "directional_margin": forward - reverse,
    }
...
def synthetic_receipt() -> dict[str, Any]:
    ...
    feature = cross_quantilogram_features(sequence)
    return {
...
def preflight_payload() -> dict[str, Any]:
    ...
    if ARTIFACT.exists() or LOCK.exists():
        raise ContractError("v38 exactly-once namespace is consumed")
    payload = {
        "schema_version": "p3.cross_quantilogram_residual.preflight.v38",
...
def surface_features(cases: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    for position, row in enumerate(cases.itertuples(index=False)):
        ...
        features[position] = cross_quantilogram_features(sequences[anchor_id])
    return features, {
        "rows": len(features),
...
def execute(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    result = {
        "schema_version": "p3.cross_quantilogram_residual.result.v38",
        "experiment_id": EXPERIMENT_ID,
        "created_at_utc": datetime.now(UTC).isoformat(),
        "status": "COMPLETE",
        "decision": "PASS_CANDIDATE_AVAILABLE"
        if passing
        else "NO_GO_ALL_CROSS_QUANTILOGRAM_CANDIDATES",
...
def main() -> int:
    ...
    if ARTIFACT.exists() or REPORT.exists() or LOCK.exists():
        raise ContractError("v38 exactly-once namespace already exists")
    config = load_config()
    ...
    write_new(
        ...
        b"# Claim-source ledger\n\n| Claim | Source | Use |\n|---|---|---|\n| Cross-quantilograms measure quantile dependence and directional predictability through quantile-hit correlations | Han et al., Journal of Econometrics 193, 2016, DOI:10.1016/j.jeconom.2016.03.001 | mechanism only |\n| v30 uses conditional-bin transfer entropy; v38 uses fixed marginal tail-hit correlations | repository semantic audit | novelty boundary |\n| No prior-cycle prediction, router, or threshold is reused | sealed v38 contract | anti-posthoc boundary |\n",
    )
    print(
        json.dumps(
            {
