"""Independent QA for P3 continuous-steepness KMA factor v17."""
...
from run_p3_kma_continuous_steepness_factor_cycle_20260831_v17 import (  # noqa: E402
    ARTIFACT_DIR,
    CALIBRATION,
    CALIBRATION_SHA,
    CONFIG,
...
def main() -> int:
    ...
    recomputed = evaluate(attach_steepness(frame, history), history)
    candidate = result["candidate"]
    lock = json.loads(LOCK.read_text(encoding="utf-8"))
    runner = Path(__file__).with_name("run_p3_kma_continuous_steepness_factor_cycle_20260831_v17.py")
    checks = {
        ...
        "governing_gate_uses_central": "central_raw_points_meets_family_threshold_inclusive" in candidate["gate_checks"],
        "six_ecdf_zero_models": result["fit_budget"]["ecdf_calibration_fits"] == 6 and result["fit_budget"]["model_fits"] == 0,
        "surface_claim_is_not_independent": candidate["surface_claim"] == "adaptive_182_case_development_surface_not_independent_confirmation",
    ...
    payload = {
        "schema_version": "p3.kma_continuous_steepness.independent_qa.v17",
