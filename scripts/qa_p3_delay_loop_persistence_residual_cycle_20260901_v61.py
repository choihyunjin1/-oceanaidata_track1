"""Independent zero-fit QA for P3 v61 H1 loop persistence."""
...
def main() -> int:
    ...
    checks = {
        ...
        "variance_support_present": support["positive_variance_features"] == 32,
        "feature_shape": support["rows"] == 182 and support["columns"] == 32,
        ...
        "no_v36_reuse": "v36/v36r1 features or predictions" in config["duplication_audit"]["forbidden_reuse"],
        "no_official_result_reuse": "v42 official result or slices" in config["duplication_audit"]["forbidden_reuse"],
    }
    payload = {"schema_version": "p3.delay_loop_persistence_residual.independent_qa.v61", "experiment_id": EXPERIMENT_ID, "decision": "PASS" if all(checks.values()) else "FAIL", "checks": checks, "check_count": len(checks), "passed": sum(checks.values()), "failed": len(checks) - sum(checks.values()), "model_fits": 0, "support_receipt": support, "hashes": {"config": sha256(CONFIG), "runner": sha256(RUNNER), "result": sha256(RESULT)}, "official_rows": 0, "csv_materializations": 0, "uploads": 0}
