"""Isolated-snapshot one-shot runner for the P1 Cycle-1 segment screen.
...
OUTPUT_PROJECT_ROOT: Path | None = None
CONFIG_PATH = (
    PROJECT_ROOT
    / "configs"
    / "experiments"
...
BOOTSTRAP_TARGET_FIREWALL_V6_PATH = (
    PROJECT_ROOT
    / "configs"
    / "experiments"
    / "p1_long_event_segment_proposal_rescore_v6_bootstrap_target_firewall_amendment.json"
)
EXECUTION_MODULE_PATH = (
    PROJECT_ROOT / "src" / "p1_qc" / "long_event_segment_proposal_rescore_execution_v6.py"
)
LEGACY_CONFIG_PATH = (
...
TEST_PATH = PROJECT_ROOT / "tests" / "test_run_p1_long_event_segment_proposal_rescore_v6.py"
AUTHORIZATION_TEMPLATE_PATH = (
    PROJECT_ROOT
    / "configs"
    / "experiments"
    / "p1_long_event_segment_proposal_rescore_v6_execution_authorization_template.json"
)
AUTHORIZATION_PATH = (
    PROJECT_ROOT
    / "configs"
    / "experiments"
    / "p1_long_event_segment_proposal_rescore_v6_execution_authorization.json"
)
R6_QA_PATH = (
    PROJECT_ROOT
    / "reports"
    / "p1_long_event_segment_proposal_rescore_v6_independent_preexecution_qa_20260826.json"
)
R5_QA_NO_GO_PATH = (
    PROJECT_ROOT
    / "reports"
...
R5_RUNNER_PATH = PROJECT_ROOT / "scripts" / "run_p1_long_event_segment_proposal_rescore_v2.py"
R5_EXECUTION_MODULE_PATH = (
    PROJECT_ROOT / "src" / "p1_qc" / "long_event_segment_proposal_rescore_execution_v2.py"
)
R5_TEST_PATH = PROJECT_ROOT / "tests" / "test_run_p1_long_event_segment_proposal_rescore_v2.py"
R5_SEAL_PATH = (
    PROJECT_ROOT
    / "artifacts"
    / "p1_long_event_segment_proposal_rescore_20260826_v2"
    / "preexecution_seal_r5.json"
)
ARTIFACT_DIR = (
    PROJECT_ROOT
    / "artifacts"
...
EXPERIMENT_ID = "p1_long_event_segment_proposal_rescore_20260826_v2_infrastructure_v6"
BASE_EXECUTION_EXPERIMENT_ID = "p1_long_event_segment_proposal_rescore_20260826_v2"
SCIENTIFIC_EXPERIMENT_ID = "p1_long_event_segment_proposal_rescore_20260826_v1"
AUTHORIZATION_ENV_VAR = "P1_LONG_EVENT_SEGMENT_V6_AUTHORIZATION_SHA256"
AUTHORIZATION_SCHEMA_VERSION = (
    "p1_long_event_segment_proposal_rescore.execution_authorization.v6"
)
...
BOOTSTRAP_TARGET_FIREWALL_V6_SHA256 = (
    "16d726d217dc2b0d03264ce99eeacd796c8e34017901e02b29bcc76d479846f6"
)
R5_QA_NO_GO_SHA256 = "211779edc945a7923caac6df74709c3149ea51212af0612547ece2f70096a532"
R5_RUNNER_SHA256 = "e97770f6b9a8657adfddb0cddb9a4fbc0416962810094eb4b86ca5bcc75aef84"
R5_EXECUTION_MODULE_SHA256 = (
    "68b644b1523fef498c00e03ee09058a7f9280761de70ec771e09e8c32a768e81"
)
R5_TEST_SHA256 = "d6281d9b38852fc2a31e5b7aebd16b0ac3bc933c24a80eef8ba2002cc8480673"
R5_SEAL_SHA256 = "20619d35471f396eecbb6d7e4e60d452eca63feea9488038663b2d41ce7da21d"
...
EXECUTION_MODULE_SHA256 = "8b91db9234bde301730728dcffa4fd2c014b099d0c518571ddaa733608b81636"
...
HISTORICAL_TARGET_CUTOFF_INCLUSIVE = "2025-03-01T23:50:00+09:00"
...
AUTHORIZATION_SEAL_KEYS = frozenset({"path", "bytes", "sha256"})
AUTHORIZATION_QA_KEYS = frozenset({"path", "bytes", "sha256", "verdict"})
AUTHORIZATION_ZERO_KEYS = frozenset(
    {
        "claims",
        "physical_fits",
...
AUTHORIZATION_OPERATION_KEYS = frozenset(
    {
        "single_attempt",
...
class HeldReadDenyMutation:
    """Keep source/bootstrap identity stable while the worker authenticates.

    Windows uses a read-only sharing mode that denies concurrent write/delete.
    Other platforms retain a read descriptor and verify identity on close; the
    production environment for this sealed experiment is Windows.
    """

    def __init__(self, paths: Sequence[Path]) -> None:
        self.paths = tuple(path.resolve(strict=True) for path in paths)
            ...
            for path in self.paths:
                ...
                if handle in {None, invalid}:
                    ...
                    raise OSError(
                        ...
                        f"unable to hold immutable authority: {path.name}",
                    )
                self._handles.append((kernel32, handle))
                self._digests[path] = _sha256(path)
        else:
            for path in self.paths:
                handle = path.open("rb")
                self._handles.append(handle)
                self._digests[path] = _sha256_bytes(handle.read())
                handle.seek(0)
        return self

    def assert_digest(self, path: Path, expected: str) -> None:
        resolved = path.resolve(strict=True)
        if self._digests.get(resolved) != expected:
            raise RuntimeError(f"held authority digest changed: {resolved.name}")
        ...
        while self._handles:
            ...
            if isinstance(value, tuple):
                ...
                if not kernel32.CloseHandle(handle):
                    raise OSError("unable to close immutable authority handle")
            else:
                value.close()
...
def _require_exact_authorization_schema(value: Any) -> dict[str, Any]:
    """Validate every authorization key and primitive type fail-closed.

    ``bool`` is deliberately not accepted where an integer is required even
    though Python makes it an ``int`` subclass.
    """

    if not isinstance(value, dict) or set(value) != AUTHORIZATION_REQUIRED_KEYS:
    ...
    if value["schema_version"] != AUTHORIZATION_SCHEMA_VERSION:
        raise AuthorizationError("authorization schema version changed")
    for key in (
        "contract_sha256",
        "trust_firewall_v5_sha256",
        "runner_sha256",
        ...
        if not re.fullmatch(r"[0-9a-f]{64}", value[key]):
            raise AuthorizationError(f"authorization digest format changed: {key}")

    seal = value["preexecution_seal"]
    if not isinstance(seal, dict) or set(seal) != AUTHORIZATION_SEAL_KEYS:
        raise AuthorizationError("authorization seal schema membership changed")
    if type(seal["path"]) is not str or type(seal["bytes"]) is not int:
        raise AuthorizationError("authorization seal primitive type changed")
    if seal["bytes"] <= 0 or not isinstance(seal["sha256"], str) or not re.fullmatch(
        r"[0-9a-f]{64}", seal["sha256"]
    ):
        raise AuthorizationError("authorization seal value type changed")

    qa = value["independent_qa"]
    if not isinstance(qa, dict) or set(qa) != AUTHORIZATION_QA_KEYS:
        raise AuthorizationError("authorization QA schema membership changed")
    if (
        type(qa["path"]) is not str
        or type(qa["bytes"]) is not int
        or qa["bytes"] <= 0
        or type(qa["sha256"]) is not str
        or not re.fullmatch(r"[0-9a-f]{64}", qa["sha256"])
        or type(qa["verdict"]) is not str
    ):
        raise AuthorizationError("authorization QA primitive type changed")

    zero = value["zero_prior_state"]
    if not isinstance(zero, dict) or set(zero) != AUTHORIZATION_ZERO_KEYS:
        raise AuthorizationError("authorization zero-state schema membership changed")
    if any(type(item) is not int for item in zero.values()):
        raise AuthorizationError("authorization zero-state primitive type changed")

    operation = value["operation_authorization"]
    if not isinstance(operation, dict) or set(operation) != AUTHORIZATION_OPERATION_KEYS:
        raise AuthorizationError("authorization operation schema membership changed")
    if type(operation["single_attempt"]) is not bool or any(
        type(operation[key]) is not int
        for key in AUTHORIZATION_OPERATION_KEYS - {"single_attempt"}
    ):
        raise AuthorizationError("authorization operation primitive type changed")
    return value
...
def _relative(path: Path) -> str:
    resolved = path.resolve(strict=True)
    for root in (PROJECT_ROOT, OUTPUT_PROJECT_ROOT):
        if root is not None and resolved.is_relative_to(root):
            return resolved.relative_to(root).as_posix()
    raise RuntimeError("path is outside sealed project/output roots")
    ...
    for root in (PROJECT_ROOT, OUTPUT_PROJECT_ROOT):
        if root is not None:
            try:
                ...
                continue
    raise RuntimeError("literal path is outside sealed project/output roots")


def _resolve_repo_path(relative: str, *, must_exist: bool = True) -> Path:
    root = PROJECT_ROOT
    if OUTPUT_PROJECT_ROOT is not None and relative.startswith(
        "artifacts/p1_long_event_segment_proposal_rescore_20260826_v2_infrastructure_v6/"
    ):
        root = OUTPUT_PROJECT_ROOT
...
def _validate_closure(
    ...
    if contract.get("experiment_id") != BASE_EXECUTION_EXPERIMENT_ID:
...
def _static_package_checks(
    ...
    expected_paths = {
        ...
        BOOTSTRAP_TARGET_FIREWALL_V6_PATH: BOOTSTRAP_TARGET_FIREWALL_V6_SHA256,
        R5_QA_NO_GO_PATH: R5_QA_NO_GO_SHA256,
        R5_RUNNER_PATH: R5_RUNNER_SHA256,
        R5_EXECUTION_MODULE_PATH: R5_EXECUTION_MODULE_SHA256,
        R5_TEST_PATH: R5_TEST_SHA256,
        R5_SEAL_PATH: R5_SEAL_SHA256,
    ...
    v6 = _json_from_bytes(
        raws[BOOTSTRAP_TARGET_FIREWALL_V6_PATH],
        label=BOOTSTRAP_TARGET_FIREWALL_V6_PATH.name,
    )
    r5_qa = _json_from_bytes(raws[R5_QA_NO_GO_PATH], label=R5_QA_NO_GO_PATH.name)
    ...
    if (
        v6.get("amendment_id") != EXPERIMENT_ID
        or v6.get("status") != "PROSPECTIVE_INFRASTRUCTURE_RECOVERY_NOT_AUTHORIZED"
        or v6.get("prospective") is not True
        or v6.get("created_before_v6_implementation_test_seal_claim_fit_materialization_or_score")
        is not True
        or v6["unchanged_scientific_contract"]["maximum_lifetime_physical_fits"] != 72
        or v6["unchanged_scientific_contract"]["maximum_scientific_materializations"]
        != 21
        or v6["unchanged_scientific_contract"][
            "hypothesis_features_windows_split_model_seeds_postprocess_and_gates_changed"
        ]
        is not False
        or any(int(value) != 0 for value in v6["prohibitions"].values())
    ):
        raise RuntimeError("v6 bootstrap/target-firewall authority changed")
    predecessor = v6["predecessor_r5"]
    predecessor_checks = {
        "runner": (R5_RUNNER_SHA256, R5_RUNNER_PATH.stat().st_size),
        "execution_module": (
            R5_EXECUTION_MODULE_SHA256,
            R5_EXECUTION_MODULE_PATH.stat().st_size,
        ),
        "tests": (R5_TEST_SHA256, R5_TEST_PATH.stat().st_size),
        "seal": (R5_SEAL_SHA256, R5_SEAL_PATH.stat().st_size),
    }
    for name, (digest, size) in predecessor_checks.items():
        if (
            predecessor[name]["sha256"] != digest
            or int(predecessor[name]["bytes"]) != int(size)
        ):
            raise RuntimeError(f"v6 predecessor pin changed: {name}")
    if (
        r5_qa.get("verdict") != "NO_GO"
        or r5_qa.get("severity_counts") != {"P0": 1, "P1": 1, "P2": 1}
        or r5_qa.get("seal_sha256") != R5_SEAL_SHA256
        or r5_qa.get("runner_sha256") != R5_RUNNER_SHA256
        or r5_qa.get("execution_module_sha256") != R5_EXECUTION_MODULE_SHA256
    ):
        raise RuntimeError("r5 NO_GO evidence changed")
    ...
    return {
        ...
        "bootstrap_target_firewall_v6_sha256": BOOTSTRAP_TARGET_FIREWALL_V6_SHA256,
        "r5_no_go_sha256": R5_QA_NO_GO_SHA256,
...
def _runtime_receipt(legacy_config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    executable_raw, executable_receipt = _read_bound_bytes(executable_path)
    return {
        "runtime_versions": observed,
        "lightgbm_native": receipt | dict(native),
        "python_executable": {
            "path": str(executable_path),
            "bytes": len(executable_raw),
            "sha256": executable_receipt["actual_read_sha256"],
        },
        "explicit_import_inventory": _explicit_runtime_import_inventory(
            legacy_config
        ),
    }
...
def _explicit_runtime_import_inventory(
    legacy_config: Mapping[str, Any],
) -> dict[str, Any]:
    expected_versions = legacy_config["trust_contract"]["runtime_versions"]
    package_map = importlib.metadata.packages_distributions()
    records: dict[str, Any] = {}
    search_paths: set[str] = set()
    for distribution_name, expected_version in sorted(expected_versions.items()):
        if distribution_name in {"python", "p1-qc"}:
            continue
        distribution = importlib.metadata.distribution(distribution_name)
        if distribution.version != expected_version:
            raise RuntimeError(f"runtime distribution version changed: {distribution_name}")
        root = Path(distribution.locate_file("")).resolve(strict=True)
        metadata_root = Path(str(getattr(distribution, "_path", ""))).resolve(strict=True)
        metadata_path = (metadata_root / "METADATA").resolve(strict=True)
        if not metadata_path.is_file() or not metadata_path.is_relative_to(root):
            raise RuntimeError(f"runtime metadata path changed: {distribution_name}")
        normalised = _normalise_distribution_name(distribution_name)
        top_levels = sorted(
            name
            for name, owners in package_map.items()
            if any(_normalise_distribution_name(owner) == normalised for owner in owners)
        )
        if not top_levels:
            raise RuntimeError(f"runtime distribution has no explicit import root: {distribution_name}")
        entrypoints: dict[str, Any] = {}
        for top_level in top_levels:
            specification = importlib.util.find_spec(top_level)
            if specification is None:
                raise RuntimeError(f"runtime import root cannot be resolved: {top_level}")
            origin = specification.origin
            if origin in {None, "built-in", "frozen"}:
                continue
            entry = Path(str(origin)).resolve(strict=True)
            if not entry.is_file() or not entry.is_relative_to(root):
                raise RuntimeError(f"runtime import entry escapes sealed root: {top_level}")
            entrypoints[top_level] = {
                "relative_path": entry.relative_to(root).as_posix(),
                "bytes": entry.stat().st_size,
                "sha256": _sha256(entry),
            }
        if not entrypoints:
            raise RuntimeError(f"runtime distribution import entries are empty: {distribution_name}")
        records[distribution_name] = {
            ...
            "top_level_entrypoints": entrypoints,
        }
        search_paths.add(str(root))
    return {
        "schema_version": "p1_segment_rescore.explicit_runtime_import_inventory.v6",
...
def _activate_explicit_runtime_import_inventory(inventory: Mapping[str, Any]) -> None:
    if inventory.get("schema_version") != (
        "p1_segment_rescore.explicit_runtime_import_inventory.v6"
    ):
        raise RuntimeError("explicit runtime inventory schema changed")
    if inventory.get("site_module_processed") is not False or inventory.get(
        "pth_files_processed"
    ) != 0:
        raise RuntimeError("runtime inventory would process startup hooks")
    distributions = inventory.get("distributions")
    search_paths = inventory.get("search_paths")
    if not isinstance(distributions, dict) or not isinstance(search_paths, list):
        raise RuntimeError("runtime import inventory types changed")
    observed_roots: set[str] = set()
    for distribution_name, record in sorted(distributions.items()):
        if not isinstance(record, dict):
            raise RuntimeError("runtime distribution record type changed")
        root = Path(str(record["search_path"])).resolve(strict=True)
        observed_roots.add(str(root))
        metadata = (root / str(record["metadata_relative_path"])).resolve(strict=True)
        if (
            not metadata.is_relative_to(root)
            or metadata.stat().st_size != int(record["metadata_bytes"])
            or _sha256(metadata) != record["metadata_sha256"]
        ):
            raise RuntimeError(f"runtime metadata pin changed: {distribution_name}")
        entrypoints = record.get("top_level_entrypoints")
        if not isinstance(entrypoints, dict) or not entrypoints:
            raise RuntimeError(f"runtime import entrypoints changed: {distribution_name}")
        for top_level, entry_record in sorted(entrypoints.items()):
            entry = (root / str(entry_record["relative_path"])).resolve(strict=True)
            if (
                not entry.is_relative_to(root)
                or entry.stat().st_size != int(entry_record["bytes"])
                or _sha256(entry) != entry_record["sha256"]
            ):
                raise RuntimeError(f"runtime import entry pin changed: {top_level}")
    if observed_roots != set(str(value) for value in search_paths):
        raise RuntimeError("runtime explicit search path membership changed")
...
def _prepare_snapshot(
    ...
    root = Path(tempfile.mkdtemp(prefix="p1_segment_v6_verified_snapshot_"))
    ...
    try:
        ...
        fixed = {
            ...
            _relative(BOOTSTRAP_TARGET_FIREWALL_V6_PATH): (
                BOOTSTRAP_TARGET_FIREWALL_V6_SHA256
            ),
            _relative(R5_QA_NO_GO_PATH): R5_QA_NO_GO_SHA256,
            _relative(R5_RUNNER_PATH): R5_RUNNER_SHA256,
            _relative(R5_EXECUTION_MODULE_PATH): R5_EXECUTION_MODULE_SHA256,
            _relative(R5_TEST_PATH): R5_TEST_SHA256,
            _relative(R5_SEAL_PATH): R5_SEAL_SHA256,
            ...
            _relative(TEST_PATH): _sha256(TEST_PATH),
            _relative(AUTHORIZATION_TEMPLATE_PATH): _sha256(
                AUTHORIZATION_TEMPLATE_PATH
            ),
...
def _cleanup_snapshot(root: Path) -> None:
    ...
    if resolved.parent != temporary_root or not resolved.name.startswith(
        "p1_segment_v6_verified_snapshot_"
    ):
...
def _load_snapshot_numerical(
    ...
    legacy_config: Mapping[str, Any],
    legacy: ModuleType,
    execution_source_bytes: bytes | None = None,
    ...
    raw = execution_source_bytes
    if raw is None:
        raw, _receipt = _read_bound_bytes(
            expected_path,
            expected_sha256=EXECUTION_MODULE_SHA256,
        )
    elif _sha256_bytes(raw) != EXECUTION_MODULE_SHA256:
        raise RuntimeError("held numerical execution module bytes changed")
    module_name = "p1_qc.long_event_segment_proposal_rescore_execution_v6"
    module = ModuleType(module_name)
    module.__file__ = str(expected_path)
    module.__package__ = "p1_qc"
    ...
    try:
        exec(compile(raw, str(expected_path), "exec"), module.__dict__)
    except BaseException:
    ...
    if (
        module.BOOTSTRAP_TARGET_FIREWALL_V6_SHA256
        != BOOTSTRAP_TARGET_FIREWALL_V6_SHA256
    ):
        raise RuntimeError("numerical execution module v6 trust anchor changed")
    return numerical, module, runtime


def _target_free_train_and_bounded_historical_targets(
    raw: bytes,
    numerical: ModuleType,
    *,
    historical_suffix_decoder: Any | None = None,
) -> tuple[Any, Any, dict[str, Any]]:
    """Parse no outer target field before freeze.

    pandas receives only the seven target-free columns.  The separate binary
    line scanner decodes target fields exclusively for rows at or before the
    frozen historical cutoff; protected later rows are never split, indexed,
    decoded, converted, or aggregated here.
    """

    expected_header = b"station,year,layer,time,temp,psal,depth,label,anomaly_type"
    handle = io.BytesIO(raw)
    header = handle.readline().rstrip(b"\r\n")
    if header != expected_header:
        raise RuntimeError("training source header changed")
    target_free_lines = [b",".join(expected_header.split(b",")[:7]) + b"\n"]
    opaque_suffixes: list[bytes] = []
    for line in handle:
        stripped = line.rstrip(b"\r\n")
        fields = stripped.split(b",", 7)
        if len(fields) != 8:
            raise RuntimeError("training source target boundary changed")
        target_free_lines.append(b",".join(fields[:7]) + b"\n")
        # This suffix remains an uninterpreted byte string until the row time
        # has independently proved that it belongs to the bounded historical
        # prefix.  In particular it is never passed to pandas pre-freeze.
        opaque_suffixes.append(fields[7])
    target_free = numerical.pd.read_csv(
        io.BytesIO(b"".join(target_free_lines)),
        low_memory=False,
    )
    if tuple(target_free.columns) != TARGET_FREE_TRAIN_COLUMNS:
        raise RuntimeError("target-free training column order changed")
    if {"label", "anomaly_type"}.intersection(target_free.columns):
        raise RuntimeError("protected target leaked into pre-freeze training view")
    parsed = numerical.pd.to_datetime(
        target_free["time"],
        errors="raise",
        utc=True,
        format="mixed",
    )
    cutoff = numerical.pd.Timestamp(HISTORICAL_TARGET_CUTOFF_INCLUSIVE).tz_convert("UTC")
    historical_mask = parsed.le(cutoff).to_numpy(dtype=bool)
    historical_positions = numerical.np.flatnonzero(historical_mask)
    labels: list[int] = []
    anomaly_types: list[str] = []
    decoded_historical_rows = 0
    opaque_protected_rows = 0
    if len(opaque_suffixes) != len(target_free):
        raise RuntimeError("training source row count differs from target-free view")
    decoder = historical_suffix_decoder or _decode_historical_target_suffix
    for ordinal, suffix in enumerate(opaque_suffixes):
        if not historical_mask[ordinal]:
            opaque_protected_rows += 1
            continue
        label, anomaly_type = decoder(suffix, ordinal)
        if label not in (0, 1):
            raise RuntimeError("historical target label is not binary")
        labels.append(label)
        anomaly_types.append(anomaly_type)
        decoded_historical_rows += 1
    historical = target_free.iloc[historical_positions][
        ["station", "year", "layer", "time"]
    ].copy()
    historical.insert(
        0,
        "row_position",
        historical_positions.astype(numerical.np.int64),
    )
    historical["label"] = numerical.np.asarray(labels, dtype=numerical.np.int8)
    historical["anomaly_type"] = numerical.pd.Series(
        anomaly_types,
        index=historical.index,
        dtype="string",
    )
    historical.reset_index(drop=True, inplace=True)
    if len(historical) != decoded_historical_rows or not len(historical):
        raise RuntimeError("bounded historical target row count changed")
    if historical["row_position"].duplicated().any():
        raise RuntimeError("bounded historical target positions are duplicated")
    historical_time = parsed.iloc[historical_positions]
    if historical_time.max() > cutoff:
        raise RuntimeError("bounded historical target exceeds frozen cutoff")
    return (
        target_free,
        historical,
        {
            "status": "PASS_PHYSICALLY_BOUNDED_HISTORICAL_TARGET_VIEW",
            "historical_cutoff_inclusive": HISTORICAL_TARGET_CUTOFF_INCLUSIVE,
            "historical_rows_decoded": decoded_historical_rows,
            "protected_outer_rows_left_opaque": opaque_protected_rows,
            "pre_freeze_protected_target_rows_decoded": 0,
            "target_free_columns": list(TARGET_FREE_TRAIN_COLUMNS),
            "historical_max_time_utc": historical_time.max().isoformat(),
        },
    )


def _decode_historical_target_suffix(suffix: bytes, ordinal: int) -> tuple[int, str]:
    del ordinal
    decoded = suffix.decode("utf-8")
    values = next(csv.reader([decoded]))
    if len(values) != 2:
        raise RuntimeError("historical target suffix CSV width changed")
...
def _strict_target_free_snapshot_readiness(
    ...
    train_source_raw = reader.read_bytes(
        "inputs/train.csv",
        str(cache_spec["source_sha256"]),
    )
    train, historical_targets, historical_receipt = (
        _target_free_train_and_bounded_historical_targets(
            train_source_raw,
            numerical,
        )
    )
    ...
    overlap = historical_targets.loc[:, list(legacy.KEY_COLUMNS)].merge(
        truth_keys.loc[:, list(legacy.KEY_COLUMNS)],
        on=list(legacy.KEY_COLUMNS),
        how="inner",
    )
    if len(overlap):
        raise RuntimeError("bounded historical targets contain registered outer keys")
    historical_receipt["registered_outer_key_overlap"] = 0
    historical_receipt["registered_outer_rows"] = len(truth_keys)
    ...
    receipt = {
        ...
        "historical_target_firewall": historical_receipt,
        "left_censor_audit": {
            "status": "DEFERRED_UNTIL_AFTER_ALL_FOLD_OUTER_FREEZE",
            "pre_freeze_label_accesses": 0,
        },
    ...
    state = {
        "train_target_free": train,
        "historical_targets": historical_targets,
        "train_source_opaque_bytes": train_source_raw,
        "train_source_sha256": str(cache_spec["source_sha256"]),
        "bundle": bundle,
...
def _selected_readiness(readiness: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "historical_target_firewall": readiness["historical_target_firewall"],
        "left_censor_audit": readiness["left_censor_audit"],
        ...
        "runtime": {
            ...
            "explicit_import_inventory": runtime["explicit_import_inventory"],
        },
        "official_test_reads": 0,
        "sample_format_reads": 0,
        "submission_candidate_reads": 0,
...
def _complete_readiness(
    *,
    ...
    try:
        ...
        loaded_runtime["explicit_import_inventory"] = runtime[
            "explicit_import_inventory"
        ]
        loaded_runtime["python_executable"] = runtime["python_executable"]
        ...
        result = {
            ...
            "static": {
                ...
                "bootstrap_target_firewall_v6_sha256": static[
                    "bootstrap_target_firewall_v6_sha256"
                ],
                "r5_no_go_sha256": static["r5_no_go_sha256"],
                "superseded_v1_preserved": True,
            },
            "readiness": selected,
...
def _require_execution_authorization_preimport() -> dict[str, Any]:
    ...
    authorization = _require_exact_authorization_schema(
        _json_from_bytes(raw, label=AUTHORIZATION_PATH.name)
    )
    if (
    ...
    if qa_spec.get("path") != _relative(R6_QA_PATH):
...
def _verify_seal_value(
    ...
    checks = {
        ...
        "trust_firewall_v5_sha256": TRUST_FIREWALL_V5_SHA256,
        "bootstrap_target_firewall_v6_sha256": BOOTSTRAP_TARGET_FIREWALL_V6_SHA256,
        "r5_no_go_qa_sha256": R5_QA_NO_GO_SHA256,
...
def seal() -> Path:
    ...
    receipt = {
        "schema_version": "p1_long_event_segment_proposal_rescore.preexecution_seal.v6",
        "experiment_id": EXPERIMENT_ID,
        ...
        "supersedes": {
            "r5_runner_sha256": R5_RUNNER_SHA256,
            "r5_execution_module_sha256": R5_EXECUTION_MODULE_SHA256,
            "r5_test_sha256": R5_TEST_SHA256,
            "r5_preexecution_seal_sha256": R5_SEAL_SHA256,
            "r5_independent_qa_no_go_sha256": R5_QA_NO_GO_SHA256,
            "r5_status": "IMMUTABLE_NO_GO_P0_BOOTSTRAP_P1_PREFREEZE_TARGET_P2_AUTH_SCHEMA",
...
def _snapshot_manifest(
    ...
    bootstrap_relative = _relative_literal(Path(__file__).resolve())
    manifest = {
        "schema_version": "p1_segment_rescore.private_snapshot.v6",
        "experiment_id": EXPERIMENT_ID,
        ...
        "runner_sha256": _sha256(Path(__file__).resolve()),
        "runner_normalized_sha256": _normalised_runner_sha256(),
        "bootstrap_runner_relative_path": bootstrap_relative,
        "bootstrap_runner_sha256": _sha256(Path(__file__).resolve()),
        "required_python_flags": {"isolated": 1, "no_site": 1, "dont_write_bytecode": 1},
        "output_project_root": str(PROJECT_ROOT.resolve(strict=True)),
        "output_artifact_relative_path": _relative_literal(ARTIFACT_DIR),
...
def _assert_isolated_snapshot_bootstrap(
    snapshot_manifest_path: Path,
    expected_bootstrap_sha256: str,
) -> None:
    """Prove that no environment/site startup hook preceded this bootstrap."""
    ...
    if flags != {"isolated": 1, "no_site": 1, "dont_write_bytecode": 1}:
        raise AuthorizationError("worker Python isolation flags changed")
    if any(name in sys.modules for name in ("site", "sitecustomize", "usercustomize")):
        raise AuthorizationError("worker startup hook module was imported")
    if not re.fullmatch(r"[0-9a-f]{64}", expected_bootstrap_sha256):
        raise AuthorizationError("worker bootstrap digest is malformed")
    manifest_root = snapshot_manifest_path.parent.resolve(strict=True)
    expected_runner = (
        manifest_root / _relative_literal(Path(__file__).resolve())
    ).resolve(strict=True)
    actual_runner = Path(__file__).resolve(strict=True)
    if actual_runner != expected_runner or not actual_runner.is_relative_to(manifest_root):
        raise AuthorizationError("worker did not start from the private snapshot")
    if _sha256(actual_runner) != expected_bootstrap_sha256:
        raise AuthorizationError("worker bootstrap bytes changed")


def _bind_output_namespace_from_manifest(manifest: Mapping[str, Any]) -> None:
    global ARTIFACT_DIR, OUTPUT_PROJECT_ROOT, SEAL_PATH

    root_value = manifest.get("output_project_root")
    relative = manifest.get("output_artifact_relative_path")
    if type(root_value) is not str or type(relative) is not str:
        raise RuntimeError("snapshot output namespace types changed")
    if relative != (
        "artifacts/"
        "p1_long_event_segment_proposal_rescore_20260826_v2_infrastructure_v6"
    ):
        raise RuntimeError("snapshot output artifact namespace changed")
    root = Path(root_value).resolve(strict=True)
    artifact = (root / relative).resolve(strict=False)
    if not artifact.is_relative_to(root):
        raise RuntimeError("snapshot output namespace escapes project root")
    OUTPUT_PROJECT_ROOT = root
...
def _verify_exact_snapshot_inventory(
    ...
    preverified_records: Mapping[str, Mapping[str, Any]] | None = None,
) -> None:
    ...
    executable_pins = {
        _relative_literal(Path(__file__).resolve()): _sha256(Path(__file__).resolve()),
    ...
    for relative, record in expected_inventory.items():
        ...
        held = (preverified_records or {}).get(relative)
        if held is not None:
            if (
                int(held["bytes"]) != int(record["bytes"])
                or held["sha256"] != record["sha256"]
            ):
                raise RuntimeError(f"snapshot held digest changed: {relative}")
...
def _verify_snapshot_manifest(
    ...
    _assert_isolated_snapshot_bootstrap(path, expected_bootstrap_sha256)
    ...
    checks = {
        ...
        "runner_sha256": _sha256(Path(__file__).resolve()),
        "runner_normalized_sha256": _normalised_runner_sha256(),
        "bootstrap_runner_relative_path": _relative_literal(Path(__file__).resolve()),
        "bootstrap_runner_sha256": expected_bootstrap_sha256,
    ...
    qa_relative = _relative_literal(R6_QA_PATH)
    ...
    qa = _json_from_bytes(qa_raw, label=R6_QA_PATH.name)
    if (
    ...
    held_snapshot_raw: dict[str, bytes] = {
        auth_relative: auth_raw,
        seal_relative: seal_raw,
        qa_relative: qa_raw,
    }
    held_snapshot_receipts: dict[str, dict[str, Any]] = {
        auth_relative: {
            "bytes": len(auth_raw),
            "sha256": auth_receipt["actual_read_sha256"],
        },
        seal_relative: {
            "bytes": len(seal_raw),
            "sha256": seal_receipt["actual_read_sha256"],
    ...
    for authority_path, expected in (
        (DESIGN_PATH, DESIGN_SHA256),
        (EXECUTION_MODULE_PATH, EXECUTION_MODULE_SHA256),
        (TRUST_FIREWALL_V5_PATH, TRUST_FIREWALL_V5_SHA256),
    ):
        relative = _relative_literal(authority_path)
        raw, receipt = _read_bound_bytes(root / relative, expected_sha256=expected)
        held_snapshot_raw[relative] = raw
        held_snapshot_receipts[relative] = {
            "bytes": len(raw),
            "sha256": receipt["actual_read_sha256"],
        }
    _verify_exact_snapshot_inventory(
        root,
        entries,
        expected_inventory,
        held_snapshot_receipts,
    )
    ...
    _bind_output_namespace_from_manifest(manifest)
    return (
        root,
        manifest,
        {
            ...
            "snapshot_held_raw": held_snapshot_raw,
            "snapshot_held_receipts": held_snapshot_receipts,
...
def _load_worker_state(
    ...
    sealed_runtime = manifest["strict_readiness"]["runtime"]
    runtime["explicit_import_inventory"] = sealed_runtime[
        "explicit_import_inventory"
    ]
    runtime["python_executable"] = sealed_runtime["python_executable"]
...
def _worker_execute(
    ...
    sealed_runtime = manifest["strict_readiness"]["runtime"]
    executable = sealed_runtime["python_executable"]
    current_executable = Path(sys.executable).resolve(strict=True)
    if (
        str(current_executable) != executable["path"]
        or current_executable.stat().st_size != int(executable["bytes"])
        or _sha256(current_executable) != executable["sha256"]
    ):
        raise RuntimeError("worker Python executable pin changed")
    _activate_explicit_runtime_import_inventory(
        sealed_runtime["explicit_import_inventory"]
    )
    held_receipts = authorization["snapshot_held_receipts"]
    _verify_exact_snapshot_inventory(
        snapshot_root,
        manifest["files"],
        manifest["files"],
    ...
    execution_relative = _relative_literal(EXECUTION_MODULE_PATH)
    numerical, execution, readiness, state = _load_worker_state(
        snapshot_root,
        manifest,
        authorization["snapshot_held_raw"][execution_relative],
    )
    _verify_exact_snapshot_inventory(
        snapshot_root,
        manifest["files"],
...
def _worker_command(
    ...
    snapshot_runner = snapshot_root / _relative_literal(Path(__file__).resolve())
    bootstrap_sha256 = _sha256(snapshot_runner)
    return [
        sys.executable,
        "-I",
        "-S",
...
def _parent_read_only_verify(result_path: Path) -> dict[str, Any]:
    ...
    qa_record = held[_relative_literal(R6_QA_PATH)]
...
def _parent_authority_guard_paths() -> tuple[Path, ...]:
    values = {
        Path(__file__).resolve(),
        CONFIG_PATH,
        DESIGN_PATH,
        ...
        R5_TEST_PATH,
        R5_SEAL_PATH,
        *(_resolve_repo_path(relative) for relative in PROJECT_FILE_SHA256),
    }
    raw_data = os.environ.get("P1_DATA_DIR")
    if raw_data:
        data_root = Path(raw_data).expanduser().resolve(strict=True)
        train_path = (data_root / "train.csv").resolve(strict=True)
        if train_path.parent != data_root:
            raise RuntimeError("training source guard escapes P1_DATA_DIR")
        values.add(train_path)
...
def execute_parent() -> tuple[Path, dict[str, Any]]:
    ...
    with HeldReadDenyMutation(_parent_authority_guard_paths()) as guards:
        for path, expected in (
            (Path(__file__).resolve(), authorization["value"]["runner_sha256"]),
            (
                AUTHORIZATION_PATH,
                authorization["external_authorization_sha256"],
            ),
            (SEAL_PATH, authorization["seal_read_receipt"]["actual_read_sha256"]),
            (R6_QA_PATH, authorization["qa_read_receipt"]["actual_read_sha256"]),
            (DESIGN_PATH, DESIGN_SHA256),
            (EXECUTION_MODULE_PATH, EXECUTION_MODULE_SHA256),
            (TRUST_FIREWALL_V5_PATH, TRUST_FIREWALL_V5_SHA256),
        ):
            guards.assert_digest(path, expected)
        return _execute_parent_with_held_authorities(started, authorization)


def _execute_parent_with_held_authorities(
    started: float,
    authorization: Mapping[str, Any],
    ...
    try:
        ...
        with HeldReadDenyMutation(guarded_snapshot_authorities) as bootstrap_guard:
            bootstrap_guard.assert_digest(
                snapshot_runner,
                _sha256(Path(__file__).resolve()),
            )
            stdout, _stderr = _run_supervised(
                _worker_command(snapshot_root, manifest_path, manifest_sha, deadline),
                deadline,
                snapshot_root,
            )
