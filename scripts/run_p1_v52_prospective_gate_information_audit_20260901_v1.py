"""Zero-fit prospective audit of one independently preserved v28 reject."""
...
EXPERIMENT_ID = "p1_v52_prospective_gate_information_audit_20260901_v1"
...
def preflight(data_dir: Path) -> dict[str, Any]:
    ...
    if rule["selected_parent"] != config["sealed_parent"]["experiment_id"] or rule["target_information_used_for_parent_selection"] != 0:
        raise RuntimeError("parent selection rule drifted")
    parent = config["sealed_parent"]
    for key in ("config", "result", "qa", "manifest", "bundle", "q2_part"):
        path = ROOT / parent[f"{key}_path"]
        if _sha(path) != parent[f"{key}_sha256"]:
            raise RuntimeError(f"sealed parent {key} drifted")
    if _sha(ROOT / config["prior_audit_boundary"]["v35_report_path"]) != config["prior_audit_boundary"]["v35_report_sha256"]:
        raise RuntimeError("v35 boundary drifted")

    result = _read(ROOT / parent["result_path"])
    qa = _read(ROOT / parent["qa_path"])
    ...
    if qa["verdict"] != "PASS" or qa["checks"]["q2_q3_target_zero"] is not True:
        raise RuntimeError("parent QA is not audit eligible")
    ...
    if manifest["q2_target_reads_before_seal"] != 0 or manifest["q3_q4_target_reads"] != 0:
        raise RuntimeError("parent bundle was not sealed target-free")
    ...
    return {
        ...
        "parent_selection_rule": rule["rule"],
        "sealed_parent_hashes": {key: parent[f"{key}_sha256"] for key in ("config", "result", "qa", "manifest", "bundle", "q2_part")},
        ...
        "counters": {"fits": 0, "refits": 0, "score_recomputations": 0, "reselection": 0, "q2_target_reads": 0, "q3_target_reads": 0, "q4_target_reads": 0, "official": 0, "test": 0, "sample_submission": 0, "submission": 0, "hidden": 0, "csv": 0, "uploads": 0},
    }
...
def execute(data_dir: Path) -> dict[str, Any]:
    ...
    seal = {
        "experiment_id": EXPERIMENT_ID,
        "parent_selection_rule": ready["parent_selection_rule"],
        ...
        "fits_refits_score_recomputation_reselection": [0, 0, 0, 0],
        "q2_q3_q4_target_reads_before_seal": [0, 0, 0],
        "promotion_rescue_retune_csv": [0, 0, 0, 0],
    }
    ...
    recommendation = (
        "Do not rescue or promote v38. This outcome motivates only a future preregistered guard-design study with new independent candidates."
        if false_negative
        else "Keep v28 unchanged prospectively; this fixed reject was nonbeneficial, while two reject audits remain insufficient to estimate sensitivity."
    )
    completion = {"experiment_id": EXPERIMENT_ID, "status": "TERMINAL_AUDIT_ONLY", "q2_target_reads": 1, "q3_target_reads": 0, "q4_target_reads": 0, "parent_decision_unchanged": True, "promotion_rescue_retune_csv": [0, 0, 0, 0]}
    _write(ARTIFACT / "completion.json", completion)
    result = {
        "schema_version": config["result_schema_version"],
        "experiment_id": EXPERIMENT_ID,
        "decision": diagnosis,
        "scope": "AUDIT_ONLY_NO_PROMOTION_RESCUE_RETUNE_OR_CSV",
        ...
        "guard_diagnosis": {"false_negative": false_negative, "recommendation": recommendation, "parent_v38_decision_unchanged": True, "overall_sensitivity_identifiable": False},
        "counters": {"fits": 0, "refits": 0, "score_recomputations": 0, "threshold_reselections": 0, "q2_target_reads": 1, "q3_target_reads": 0, "q4_target_reads": 0, "official": 0, "test": 0, "sample_submission": 0, "submission": 0, "hidden": 0, "csv": 0, "uploads": 0},
...
def qa(data_dir: Path) -> dict[str, Any]:
    ...
    if not result_path.exists():
        ...
        checks = {
            ...
            "max_lcb_fixed": ready["selected_candidate"]["candidate_index"] == 1,
            "bundle_target_free": not ready["bundle_receipt"]["target_values_present"],
            "q3_q4_unopened": config["contract"]["q3_q4_target_reads"] == 0,
            "no_rescue_or_csv": config["contract"]["candidate_promotion_rescue_retune"] == config["contract"]["csv_materialization"] == 0,
    ...
    checks = {
        ...
        "zero_fit_refit_score_recompute_reselection": counters["fits"] == counters["refits"] == counters["score_recomputations"] == counters["threshold_reselections"] == 0,
        "only_q2_opened": counters["q2_target_reads"] == 1 and counters["q3_target_reads"] == counters["q4_target_reads"] == 0,
        "parent_unchanged": result["guard_diagnosis"]["parent_v38_decision_unchanged"],
        "no_promotion_rescue_retune_csv": completion["promotion_rescue_retune_csv"] == [0, 0, 0, 0],
        "access0": all(counters[key] == 0 for key in ("official", "test", "sample_submission", "submission", "hidden", "csv", "uploads")),
