"""Independent QA for sealed P2 v39 target-layer AND-mask run."""
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        task = receipt["weight_receipt"]["gradient_task_contract"]
        task_contract_ok &= task["tasks"] == [2, 3, 4]
        task_contract_ok &= task["expected_tasks"] == [2, 3, 4]
        task_contract_ok &= task["exact"]
        task_contract_ok &= sum(task["rows"]) == receipt["training_rows"]
        for fit in receipt["fit_receipts"]:
            fit_receipts_ok &= bool(
                np.isfinite(
                    [
                        fit["loss_first"],
                        fit["loss_last"],
                        fit["mean_kept_coordinate_share"],
                        fit["minimum_kept_coordinate_share"],
                        fit["maximum_kept_coordinate_share"],
                    ]
                ).all()
            )
            ...
            fit_receipts_ok &= fit["gradient_mask_steps"] == fit["optimizer_steps"]
            fit_receipts_ok &= fit["task_order"] == [2, 3, 4]
            fit_receipts_ok &= fit["task_order_used_for_result"] is False
            fit_receipts_ok &= fit["agreement_threshold"] == 1.0
            fit_receipts_ok &= all(
                int(value) >= 2
                for value in fit["minimum_task_rows_per_batch"].values()
            )
            fit_receipts_ok &= fit["projection_steps"] == 0
            fit_receipts_ok &= fit["partial_agreement_steps"] == 0
            fit_receipts_ok &= fit["group_reweighting_steps"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True

    semantic = runner.semantic_audit(config)
    mask_receipt = runner._and_mask_contract_receipt()
    isolation = runner.v37._isolation_receipt()
    ...
    checks = {
        ...
        "new_p2_and_mask_axis": semantic["classification"]
        == "NEW_P2_TARGET_LAYER_PARAMETER_COORDINATE_GRADIENT_SIGN_UNANIMITY",
        "p2_execution_hits_zero": semantic["repository_p2_exact_execution_hits"] == 0,
        "cross_problem_adjacency_disclosed": semantic[
            "p1_v45_cross_problem_adjacency_disclosed"
        ],
        ...
        "and_mask_contract": all(
            mask_receipt[name]
            for name in (
                "formula_exact",
                "conflicting_coordinate_zeroed",
                "zero_coordinate_non_unanimous",
                "all_agree_mean_identity",
                "task_permutation_invariant",
                "finite",
            )
        ),
        ...
        "task_contract": task_contract_ok,
    ...
    qa = {
        "schema_version": "p2.target_layer_gradient_sign_unanimity_deepset.independent_qa.20260901.v39",
