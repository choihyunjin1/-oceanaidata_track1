...
from run_p3_numeric_lead_forward_gpu_20260906_v2 import (
    CONFIG,
    KEYS,
    LEADS,
    OUT,
...
def run():
    ...
    check(
        ...
        len(result["fit_receipts"]) == result["fit_count"] == 15
        and result["router_fit_count"] == 8,
    )
    check(
        "actual_models_15_plus8",
        len(list((OUT / "models").rglob("*.cbm"))) == 15
        and len(list((OUT / "models").rglob("*.joblib"))) == 8,
    )
    check(
    ...
    check(
        "shared_fresh_multi_exact",
        np.array_equal(base.multi_prediction, candidate.multi_prediction),
    )
    check(
        "finite_range",
    ...
    check(
        "replay_exact15plus8",
        replay["status"] == "PASS"
        and replay["backbone_models"] == 15
        and replay["router_models"] == 8
