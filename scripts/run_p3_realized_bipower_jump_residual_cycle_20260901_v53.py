"""Sealed P3 v53 realized bipower/jump residual experiment."""
...
import run_p3_variational_mode_residual_cycle_20260901_v52 as v52  # noqa: E402

EXPERIMENT_ID = "p3_realized_bipower_jump_residual_cycle_20260901_v53"
...
LAGS, WINDOWS, EPSILON = (1, 3, 6), ((0, 145), (72, 145)), 1e-12
FEATURE_COUNT = 72
SPECS = (
    v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.v38.v36.v26.Spec("P3_1_RBVJ72_RIDGE512_ADD10", 512.0),
    v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.v38.v36.v26.Spec("P3_2_RBVJ72_RIDGE2048_ADD10", 2048.0),
)
BLEND = 0.10
sha256, canonical, write_new = v52.sha256, v52.canonical, v52.write_new


class ContractError(RuntimeError):
    """Raised when the sealed v53 contract differs."""
...
def load_config() -> dict[str, Any]:
    ...
    checks = {
        "schema": config["schema_version"] == "p3.realized_bipower_jump_residual.config.v53",
        "experiment": config["experiment_id"] == EXPERIMENT_ID,
        "novel": config["duplication_audit"]["semantic_verdict"] == "NON_DUPLICATE_P3_REALIZED_BIPOWER_JUMP_AXIS",
    ...
    if not all(checks.values()):
        raise ContractError(f"v53 config contract failed: {checks}")
...
def robust_case_normalize(values: np.ndarray) -> np.ndarray:
    ...
    if raw.ndim != 1 or not np.isfinite(raw).all():
        raise ContractError("realized-variation input contract differs")
    median = float(np.median(raw))
    q25, q75 = np.quantile(raw, [0.25, 0.75])
    return (raw - median) / max(float(q75 - q25), EPSILON)


def realized_bipower_statistics(values: np.ndarray, lag: int) -> np.ndarray:
    path = robust_case_normalize(values)
    increments = path[lag:] - path[:-lag]
    if len(increments) < 3:
        raise ContractError("insufficient realized-variation increments")
    realized = float(np.mean(np.square(increments)))
    bipower = float((np.pi / 2.0) * np.mean(np.abs(increments[:-1]) * np.abs(increments[1:])))
    jump_share = float(max(realized - bipower, 0.0) / max(realized, EPSILON))
    result = np.asarray([realized, bipower, jump_share], dtype=np.float64)
    if not np.isfinite(result).all() or realized < 0.0 or bipower < 0.0 or not 0.0 <= jump_share <= 1.0:
        raise ContractError("realized-bipower statistic contract differs")
    return result


def realized_bipower_features(sequence: np.ndarray) -> np.ndarray:
    path = v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.v38.v36.v26.transformed_path(sequence)[::2]
    ...
    for channel in CHANNELS:
        for start, stop in WINDOWS:
            for lag in LAGS:
                output.extend(realized_bipower_statistics(path[start:stop, channel], lag))
    features = np.asarray(output, dtype=np.float64)
    if features.shape != (FEATURE_COUNT,) or not np.isfinite(features).all():
        raise ContractError("realized-bipower feature contract differs")
    return features


def synthetic_receipt() -> dict[str, Any]:
    axis = np.arange(145, dtype=np.float64)
    smooth = np.sin(2.0 * np.pi * axis / 55.0)
    spike = smooth.copy()
    spike[72] += 8.0
    constant = np.ones(145, dtype=np.float64)
    smooth_stat = realized_bipower_statistics(smooth, 1)
    spike_stat = realized_bipower_statistics(spike, 1)
    constant_stat = realized_bipower_statistics(constant, 1)
    if spike_stat[2] <= smooth_stat[2] + 0.15 or np.max(np.abs(constant_stat)) > 1e-12:
        raise ContractError("synthetic smooth/spike/constant bipower guard failed")
    base = np.linspace(-1.0, 1.0, 289)
    sequence = np.column_stack([np.sin((index + 1) * base) + 0.1 * index * base for index in range(10)])
    sequence[1::7, (0, 3, 6)] = np.nan
    feature = realized_bipower_features(sequence)
    return {"feature_count": len(feature), "feature_sha256": hashlib.sha256(feature.astype("<f8").tobytes()).hexdigest(), "finite": bool(np.isfinite(feature).all()), "smooth_jump_share": float(smooth_stat[2]), "single_spike_jump_share": float(spike_stat[2]), "constant_max_abs": float(np.max(np.abs(constant_stat)))}


def surface_features(cases: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    sequences = np.load(v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.SEQUENCES, mmap_mode="r")
    station_codes = np.load(v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.STATIONS, mmap_mode="r")
    ...
    for position, row in enumerate(cases.itertuples(index=False)):
        ...
        features[position] = realized_bipower_features(sequences[anchor_id])
...
def support_receipt(config: dict[str, Any]) -> dict[str, Any]:
    cases, _, _, _ = v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39.v38.v36.v32.v23.case_surface()
    ...
    if not passed:
        raise ContractError("historical target-free realized-bipower support gate failed")
...
def preflight_payload() -> dict[str, Any]:
    ...
    if ARTIFACT.exists() or LOCK.exists():
        raise ContractError("v53 exactly-once namespace is consumed")
    payload = {"schema_version": "p3.realized_bipower_jump_residual.preflight.v53", "experiment_id": EXPERIMENT_ID, "status": "READY_EXACTLY_ONCE", "config_sha256": sha256(CONFIG), "runner_sha256": sha256(Path(__file__)), "candidate_count": 2, "maximum_model_fits": 12, "synthetic": synthetic_receipt(), "historical_support": support_receipt(config), "official_access": 0, "csv_materializations": 0, "uploads": 0, "config_status": config["status"]}
...
def execute(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    base = v52.v50.v49.v48.v47.v46.v44.v43.v42.v41.v40.v39
    ...
    result.update({"schema_version": "p3.realized_bipower_jump_residual.result.v53", "experiment_id": EXPERIMENT_ID, "decision": "PASS_CANDIDATE_AVAILABLE" if any(item["decision"] != "NO_GO" for item in result["candidates"]) else "NO_GO_ALL_REALIZED_BIPOWER_JUMP_CANDIDATES", "duplication_audit": config["duplication_audit"], "primary_sources": config["primary_sources"]})
    return result, arrays


def render_report(result: dict[str, Any]) -> str:
    lines = ["# P3 realized bipower/jump residual cycle v53", "", "## 결론", "", f"- overall decision: **{result['decision']}**.", "- Fixed quadratic-versus-bipower variation is distinct from crossing counts, increment ECFs and Teager energy; no prior output is reused.", "- Barndorff-Nielsen and Shephard (2004) motivates the mechanism only; the repeatedly exposed 182-case surface is EXPLORATORY_ONLY."]
...
def main() -> int:
    ...
    if ARTIFACT.exists() or REPORT.exists() or LOCK.exists():
        raise ContractError("v53 exactly-once namespace already exists")
    ...
    write_new(REPORT / "claim-source-ledger.md", b"# Claim-source ledger\n\n| Claim | Source | Use |\n|---|---|---|\n| Bipower variation can separate continuous variation from jump contribution | Barndorff-Nielsen and Shephard 2004, DOI:10.1093/jjfinec/nbh001 | mechanism only |\n| v40/v43/v50 do not compute realized-versus-bipower jump share | sealed duplication audit | novelty boundary |\n| Channels, lags, windows, normalization, epsilon, statistics and residual model were fixed before scoring | sealed v53 config | execution contract |\n")
