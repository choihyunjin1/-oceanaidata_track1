"""Independent QA for sealed P2 v32 score-aligned weighted-MSE run."""
...
import run_p2_score_aligned_weighted_mse_deepset_20260901_v32 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            ...
            fit_receipts_ok &= fit["loss"] == "weighted_MSE_uncapped_residual"
            fit_receipts_ok &= fit["extra_coefficient"] is None
            fit_receipts_ok &= fit["residual_clipping"] == 0
            fit_receipts_ok &= fit["winsorization"] == 0
            fit_receipts_ok &= fit["extra_row_downweighting"] == 0
    ...
    mse_contract = runner._weighted_mse_contract_receipt()
    ...
    checks = {
        ...
        "new_weighted_mse_axis": semantic["classification"]
        == "NEW_P2_FIXED_SCORE_ALIGNED_WEIGHTED_MSE_DEEPSET",
        "repo_exact_execution_hits_zero": semantic["repository_p2_exact_execution_hits"] == 0,
        "semantic_distinctions_all_true": all(
            semantic[name]
            for name in (
                "v13_smoothl1_distinguished",
                "v21_student_t_distinguished",
                "v23_input_gradient_distinguished",
                "v25_heteroscedastic_distinguished",
                "v31_domain_adversary_distinguished",
            )
        ),
        "weighted_mse_formula": (
            mse_contract["formula_exact"]
            and mse_contract["gradient_exact"]
            and not mse_contract["residual_clipping"]
            and not mse_contract["winsorization"]
            and not mse_contract["row_deletion"]
        ),
    ...
    qa = {
        "schema_version": "p2.score_aligned_weighted_mse_deepset.independent_qa.20260901.v32",
