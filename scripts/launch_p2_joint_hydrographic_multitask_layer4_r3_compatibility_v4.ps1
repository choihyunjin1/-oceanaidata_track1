    ...
    public void PostChildRehash(long expectedBytes, string expectedSha256)
    {
            ...
            || observedWrite != lastWrite)
        {
            throw new InvalidOperationException(
                "P2 v4 held-file identity changed after child exit: " + RequestedPath);
        }
        ...
        {
            throw new InvalidOperationException(
                "P2 v4 held-file final path changed after child exit: " + RequestedPath);
        }
        Stream.Position = 0;
        string digest;
        ...
        {
            throw new InvalidOperationException(
                "P2 v4 held-file bytes changed after child exit: " + RequestedPath);
        }
    }
...
public static class P2StableNative
{
    ...
    public static void AssertNoReparseChain(string path, bool leafMayBeMissing)
    {
            ...
            StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            if (!exists)
            {
                if (!leafMayBeMissing)
                {
                    throw new FileNotFoundException(
                        "P2 v4 stable path component is missing", current);
                }
                missing = true;
                continue;
            }
            ...
            {
                throw new InvalidOperationException(
                    "P2 v4 path exists below a missing ancestor: " + current);
            }
            FileAttributes attributes = File.GetAttributes(current);
            if ((attributes & FileAttributes.ReparsePoint) != 0)
            {
                throw new InvalidOperationException(
                    "P2 v4 reparse ancestor is forbidden: " + current);
            }
        }
    }
    ...
    public static P2HeldFile OpenReadStable(string path)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    "P2 v4 opened target is a reparse point: " + full);
            }
            if (info.NumberOfLinks != 1)
            {
                throw new InvalidOperationException(
                    "P2 v4 regular file must have exactly one hard link: " + full);
            }
            ...
            {
                throw new InvalidOperationException(
                    "P2 v4 stable handle final path differs: " + full);
            }
            ...
            try
            {
                ...
                {
                    throw new InvalidOperationException(
                        "P2 v4 stable read was truncated: " + full);
                }
                stream.Position = 0;
...
function Assert-Pin {
    ...
    if ($Held.Size -ne $Bytes -or $Held.Sha256 -cne $Sha256) {
        throw "P2 v4 external startup pin changed: $Label"
    }
}
...
if (
    ...
    throw "P2 v4 launcher host differs from P2_POWERSHELL_HOST"
}

$workspace = [IO.Path]::GetFullPath($Root)
if (-not [IO.Directory]::Exists($workspace)) {
    throw "P2 v4 workspace directory is missing"
}
...
$canonicalPycache = [IO.Path]::Combine(
    $workspace,
    "artifacts",
    "p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v4_absent_pycache"
)
[P2StableNative]::AssertNoReparseChain($canonicalPycache, $true)
if ([IO.File]::Exists($canonicalPycache) -or [IO.Directory]::Exists($canonicalPycache)) {
    throw "P2 v4 canonical pycache prefix must be absent before child start"
}
...
try {
    ...
    $launcherHeld = [P2StableNative]::OpenReadStable($launcherPath)
    $heldAll.Add($launcherHeld)
    $ExternalLauncherAttestation = $launcherHeld.Sha256

    $bootstrapPath = [IO.Path]::Combine(
        $workspace,
        "scripts",
        "bootstrap_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v4.py"
    )
    $bootstrapHeld = [P2StableNative]::OpenReadStable($bootstrapPath)
    $heldAll.Add($bootstrapHeld)
    Assert-Pin -Held $bootstrapHeld -Bytes 57216L -Sha256 "7344b6174ad353f25055a7bd0a418431a2c31fc5f76fc002b7d2de798876c3b5" -Label "v4 Python bootstrap"

    foreach ($role in @("VENV_PYTHON", "PYVENV_CFG")) {
    ...
    if ($homes.Count -ne 1 -or -not [IO.Path]::IsPathFullyQualified($homes[0])) {
        throw "P2 v4 pyvenv.cfg base-prefix resolution changed"
    }
    ...
    foreach ($role in $startupPins.Keys) {
        ...
        if ($pin.scope -cne "base") {
            throw "P2 v4 startup scope changed: $role"
        }
        $path = [IO.Path]::GetFullPath(
    ...
    if (-not $child.Start()) {
        throw "P2 v4 Python child did not start"
    }
    ...
    if ($child.ExitCode -ne 0) {
        throw "P2 v4 Python child failed closed (exit $($child.ExitCode)): $stderr"
    }
    if (-not [string]::IsNullOrWhiteSpace($stderr)) {
        throw "P2 v4 Python child wrote unexpected stderr: $stderr"
    }
    ...
    $bootstrapHeld.PostChildRehash(57216L, "7344b6174ad353f25055a7bd0a418431a2c31fc5f76fc002b7d2de798876c3b5")
    $hostHeld.PostChildRehash(301368L, "db6dd81183fe57d22e03b911ec9a30a2fd7c40542e97743615355a6fb44f458f")
    $launcherHeld.PostChildRehash($launcherHeld.Size, $ExternalLauncherAttestation)

    if ([IO.File]::Exists($canonicalPycache) -or [IO.Directory]::Exists($canonicalPycache)) {
        throw "P2 v4 canonical pycache prefix appeared during child execution"
    }

    $envelope = [ordered]@{
        external_startup_trust = [ordered]@{
            model = "EXTERNAL_QA_PREEXECUTION_PINNED_HOST_AND_LAUNCHER"
            ...
            bootstrap_pin = [ordered]@{
                path = "scripts/bootstrap_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v4.py"
            ...
            canonical_pycache_prefix_absent_before = $true
            canonical_pycache_prefix_absent_after = $true
        }
