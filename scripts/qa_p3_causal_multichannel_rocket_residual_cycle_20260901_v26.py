"""Independent post-run QA for P3 v26 causal multichannel ROCKET residuals."""
...
def main() -> int:
    ...
    checks["terminal_no_go"] = result["decision"] == "NO_GO_ALL_ROCKET_CANDIDATES"
    ...
    checks["six_outer_blocks_per_candidate"] = all(
        sum(item["candidate"] == candidate["name"] for item in result["fit_receipts"]) == 6
        for candidate in result["candidates"]
    )
    checks["sealed_kernel_bank"] = result["kernel_receipt"]["count"] == 256 and result["feature_receipt"]["columns"] == 512
    ...
    payload = {
        "schema_version": "p3.causal_multichannel_rocket_residual.independent_qa.v26",
