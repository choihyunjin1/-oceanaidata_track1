"""Independent QA for sealed P2 v40 predictive-dropout consistency run."""
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            finite_values = [
                fit["base_loss_first"],
                fit["base_loss_last"],
                fit["consistency_penalty_first"],
                fit["consistency_penalty_last"],
                fit["total_loss_first"],
                fit["total_loss_last"],
                fit["mean_abs_disagreement_first"],
                fit["mean_abs_disagreement_last"],
            ]
            ...
            fit_receipts_ok &= fit["two_pass_steps"] == fit["optimizer_steps"]
            fit_receipts_ok &= fit["training_passes_per_batch"] == 2
            fit_receipts_ok &= fit["dropout_probability"] == 0.1
            fit_receipts_ok &= fit["consistency_coefficient"] == 1.0
            fit_receipts_ok &= fit["inference_dropout"] is False
            fit_receipts_ok &= fit["mean_abs_disagreement_first"] > 0.0
            fit_receipts_ok &= fit["mean_abs_disagreement_last"] > 0.0
            fit_receipts_ok &= fit["input_gradient_steps"] == 0
            fit_receipts_ok &= fit["parameter_perturbation_steps"] == 0
            fit_receipts_ok &= fit["row_mixing_steps"] == 0
            fit_receipts_ok &= fit["gradient_surgery_steps"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True

    semantic = runner.semantic_audit(config)
    penalty = runner._penalty_contract_receipt()
    stochastic = runner._stochastic_contract_receipt()
    ...
    checks = {
        ...
        "new_p2_predictive_consistency_axis": semantic["classification"]
        == "NEW_P2_FIXED_DROPOUT_PREDICTIVE_CONSISTENCY",
        "p2_execution_hits_zero": semantic["repository_p2_exact_execution_hits"]
        == 0,
        "cross_problem_adjacency_disclosed": semantic[
            "p1_v39_cross_problem_adjacency_disclosed"
        ],
        "cross_problem_transfer_zero": semantic[
            "p1_code_result_gate_transfer_count"
        ]
        == 0,
        "semantic_distinctions": all(
            ...
            for name in (
                ...
                "v38_output_shrinkage_distinguished",
                "v39_gradient_mask_distinguished",
            )
        ),
        "official_feedback_selection_zero": not semantic[
            "official_v23_feedback_used_for_selection"
        ],
        "penalty_contract": all(
            penalty[name]
            for name in (
                "formula_exact",
                "symmetric",
                "equal_predictions_zero",
                "finite_gradient",
            )
        ),
        "stochastic_contract": stochastic[
            "pair_reproducible_under_seed_reset"
        ]
        and stochastic["stochastic_pair_maximum_abs_difference"] > 0.0
        and stochastic["dropout_off_inference_deterministic"]
        and stochastic["dropout_module_count"] == 4,
    ...
    qa = {
        "schema_version": "p2.predictive_dropout_consistency_deepset.independent_qa.20260901.v40",
