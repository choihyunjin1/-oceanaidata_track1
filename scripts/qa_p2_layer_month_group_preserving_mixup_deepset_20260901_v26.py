"""Independent QA for sealed P2 v26 layer-month MixUp run."""
...
import run_p2_layer_month_group_preserving_mixup_deepset_20260901_v26 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        group_mass_ok &= receipt["weight_receipt"]["mixup_environment_count"] > 0
        for fit in receipt["fit_receipts"]:
            values = [
                ...
                fit["mixup_rows_seen"],
                fit["mixed_rows"],
                fit["mask_intersection_fallback_rows"],
                fit["singleton_group_noop_rows"],
                fit["minimum_output_mask_tokens"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["mixup_alpha"] == 0.2
            fit_receipts_ok &= fit["cross_group_pairs"] == 0
            fit_receipts_ok &= fit["minimum_output_mask_tokens"] >= 2.0
            fit_receipts_ok &= fit["inference_augmentation"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            mixup_active &= fit["mixed_rows"] > 0
    ...
    mixup_contract = runner._mixup_contract_receipt()
    checks = {
        "terminal": result["status"].startswith("EXPLORATORY_"),
        "exactly_nine_fits": result["fit_count"] == 9,
        "new_group_preserving_mixup_axis": semantic["classification"]
        == "NEW_P2_LAYER_MONTH_GROUP_PRESERVING_VICINAL_MIXUP_AUGMENTATION",
        ...
        "semantic_distinctions_all_true": all(
            semantic[name]
            for name in (
                "v17_masked_auxiliary_distinguished",
                "v23_input_gradient_distinguished",
                "v24_sam_distinguished",
                "v25_heteroscedastic_distinguished",
            )
        ),
        "fixed_alpha_and_group": (
            result["training"]["mixup"]["alpha"] == 0.2
            and result["training"]["mixup"]["partner_group"]
            == "exact_target_layer_x_calendar_month"
            and result["training"]["mixup"]["alpha_sweep"] is False
        ),
        "cross_group_mixing_zero": result["training"]["mixup"][
            "cross_group_mixing"
        ]
        is False,
        "deterministic_mixup_replay": mixup_contract["byte_identical_seed_replay"],
        "intersection_fallback_exact_noop": mixup_contract[
            "insufficient_intersection_exact_noop"
        ],
        "fit_receipts_valid": fit_receipts_ok,
        "mixup_actually_used": mixup_active,
    ...
    qa = {
        "schema_version": "p2.layer_month_group_preserving_mixup_deepset.independent_qa.20260901.v26",
