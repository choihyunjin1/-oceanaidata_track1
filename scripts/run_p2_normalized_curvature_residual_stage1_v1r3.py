"""Crash-safe, literal-path-sealed P2 NCR_LGBM Stage 1 v1r3 runner.
...
EXPECTED_CONFIG_SHA256 = "7cd9b8025ba01855dec7120ee4717ff01cc943b1832f762120c84ecfa93d0c04"
...
WORKER_TOKEN_ENV = "P2_NCR_V1R3_ATTEMPT_TOKEN"
WORKER_DEADLINE_ENV = "P2_NCR_V1R3_DEADLINE_EPOCH"
WORKER_PARENT_PID_ENV = "P2_NCR_V1R3_PARENT_PID"
    ...
    for name, pin in pins.items():
        ...
        if name == runner_name:
            if path != runner_path or mode != RUNNER_HASH_MODE:
                raise RuntimeError("v1r3 runner pin contract drift")
...
def _verify_static_bundle(config_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if config_sha256 != EXPECTED_CONFIG_SHA256:
        raise RuntimeError("NCR_LGBM v1r3 preregistration hash drift")
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if not isinstance(config, dict):
        raise TypeError("v1r3 preregistration must be a JSON object")
    if config.get("schema_version") != (
        "p2_normalized_curvature_residual_lgbm_stage1.prereg.v1r3"
    ):
        raise ValueError("unexpected v1r3 preregistration schema")
    if config.get("experiment_id") != (
        "p2_normalized_curvature_residual_lgbm_stage1_20260826_v1r3"
    ):
        raise ValueError("unexpected v1r3 experiment id")
    if config.get("status") != "PREREGISTERED_NOT_EXECUTED_SUPERSEDES_V1_AND_V1R2":
        raise ValueError("v1r3 is not in its sealed preregistered state")
    ...
    if "run_p2_normalized_curvature_residual_stage1_v1r3.py" not in command:
        raise ValueError("execution command is not the v1r3 sealed runner")
    if "run_p2_normalized_curvature_residual_stage1.py" in command or (
        ...
        raise ValueError("superseded v1/v1r2 runner execution is forbidden")
...
def _assert_static_bundle_unchanged(
    ...
    if observed_config != expected_config or observed_bundle != expected_bundle:
        raise RuntimeError("v1r3 static bundle changed after preflight")
...
def _literal_source_readiness(
    ...
    *,
    ...
    if require_environment and not environment["present"]:
        raise RuntimeError("P2_DATA_DIR is required before any v1r3 claim")
    ...
    with _held_verified_bytes(
        ...
        receipt: dict[str, Any] = {
            "schema_version": "p2_ncr_literal_data_readiness.v1r3",
...
def _acquire_attempt(
    ...
    bundle_sha256 = str(bundle["bundle_sha256"])
    claim = {
        "schema_version": "p2_ncr_stage1_execution_claim.v1r3",
        "experiment_id": config["experiment_id"],
        "contract_sha256": contract_sha256,
        "bundle_sha256": bundle_sha256,
...
def _run_internal_worker(
    ...
    with _held_verified_bytes(
        ...
        worker_result: dict[str, Any] = {
            "schema_version": "p2_ncr_stage1_worker.result.v1r3",
            "experiment_id": config["experiment_id"],
            "contract_sha256": authorization["contract_sha256"],
            "bundle_sha256": authorization["bundle_sha256"],
...
def _execute_parent(
    ...
    result: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.result.v1r3",
        "experiment_id": config["experiment_id"],
        "status": (
    ...
    manifest: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.manifest.v1r3",
        "experiment_id": config["experiment_id"],
            ...
            json.dumps(
                {
                    ...
                    "superseded_v1_and_v1r2_execution_forbidden": True,
                    "maximum_physical_fit_calls_lifetime": 3,
                    "worker_restart_count": 0,
                    "hard_wall_seconds": 1800,
