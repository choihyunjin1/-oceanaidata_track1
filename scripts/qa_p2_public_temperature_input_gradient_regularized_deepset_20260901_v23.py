"""Independent QA for the sealed P2 v23 input-gradient run."""
...
import run_p2_public_temperature_input_gradient_regularized_deepset_20260901_v23 as runner  # noqa: E402
...
def main() -> None:
    ...
    transport_penalty = float(config["evaluation"]["transport_penalty_points"])
    delta = float(recomputed["delta_rmse"])
    nominal = -delta * slope
    transport = nominal - transport_penalty
    record = result["candidate"]

    prefix_ok = True
    group_mass_ok = True
    fit_receipts_ok = True
    penalty_positive = True
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            values = [
                fit["data_loss_first"],
                fit["data_loss_last"],
                fit["input_gradient_penalty_first"],
                fit["input_gradient_penalty_last"],
                fit["objective_first"],
                fit["objective_last"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["input_gradient_coefficient"] == 0.01
            fit_receipts_ok &= fit["penalized_token_channel"] == 0
            fit_receipts_ok &= fit["non_temperature_channels_penalized"] == 0
            fit_receipts_ok &= fit["observed_token_mask_only"] is True
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            penalty_positive &= fit["input_gradient_penalty_last"] > 0.0
    ...
    checks = {
        ...
        "new_input_gradient_axis": semantic["classification"]
        == "NEW_P2_PUBLIC_TEMPERATURE_INPUT_GRADIENT_REGULARIZATION_OBJECTIVE",
        ...
        "semantic_distinctions_all_true": all(
            semantic[name]
            for name in (
                "v22_output_depth_derivative_distinguished",
                "v10_v11_post_model_action_rules_distinguished",
                "v16_graph_representation_distinguished",
                "dynamic_sigmoid_solver_conditioning_distinguished",
                "v21_student_t_likelihood_distinguished",
            )
        ),
        "fixed_lambda": result["training"]["input_gradient"]["coefficient"]
        == 0.01,
        "temperature_channel_only": (
            result["training"]["input_gradient"]["token_channel"] == 0
            and result["training"]["input_gradient"][
                "penalize_psal_depth_nominal_presence_context"
            ]
            is False
        ),
        "masked_token_isolation": runner._masked_token_isolation_receipt()[
            "maximum_abs_error"
        ]
        <= 1e-6,
        "gradient_scope_receipt": (
            runner._gradient_scope_receipt()[
                "non_temperature_coefficient_invariance_error"
            ]
            <= 1e-8
        ),
        "fit_receipts_valid": fit_receipts_ok,
        "penalty_positive_all_fits": penalty_positive,
    ...
    qa = {
        "schema_version": "p2.public_temperature_input_gradient_regularized_deepset.independent_qa.20260901.v23",
