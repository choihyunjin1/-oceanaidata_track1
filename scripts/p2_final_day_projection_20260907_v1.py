...
Copied from src/p2_restore/profile_projection.py; final-day amendments:
clip BEFORE exact PAVA; deep endpoint falls back through finite layers 5/6/7/8.
The operation uses only public endpoint temperatures at the same
...
def public_endpoint_frame(observations: pd.DataFrame) -> pd.DataFrame:
    ...
    if "station" in observations and observations.station.nunique() != 1:
        raise ValueError("P2 requires exactly one station")
    selected = observations.loc[observations["layer"].isin((1, 5, 6, 7, 8)), ["time", "layer", "temp"]]
    ...
    wide = keyed.pivot(index="time", columns="layer", values="temp").reindex(columns=[1, 5, 6, 7, 8])
    wide = wide.where(np.isfinite(wide))
    deep = wide[[5, 6, 7, 8]].bfill(axis=1)[5]
    return pd.DataFrame({"temp_1": wide[1], "temp_5": deep}).reset_index()
...
def project_profiles(
    ...
    for _, group in merged.groupby(group_columns, sort=False, dropna=False):
        ...
        clipped = np.clip(values[rows], min(temp_1, temp_5), max(temp_1, temp_5))
        projected = _isotonic_three(clipped, increasing=temp_1 <= temp_5)
...
def project_profiles_vectorized(
    ...
    profiles = np.clip(profiles, np.minimum(temp_1, temp_5)[:, None], np.maximum(temp_1, temp_5)[:, None])
