"""Canonical read/replay/append CLI for the v4 meaningful-score ledger.

All commands are read-only unless ``--append`` is explicitly supplied.  This
script never trains, predicts, or uploads.
"""
...
from ocean_goal.meaningful_score_ledger_v4 import (
    CONTRACT_RELATIVE,
    CONTRACT_SHA256,
    LEDGER_RELATIVE,
...
def check(root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "meaningful_score_ledger_v4.check.v1",
        ...
        "pre_init_qa_exists": receipt.exists(),
        "pre_init_qa_schema": "meaningful_score_ledger_v4.pre_init_qa.v1",
        "pre_init_qa_required_decision": "GO_INITIALIZE_V4_LEDGER",
        "writes": 0,
        "fits": 0,
        "predictions": 0,
        "uploads": 0,
...
def initialize(root: Path, *, append: bool, qa_receipt: Path | None) -> dict[str, Any]:
    ...
    if report["ledger_exists"]:
        if append:
            raise ContractError("v4 ledger is already initialized; second init forbidden")
    ...
    if qa_receipt is None:
        raise ContractError("canonical independent v4 pre-init QA receipt is required")
...
def evaluate_score(
    ...
    *,
    ...
    payload = build_official_score_payload(
        ...
        _source_path(root, curve_decision_path),
    )
    record = (
        append_ledger_event(
            root,
