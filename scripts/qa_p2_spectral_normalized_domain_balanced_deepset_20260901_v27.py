"""Independent QA for sealed P2 v27 spectral-normalization run."""
...
import run_p2_spectral_normalized_domain_balanced_deepset_20260901_v27 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["effective_spectral_norm_min"],
                fit["effective_spectral_norm_max"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["spectral_normalized_linear_count"] == 5
            fit_receipts_ok &= fit["n_power_iterations_per_training_forward"] == 1
            ...
            spectral_fit_ok &= 0.98 <= fit["effective_spectral_norm_min"] <= 1.02
            spectral_fit_ok &= 0.98 <= fit["effective_spectral_norm_max"] <= 1.02
    ...
    contract = runner._spectral_contract_receipt()
    ...
    checks = {
        ...
        "new_spectral_normalization_axis": semantic["classification"]
        == "NEW_P2_FIXED_LINEAR_OPERATOR_SPECTRAL_NORMALIZATION",
        ...
        "all_five_linear_maps_parametrized": contract[
            "spectral_normalized_linear_count"
        ]
        == 5,
        "spectral_operator_norm_contract": contract[
            "maximum_operator_norm_abs_error_from_one"
        ]
        <= 1e-4,
        "masked_token_isolation": contract[
            "masked_token_isolation_maximum_abs_error"
        ]
        <= 1e-6,
        "permutation_invariance": contract[
            "permutation_invariance_maximum_abs_error"
        ]
        <= 1e-6,
        "fit_receipts_valid": fit_receipts_ok,
        "fit_spectral_norms_bounded": spectral_fit_ok,
        ...
        "effective_safety_pass": record["safety_pass"]
        == bool(
    ...
    qa = {
        "schema_version": "p2.spectral_normalized_domain_balanced_deepset.independent_qa.20260901.v27",
