"""Crash-safe, literal-path-sealed P2 NCR_LGBM Stage 1 v1r7 runner.
...
DEFAULT_CONFIG = (
    PROJECT_ROOT
    / "configs/experiments/p2_normalized_curvature_residual_lgbm_stage1_v1r7.json"
)
EXPECTED_CONFIG_SHA256 = "5be45ad6198f55e39d3f2a32a70e4784345a75a4fcdcd9f88c9124591621209c"
...
WORKER_TOKEN_ENV = "P2_NCR_V1R7_ATTEMPT_TOKEN"
WORKER_DEADLINE_ENV = "P2_NCR_V1R7_DEADLINE_EPOCH"
WORKER_PARENT_PID_ENV = "P2_NCR_V1R7_PARENT_PID"
PRELAUNCH_JOURNAL_SCHEMA = "p2_ncr_prelaunch_journal.v1r7"
OUT_OF_BAND_FAILURE_SCHEMA = "p2_ncr_out_of_band_terminal_failure.v1r7"
TERMINAL_SUCCESS_SCHEMA = "p2_ncr_terminal_success_inventory.v1r7"
    ...
    for name, pin in pins.items():
        ...
        if name == runner_name:
            if path != runner_path or mode != RUNNER_HASH_MODE:
                raise RuntimeError("v1r7 runner pin contract drift")
...
def _verify_static_bundle(config_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if config_sha256 != EXPECTED_CONFIG_SHA256:
        raise RuntimeError("NCR_LGBM v1r7 preregistration hash drift")
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if not isinstance(config, dict):
        raise TypeError("v1r7 preregistration must be a JSON object")
    if config.get("schema_version") != (
        "p2_normalized_curvature_residual_lgbm_stage1.prereg.v1r7"
    ):
        raise ValueError("unexpected v1r7 preregistration schema")
    if config.get("experiment_id") != (
        "p2_normalized_curvature_residual_lgbm_stage1_20260826_v1r7"
    ):
        raise ValueError("unexpected v1r7 experiment id")
    if config.get("status") != (
        "PREREGISTERED_NOT_EXECUTED_SUPERSEDES_V1_V1R2_V1R3_V1R4_V1R5_V1R6"
    ):
        raise ValueError("v1r7 is not in its sealed preregistered state")
    ...
    if execution.get("execution_enabled_after_final_qa") is not True:
        raise ValueError("v1r7 final-QA execution seal is not enabled")
    command = str(execution.get("command", ""))
    if "run_p2_normalized_curvature_residual_stage1_v1r7.py" not in command:
        raise ValueError("execution command is not the v1r7 sealed runner")
    if "run_p2_normalized_curvature_residual_stage1.py" in command or (
        ...
        "run_p2_normalized_curvature_residual_stage1_v1r6.py" in command
    ):
        raise ValueError(
            "superseded v1/v1r2/v1r3/v1r4/v1r5/v1r6 execution is forbidden"
        )
    ...
    if journal_contract != {
        ...
        raise ValueError("v1r7 worker prelaunch journal contract drift")
...
def _assert_static_bundle_unchanged(
    ...
    if observed_config != expected_config or observed_bundle != expected_bundle:
        raise RuntimeError("v1r7 static bundle changed after preflight")
...
def _literal_source_readiness(
    ...
    *,
    ...
    if require_environment and not environment["present"]:
        raise RuntimeError("P2_DATA_DIR is required before any v1r7 claim")
    ...
    with _held_verified_bytes(
        ...
        receipt: dict[str, Any] = {
            "schema_version": "p2_ncr_literal_data_readiness.v1r7",
...
def _inspect_control_state(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if marker.is_file():
        ...
        state["published_success_inventory_valid"] = False
    if state["journal_init_failure_exists"] or state["postpublish_failure_exists"]:
        try:
            persisted_attempt = _attempt_identity_from_persisted_claim(
                paths,
                expected_experiment_id=str(config["experiment_id"]),
            )
            oob = _existing_out_of_band_terminal_state(persisted_attempt)
        except BaseException as error:
            oob = {
                "present": True,
                "valid": False,
                "reason": "OUT_OF_BAND_TERMINAL_VALIDATION_SETUP_FAILED",
                "error_type": type(error).__name__,
            }
        state["out_of_band_terminal"] = oob
        state["out_of_band_terminal_valid"] = bool(oob.get("valid"))
        state["status"] = (
            "TERMINAL_VALID_OOB_FAILURE_NO_RERUN"
            if oob.get("valid")
            else "INVALID_OOB_TERMINAL_FAIL_CLOSED_NO_RERUN"
        )
        state["eligible"] = False
    return state
...
def _attempt_identity_from_persisted_claim(
    paths: Mapping[str, Path],
    *,
    expected_experiment_id: str | None = None,
) -> dict[str, Any]:
    claim_path = Path(paths["claim"])
    claim_bytes = claim_path.read_bytes()
    claim = json.loads(claim_bytes)
    if not isinstance(claim, dict):
        raise RuntimeError("attempt claim is not a JSON object")
    experiment_id = str(claim.get("experiment_id", ""))
    if expected_experiment_id is not None and experiment_id != expected_experiment_id:
        raise RuntimeError("persisted claim experiment mismatch")
    return {
        "experiment_id": experiment_id,
        "contract_sha256": str(claim.get("contract_sha256", "")),
        "bundle_sha256": str(claim.get("bundle_sha256", "")),
        "token_sha256": str(claim.get("attempt_token_sha256", "")),
...
def _validate_out_of_band_terminal_receipt(
    receipt_path: Path,
    attempt: Mapping[str, Any],
) -> dict[str, Any]:
    """Fully validate the one create-only terminal authority outside the journal."""

    raw = receipt_path.read_bytes()
    receipt = json.loads(raw)
    if not isinstance(receipt, dict):
        raise RuntimeError("out-of-band terminal receipt is not an object")
    required_keys = {
        "schema_version",
        "experiment_id",
        "status",
    ...
    allowed_keys = required_keys | {"process_tree_termination"}
    if set(receipt) != required_keys and set(receipt) != allowed_keys:
        raise RuntimeError("out-of-band terminal receipt schema is not exact")
    expected_identity = {
    ...
    for name, expected in expected_identity.items():
        if receipt.get(name) != expected:
            raise RuntimeError(f"out-of-band terminal identity mismatch: {name}")
    if receipt.get("automatic_rerun_allowed") is not False:
        raise RuntimeError("out-of-band terminal rerun policy is not exact false")
    for name in ("reason", "phase", "error_type", "at_kst"):
        if not isinstance(receipt.get(name), str) or not receipt[name]:
            raise RuntimeError(f"out-of-band terminal text field invalid: {name}")
    claim_path = Path(attempt["paths"]["claim"])
    actual_claim = {
        "path": claim_path.relative_to(PROJECT_ROOT).as_posix(),
        "bytes": claim_path.stat().st_size,
        "sha256": _sha256(claim_path),
    }
    declared_claim = receipt.get("claim")
    if not isinstance(declared_claim, dict) or set(declared_claim) != {
        "path",
        "bytes",
        "sha256",
    }:
        raise RuntimeError("out-of-band terminal claim schema drift")
    if type(declared_claim.get("bytes")) is not int or (
        receipt.get("claim") != actual_claim
    ):
        raise RuntimeError("out-of-band terminal claim binding mismatch")
    ...
    if not isinstance(journal, dict) or set(journal) != {
        ...
        raise RuntimeError("out-of-band terminal journal snapshot schema drift")
    if journal.get("path") != journal_path.relative_to(PROJECT_ROOT).as_posix():
        raise RuntimeError("out-of-band terminal journal path mismatch")
    if type(journal.get("exists")) is not bool or (
        journal.get("exists") is not journal_path.exists()
    ):
        raise RuntimeError("out-of-band terminal journal existence changed")
    if type(journal.get("event_count")) is not int or journal["event_count"] < 0:
        raise RuntimeError("out-of-band terminal journal event count invalid")
    final = Path(attempt["paths"]["final"])
    if type(receipt.get("final_exists")) is not bool or (
        receipt.get("final_exists") is not final.exists()
    ):
        raise RuntimeError("out-of-band terminal final existence changed")
    if type(receipt.get("staging_count")) is not int or (
        receipt.get("staging_count") != len(_staging_paths(final))
    ):
        raise RuntimeError("out-of-band terminal staging snapshot changed")
    states = receipt.get("fit_slot_states")
    if not isinstance(states, dict) or set(states) != {"1", "2", "3"}:
        raise RuntimeError("out-of-band terminal fit slot schema drift")
    allowed_states = {
        "UNRESERVED",
        "RESERVED",
        "COMPLETED",
    ...
    if any(state not in allowed_states for state in states.values()):
        raise RuntimeError("out-of-band terminal fit slot state invalid")
    reserved = sum(state != "UNRESERVED" for state in states.values())
    completed = sum(state == "COMPLETED" for state in states.values())
    if type(receipt.get("physical_reserved_count")) is not int or type(
        receipt.get("physical_completed_count")
    ) is not int:
        raise RuntimeError("out-of-band terminal fit count types mismatch")
    if receipt.get("physical_reserved_count") != reserved or (
        receipt.get("physical_completed_count") != completed
    ):
        raise RuntimeError("out-of-band terminal fit counts mismatch")
    if "process_tree_termination" in receipt and not isinstance(
        receipt["process_tree_termination"], dict
    ):
        raise RuntimeError("out-of-band terminal process-tree evidence invalid")
    return {
...
def _existing_out_of_band_terminal_state(
    attempt: Mapping[str, Any],
) -> dict[str, Any]:
    paths = attempt["paths"]
    candidates = [
        Path(paths["journal_init_failure"]),
        Path(paths["postpublish_failure"]),
    ]
    ...
    if len(existing) != 1:
        return {
            "present": True,
            "valid": False,
            "reason": "MULTIPLE_OUT_OF_BAND_TERMINAL_RECEIPTS",
            "paths": [path.relative_to(PROJECT_ROOT).as_posix() for path in existing],
        }
    try:
        validated = _validate_out_of_band_terminal_receipt(existing[0], attempt)
    except BaseException as error:
        return {
            "present": True,
            "valid": False,
            "reason": "INVALID_OR_CORRUPT_OUT_OF_BAND_TERMINAL_RECEIPT",
            "path": existing[0].relative_to(PROJECT_ROOT).as_posix(),
            "error_type": type(error).__name__,
        }
    return {"present": True, "valid": True, "receipt": validated}
...
def _record_out_of_band_failure_create_only(
    ...
    *,
    ...
    try:
        ...
        _validate_out_of_band_terminal_receipt(receipt_path, attempt)
        return False
...
def _acquire_attempt(
    ...
    *,
    ...
    bundle_sha256 = str(bundle["bundle_sha256"])
    claim = {
        "schema_version": "p2_ncr_stage1_execution_claim.v1r7",
        "experiment_id": config["experiment_id"],
        "contract_sha256": contract_sha256,
        "bundle_sha256": bundle_sha256,
...
def _validate_worker_claim(
    ...
    if claim.get("schema_version") != "p2_ncr_stage1_execution_claim.v1r7":
    ...
    if oob["present"]:
        raise RuntimeError(
            "out-of-band terminal state has precedence; worker authorization forbidden"
        )
...
def _validate_single_same_attempt_terminal_event(
    events: Sequence[Mapping[str, Any]], attempt: Mapping[str, Any]
) -> Mapping[str, Any]:
    terminal = [
        event
        for event in events
    ...
    if len(terminal) != 1:
        raise RuntimeError("journal does not contain exactly one terminal event")
    event = terminal[0]
    identity = {
        "contract_sha256": str(attempt["contract_sha256"]),
        "bundle_sha256": str(attempt["bundle_sha256"]),
    ...
    for name, expected in identity.items():
        if event.get(name) != expected:
            raise RuntimeError(f"journal terminal identity mismatch: {name}")
    if not isinstance(event.get("at_kst"), str) or not event["at_kst"]:
        raise RuntimeError("journal terminal timestamp invalid")
    if event["event"] == "ATTEMPT_TERMINAL_COMPLETE":
        if not isinstance(event.get("final_status"), str) or not event["final_status"]:
            raise RuntimeError("journal terminal success status invalid")
        if not isinstance(event.get("publish_durability"), dict):
            raise RuntimeError("journal terminal success durability missing")
    else:
        required = {
            "reason": str,
            "error_type": str,
            ...
            "final_exists": bool,
            "staging_count": int,
            "fit_slot_states": dict,
            "physical_reserved_count": int,
            "physical_completed_count": int,
        }
        for name, expected_type in required.items():
            if not isinstance(event.get(name), expected_type):
                raise RuntimeError(f"journal terminal failure field invalid: {name}")
        for name in ("worker_stdout_sha256", "worker_stderr_sha256", "journal_prefix_sha256"):
            value = str(event[name])
            if len(value) != 64 or any(char not in "0123456789abcdef" for char in value):
                raise RuntimeError(f"journal terminal digest invalid: {name}")
    return event


def _validate_single_same_attempt_worker_failed(
    events: Sequence[Mapping[str, Any]], authorization: Mapping[str, Any]
) -> Mapping[str, Any]:
    failures = [event for event in events if event.get("event") == "WORKER_FAILED"]
    if len(failures) != 1:
        raise RuntimeError("journal does not contain exactly one WORKER_FAILED")
    event = failures[0]
    for name, expected in {
        ...
        if event.get(name) != expected:
            raise RuntimeError(f"WORKER_FAILED identity mismatch: {name}")
    for name in (
        "at_kst",
        "error_type",
        "phase",
        ...
        if not isinstance(event.get(name), str) or not event[name]:
            raise RuntimeError(f"WORKER_FAILED field invalid: {name}")
    _fit_slot_states(events)
    return event
...
def _record_worker_failure_best_effort(
    ...
    try:
        oob = _existing_out_of_band_terminal_state(authorization)
        if oob["present"]:
            diagnostic = {
                "status": (
                    "VALID_OOB_TERMINAL_PRECEDENCE_NO_WORKER_APPEND"
                    if oob["valid"]
                    else "INVALID_OOB_TERMINAL_FAIL_CLOSED_NO_WORKER_APPEND"
                ),
                "out_of_band_terminal": oob,
                "original_error_type": type(error).__name__,
                "original_exception_preserved": True,
            }
            with contextlib.suppress(BaseException):
        ...
        recovered_complete_worker_failure = False
        with contextlib.suppress(BaseException):
            recovered_events = _read_journal(
                Path(authorization["paths"]["journal"])
            )
            _validate_single_same_attempt_worker_failed(
                recovered_events,
                authorization,
            )
            recovered_complete_worker_failure = True
        diagnostic = {
            "status": "WORKER_FAILURE_LOGGING_FAILED_OR_DUPLICATE",
            "original_error_type": type(error).__name__,
            "logging_error_type": type(logging_error).__name__,
            "complete_worker_failure_recovered_after_append_error": (
                recovered_complete_worker_failure
            ),
            "original_exception_preserved": True,
        }
        with contextlib.suppress(BaseException):
...
def _run_internal_worker(
    ...
    with _held_verified_bytes(
        ...
        worker_result: dict[str, Any] = {
            "schema_version": "p2_ncr_stage1_worker.result.v1r7",
            "experiment_id": config["experiment_id"],
            "contract_sha256": authorization["contract_sha256"],
            "bundle_sha256": authorization["bundle_sha256"],
...
def _execute_parent_impl(
    ...
    result: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.result.v1r7",
        "experiment_id": config["experiment_id"],
        "status": (
    ...
    manifest: dict[str, Any] = {
        "schema_version": "p2_normalized_curvature_residual_lgbm_stage1.manifest.v1r7",
        "experiment_id": config["experiment_id"],
...
def _record_parent_terminal_failure_best_effort(
    ...
    terminal_append_attempted = False
    try:
        attempt = parent_context["attempt"]
        journal_path = Path(attempt["paths"]["journal"])
        final_path = Path(attempt["paths"]["final"])
        oob = _existing_out_of_band_terminal_state(attempt)
        if oob["present"]:
            diagnostic = {
                "status": (
                    "VALID_OOB_TERMINAL_PRECEDENCE_NO_PARENT_APPEND"
                    if oob["valid"]
                    else "INVALID_OOB_TERMINAL_FAIL_CLOSED_NO_PARENT_APPEND"
                ),
                "out_of_band_terminal": oob,
        ...
        terminal_append_attempted = True
        _append_journal_event(
            journal_path,
            _journal_event(
        ...
        complete_terminal_recovered = False
        oob_fallback_permitted = not terminal_append_attempted
        terminal_recovery = "NOT_APPLICABLE_BEFORE_TERMINAL_APPEND"
        attempt = parent_context.get("attempt")
        if terminal_append_attempted and isinstance(attempt, Mapping):
            try:
                recovered_events = _read_journal(Path(attempt["paths"]["journal"]))
            except BaseException as recovery_error:
                terminal_recovery = f"TORN_OR_UNREADABLE:{type(recovery_error).__name__}"
                oob_fallback_permitted = True
            else:
                recovered_terminal = [
                    event
                ...
                if not recovered_terminal:
                    terminal_recovery = "NO_TERMINAL_EVENT_PRESENT"
                    oob_fallback_permitted = True
                else:
                    try:
                        _validate_single_same_attempt_terminal_event(
                            recovered_events,
                            attempt,
                        )
                    except BaseException as recovery_error:
                        terminal_recovery = (
                            "INVALID_OR_MULTIPLE_EXISTING_TERMINAL:"
                            f"{type(recovery_error).__name__}"
                        )
                        oob_fallback_permitted = False
                    else:
                        terminal_recovery = "COMPLETE_SAME_ATTEMPT_TERMINAL_RECOVERED"
                        complete_terminal_recovered = True
                        oob_fallback_permitted = False
        if oob_fallback_permitted and isinstance(attempt, Mapping):
            with contextlib.suppress(BaseException):
        ...
        diagnostic = {
            "status": (
                "PARENT_TERMINAL_APPEND_EXCEPTION_RECOVERED_SINGLE_EVENT"
                if complete_terminal_recovered
                else "PARENT_TERMINAL_FAILURE_LOGGING_FAILED"
            ),
            "phase": str(parent_context.get("phase", "PARENT_PHASE_UNKNOWN")),
            "original_error_type": type(error).__name__,
            "logging_error_type": type(logging_error).__name__,
            "out_of_band_fallback_recorded": fallback_recorded,
            "out_of_band_fallback_permitted": oob_fallback_permitted,
            "complete_terminal_recovered_after_append_error": (
                complete_terminal_recovered
            ),
...
def main() -> int:
    ...
    if args.execute_stage1 and not bool(
        ...
        raise RuntimeError("v1r7 draft execution is disabled pending final QA seal")
            ...
            json.dumps(
                {
                    ...
                    "superseded_v1_v1r2_v1r3_v1r4_v1r5_v1r6_execution_forbidden": True,
                    "maximum_physical_fit_calls_lifetime": 3,
                    "worker_restart_count": 0,
                    "hard_wall_seconds": 1800,
