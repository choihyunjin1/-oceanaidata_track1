"""Independent QA for sealed P2 v25 heteroscedastic Gaussian run."""
...
import run_p2_heteroscedastic_gaussian_domain_balanced_deepset_20260901_v25 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["query_log_variance_min"],
                fit["query_log_variance_p50"],
                fit["query_log_variance_p99"],
                fit["query_log_variance_max"],
                fit["query_log_variance_clipped_share"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["mean_only_inference"] is True
            fit_receipts_ok &= fit["variance_router"] == 0
            fit_receipts_ok &= fit["variance_abstention"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            variance_bounds_ok &= fit["query_log_variance_min"] >= -6.0
            variance_bounds_ok &= fit["query_log_variance_max"] <= 3.0
    ...
    checks = {
        ...
        "new_conditional_variance_axis": semantic["classification"]
        == "NEW_P2_INPUT_CONDITIONAL_HETEROSCEDASTIC_GAUSSIAN_LOSS_ATTENUATION",
        ...
        "semantic_distinctions_all_true": all(
            semantic[name]
            for name in (
                "v21_fixed_student_t_distinguished",
                "v23_input_gradient_distinguished",
                "v24_sam_distinguished",
                "post_model_outlier_rules_distinguished",
            )
        ),
        "fixed_log_variance_bounds": (
            result["training"]["heteroscedastic_gaussian"]["log_variance_min"]
            == -6.0
            and result["training"]["heteroscedastic_gaussian"]["log_variance_max"]
            == 3.0
        ),
        "mean_only_inference": result["training"]["heteroscedastic_gaussian"][
            "inference_uses_mean_only"
        ],
        "variance_posthoc_zero": (
            result["training"]["heteroscedastic_gaussian"]["variance_router"] is False
            and result["training"]["heteroscedastic_gaussian"]["variance_abstention"]
            is False
            and result["training"]["heteroscedastic_gaussian"]["variance_head_sweep"]
            is False
        ),
        "nll_formula": runner._nll_formula_receipt()["maximum_abs_formula_error"]
        <= 1e-6,
        "mean_invariant_to_variance_head": model_contract[
            "mean_invariance_to_variance_head_perturbation_error"
        ]
        <= 1e-6,
        "masked_future_isolation": model_contract[
            "masked_future_token_maximum_abs_error"
        ]
        <= 1e-6,
        "permutation_invariance": model_contract[
            "permutation_maximum_abs_error"
        ]
        <= 1e-6,
        "fit_receipts_valid": fit_receipts_ok,
        "learned_variance_bounds": variance_bounds_ok,
    ...
    qa = {
        "schema_version": "p2.heteroscedastic_gaussian_domain_balanced_deepset.independent_qa.20260901.v25",
