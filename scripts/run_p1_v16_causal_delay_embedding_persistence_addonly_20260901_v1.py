"""Exactly-once causal delay-embedding persistence P1 falsification."""
...
EXPERIMENT_ID = "p1_v16_causal_delay_embedding_persistence_addonly_20260901_v1"
...
def _module(path: Path):
    spec = importlib.util.spec_from_file_location("p1_v16_shared_execution", path)
...
def _set_transport_context(frame: pd.DataFrame, train_boundary_ns: int) -> None:
    ...
    for item in config["parts"].values():
        ...
        index = max(0, int(len(prefix_times) * config["selection"]["inner_train_fraction"]) - 1)
        boundary = int(prefix_times[index])
...
def zero_dimensional_persistence(points: np.ndarray) -> np.ndarray:
    """Vietoris-Rips H0 finite death times, equal to Euclidean MST edges."""

    count = len(points)
    if count < 2:
        return np.zeros(0, dtype=np.float64)
    distance = np.sqrt(np.maximum(0.0, np.sum((points[:, None] - points[None, :]) ** 2, axis=2)))
    selected = np.zeros(count, dtype=bool)
    selected[0] = True
    best = distance[0].copy()
    best[0] = np.inf
    edges = np.empty(count - 1, dtype=np.float64)
    for index in range(count - 1):
        choice = int(np.argmin(np.where(selected, np.inf, best)))
        edges[index] = best[choice]
        selected[choice] = True
        best = np.minimum(best, distance[choice])
    return np.sort(edges)


def persistence_summary(points: np.ndarray) -> np.ndarray:
    edges = zero_dimensional_persistence(points)
    if len(edges) == 0:
        return np.zeros(8, dtype=np.float64)
    total = float(edges.sum())
    weights = edges / total if total > 0 else np.zeros_like(edges)
    positive = weights > 0
    entropy = -float(np.sum(weights[positive] * np.log(weights[positive])))
    if len(edges) > 1:
        entropy /= math.log(len(edges))
    distance = np.sqrt(np.maximum(0.0, np.sum((points[:, None] - points[None, :]) ** 2, axis=2)))
    centered = points - points.mean(axis=0)
    singular = np.linalg.svd(centered, compute_uv=False)
    energy = singular * singular
    anisotropy = float(energy[0] / energy.sum()) if energy.sum() > 0 else 0.0
    return np.array([total, edges.mean(), edges.max(), edges.std(), np.median(edges), entropy, distance.max(), anisotropy])


def _segment_features(values: np.ndarray, representation: dict[str, Any]) -> np.ndarray:
    window = int(representation["window_rows"])
    delay = int(representation["delay_rows"])
    dimension = int(representation["embedding_dimension"])
    point_count = int(representation["point_count"])
    stride = int(representation["evaluation_stride_rows"])
    minimum = max(window, (dimension - 1) * delay + point_count)
    output = np.zeros((len(values), 8), dtype=np.float64)
    for endpoint in range(minimum - 1, len(values), stride):
        first = endpoint - window + 1 + (dimension - 1) * delay
        candidates = np.arange(first, endpoint + 1, dtype=np.int64)
        take = np.linspace(0, len(candidates) - 1, point_count).round().astype(np.int64)
        ends = candidates[take]
        points = np.column_stack([values[ends - lag * delay] for lag in range(dimension)])
        summary = persistence_summary(points)
        output[endpoint : min(len(values), endpoint + stride)] = summary
    return output


def persistence_features(frame: pd.DataFrame, train_boundary_ns: int, representation: dict[str, Any]) -> np.ndarray:
    """Causal H0 persistence summaries reset at station-layer and cadence gaps."""
    ...
    for _key, group in frame.groupby(["station", "layer"], sort=True, observed=True):
        ...
        for start, end in zip(starts, ends, strict=True):
            feature[start:end] = _segment_features(values[start:end], representation)
        if not np.isfinite(feature).all():
            raise RuntimeError("persistence features are nonfinite")
...
def _write_v16(path: Path, value: dict[str, Any]) -> None:
    payload = value
...
def _configure() -> None:
    ...
    core.recurrence_laminar_features = persistence_features
    core._write = _write_v16
