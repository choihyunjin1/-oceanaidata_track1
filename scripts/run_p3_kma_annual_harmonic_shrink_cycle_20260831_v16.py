"""Run the sealed annual-harmonic smooth shrink of the v14b P3 KMA policy."""
...
EXPERIMENT_ID = "p3_kma_annual_harmonic_shrink_cycle_20260831_v16"
...
CONFIG = ROOT / "configs/experiments/p3_kma_annual_harmonic_shrink_cycle_20260831_v16.json"
TEST_FEATURES = ROOT / "artifacts/p3/features_all20_v1/test_features.parquet"
DELIVERY = Path(
    r"C:\Users\cedis\Downloads\해양 해커톤 제출용\20260831_P3_KMA_ANNUAL_HARMONIC_V16"
)

CANDIDATE = "P3_1_KMA_ANNUAL_HARMONIC_SMOOTH_SHRINK"
FAMILY_ID = "P3_KMA_CONTINUOUS_ENERGY_ANNUAL_HARMONIC_SHRINK"
TIER_ID = "LOW_DOF_FIXED"
REFERENCE_ALPHA = 0.425
ALPHA_18 = 0.20
TAU = 0.10
MIN_PHASE_COVERAGE_DAYS = 180.0
MAX_CONDITION = 1.0e4
PURGE_HOURS = 78
...
class ContractError(RuntimeError):
    """Raised when the sealed harmonic-shrink contract is violated."""


def harmonic(anchor_time: pd.Series) -> np.ndarray:
    local = pd.to_datetime(anchor_time, utc=True).dt.tz_convert("Asia/Seoul")
    phase = 2.0 * np.pi * local.dt.dayofyear.to_numpy(dtype=np.float64) / 365.25
    return np.column_stack([np.sin(phase), np.cos(phase)])


def v14b_correction(frame: pd.DataFrame, rank: np.ndarray) -> np.ndarray:
    correction = np.zeros(len(frame), dtype=np.float64)
    lead18 = frame["lead_h"].eq(18).to_numpy()
    lead24 = frame["lead_h"].eq(24).to_numpy()
    alpha24 = 0.20 + 0.40 * rank[lead24]
    correction[lead18] = (
        frame.loc[lead18, "base"].to_numpy(dtype=np.float64)
        + ALPHA_18 * frame.loc[lead18, "delta"].to_numpy(dtype=np.float64)
        - frame.loc[lead18, "reference"].to_numpy(dtype=np.float64)
    )
    correction[lead24] = (
        frame.loc[lead24, "base"].to_numpy(dtype=np.float64)
        + alpha24 * frame.loc[lead24, "delta"].to_numpy(dtype=np.float64)
        - frame.loc[lead24, "reference"].to_numpy(dtype=np.float64)
    )
    return correction


def fit_theta(train: pd.DataFrame, correction: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    if train.empty:
        return np.zeros(2), {
            "action": "abstain_empty_prefix",
            "train_rows": 0,
            "phase_coverage_days": 0.0,
            "condition_number": None,
            "theta_c": 0.0,
            "theta_d": 0.0,
            "ridge_solve_count": 1,
        }
    times = pd.to_datetime(train["anchor_time"], utc=True)
    coverage = float((times.max() - times.min()).total_seconds() / 86400.0)
    phase = harmonic(train["anchor_time"])
    matrix = correction[:, None] * phase
    system = matrix.T @ matrix + TAU * np.eye(2)
    condition = float(np.linalg.cond(system))
    if coverage < MIN_PHASE_COVERAGE_DAYS or condition > MAX_CONDITION:
        theta = np.zeros(2)
        action = "abstain_phase_or_condition"
    else:
        response = (
        ...
        theta = np.linalg.solve(system, matrix.T @ response)
        action = "closed_form_ridge_tau_0p10"
    if not np.isfinite(theta).all():
        raise ContractError("harmonic theta is non-finite")
    return theta, {
        "action": action,
        "train_rows": int(len(train)),
        "phase_coverage_days": coverage,
        "condition_number": condition,
        "theta_c": float(theta[0]),
        "theta_d": float(theta[1]),
        "ridge_solve_count": 1,
    }


def predict(frame: pd.DataFrame, correction: np.ndarray, theta: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    phase = harmonic(frame["anchor_time"])
    multiplier = np.clip(1.0 + phase @ theta, 0.0, 1.0)
    prediction = frame["reference"].to_numpy(dtype=np.float64) + multiplier * correction
    return np.clip(prediction, 0.0, 30.0), multiplier
...
def evaluate(frame: pd.DataFrame, history: pd.DataFrame) -> dict[str, Any]:
    ...
    for block in block_order(frame):
        ...
        theta, receipt = fit_theta(train, all_correction[train_mask.to_numpy()])
        prefix = np.sort(
            history.loc[
        ...
        correction = v14b_correction(valid, valid_rank)
        fold_prediction, fold_multiplier = predict(valid, correction, theta)
    ...
    if not np.isfinite(prediction).all():
        raise ContractError("harmonic OOF prediction incomplete")
...
def materialize(passed: bool, frame: pd.DataFrame, history: pd.DataFrame) -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    prediction, _ = predict(official, official_correction, theta)
...
def main() -> int:
    ...
    result = {"schema_version": "p3.kma_annual_harmonic_shrink.result.v16", "experiment_id": EXPERIMENT_ID, "created_at_utc": datetime.now(UTC).isoformat(), "status": "COMPLETE", "decision": "PASS_MATERIALIZED_NOT_UPLOADED" if candidate["passed"] else "NO_GO_ANNUAL_HARMONIC_SHRINK_GATE", "candidate_count": 1, "passing_candidate_count": int(candidate["passed"]), "candidate": candidate, "fit_budget": {"prefix_ridge_solves": 6, "model_fits": 0, "full_deployment_solves": int(candidate["passed"])}, "data_profile": profile, "outputs": outputs, "data_access": access, "provenance": {"runner_sha256": runner_hash, "config_sha256": sha256(CONFIG), "calibration_sha256": sha256(CALIBRATION)}, "execution": {"elapsed_seconds": elapsed, "python": platform.python_version(), "result_based_tuning_or_retry": False, "hidden_truth_rows_read": 0, "upload_attempt_count": 0}}
    result_path = ARTIFACT_DIR / "result.json"
    write_new(result_path, canonical(result))
    report = "# P3 annual harmonic smooth shrink v16\n\n## 결론\n\n" + f"- {result['decision']}; PASS {result['passing_candidate_count']}/1; CSV {len(outputs)}; upload 0.\n" + f"- pooled delta {candidate['delta_candidate_minus_reference_rmse_m']:+.9f}m; blocks {candidate['improved_bimonth_blocks']}/6; episode CI {candidate['episode_bootstrap']['ci90_m']}; group CI {candidate['block_station_bootstrap']['ci90_m']}.\n" + f"- raw LCB {candidate['expected_points']['local_lcb_raw_points']:.9f}; calibrated {candidate['expected_points']['calibrated_conservative']:.9f} points.\n"
