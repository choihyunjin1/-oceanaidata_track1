"""Independent QA for sealed P2 v29 fixed-Lookahead run."""
...
import run_p2_lookahead_domain_balanced_deepset_20260901_v29 as runner  # noqa: E402
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["slow_fast_delta_l2_first_sync"],
                fit["slow_fast_delta_l2_last_sync"],
                fit["slow_fast_delta_l2_max"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["lookahead_period_steps"] == 5
            fit_receipts_ok &= fit["lookahead_alpha"] == 0.5
            fit_receipts_ok &= (
                fit["lookahead_synchronizations"] == fit["expected_synchronizations"]
            )
            fit_receipts_ok &= fit["posthoc_weight_ensemble"] == 0
            fit_receipts_ok &= fit["input_gradient_penalty"] == 0
            fit_receipts_ok &= fit["parameter_neighborhood_perturbation"] == 0
            fit_receipts_ok &= fit["gradient_surgery"] == 0
            fit_receipts_ok &= fit["data_augmentation"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            lookahead_active &= fit["lookahead_synchronizations"] > 0
    ...
    lookahead = runner._lookahead_contract_receipt()
    ...
    checks = {
        ...
        "new_lookahead_axis": semantic["classification"]
        == "NEW_P2_FIXED_LOOKAHEAD_ADAMW_OPTIMIZER_TRAJECTORY",
        ...
        "semantic_distinctions_all_true": all(
            ...
            for name in (
                ...
                "v25_variance_head_distinguished",
                "v26_mixup_distinguished",
                "v28_pcgrad_distinguished",
            )
        ),
        "lookahead_formula": (
            lookahead["formula_exact"]
            and lookahead["zero_difference_exact_noop"]
            and lookahead["byte_identical_replay"]
            and not lookahead["posthoc_weight_ensemble"]
        ),
        ...
        "lookahead_actually_used": lookahead_active,
        ...
        == bool(record["legacy_safety_pass_without_v26a_amendment"] and independent_local_gate["pass"]),
    ...
    qa = {
        "schema_version": "p2.lookahead_domain_balanced_deepset.independent_qa.20260901.v29",
