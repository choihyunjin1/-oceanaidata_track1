"""Hermetic trust bootstrap for the P2 Layer-4 r3 compatibility verifier v5."""

from __future__ import annotations

import sys as _sys

_IDENTITY = "P2_JOINT_HYDROGRAPHIC_MULTITASK_LAYER4_R3_COMPATIBILITY_BOOTSTRAP_V5"
_FORBIDDEN_ROOTS = (
    "_ctypes",
    "_overlapped",
    "_socket",
...
_FIREWALL_SELF_TEST_BASELINE: dict[str, int] | None = None
_CAPABILITY_STATE: dict[str, object] = {
    "sealed": False,
    "blocked_calls_total": 0,
    "expected_self_test_blocked_calls": 7,
    "successful_write_process_network_registry_actions": 0,
    "retained_main_native_callables": [],
}


class BootstrapV5Error(RuntimeError):
    """The v5 isolated trust bootstrap failed closed."""
...
def _audit_hook(event: str, arguments: tuple[object, ...]) -> None:
    if _CAPABILITY_STATE["sealed"] is True and event in {"exec", "marshal.loads"}:
        _FIREWALL["write_process_network_registry_attempts"] += 1
        raise PermissionError(f"P2 v5 sealed bytecode execution rejected: {event}")
    if event == "open":
        ...
        if bytecode_read:
            ...
            raise PermissionError("P2 v5 audit firewall rejected bytecode access")
        ...
        if textual_write or flag_write:
            _FIREWALL["write_process_network_registry_attempts"] += 1
            raise PermissionError("P2 v5 audit firewall rejected a write-capable open")
    if event in _MUTATION_OR_PROCESS_EVENTS or event.startswith(_BLOCKED_EVENT_PREFIXES):
        _FIREWALL["write_process_network_registry_attempts"] += 1
        raise PermissionError(f"P2 v5 audit firewall rejected {event}")
    if event == "_winapi.CreateFile":
        ...
        if (
            ...
            _FIREWALL["write_process_network_registry_attempts"] += 1
            raise PermissionError("P2 v5 audit firewall rejected write/create WinAPI access")
    elif event.startswith("_winapi."):
        _FIREWALL["write_process_network_registry_attempts"] += 1
        raise PermissionError(f"P2 v5 audit firewall rejected {event}")
    if event == "import" and arguments:
        name = str(arguments[0])
        if _CAPABILITY_STATE["sealed"] is True:
            _FIREWALL["unexpected_import_attempts"] += 1
            _UNEXPECTED_IMPORT_NAMES.append(name)
            raise ImportError(f"P2 v5 post-seal import rejected: {name}")
        if _is_forbidden_import(name):
            _FIREWALL["forbidden_import_attempts"] += 1
            raise ImportError(f"P2 v5 forbidden import rejected: {name}")
        root = name.split(".", 1)[0]
        if (
            ...
            _FIREWALL["unexpected_import_attempts"] += 1
            _UNEXPECTED_IMPORT_NAMES.append(name)
            raise ImportError(f"P2 v5 unauthenticated import rejected: {name}")
...
def _assert_initial_runtime() -> None:
    ...
    if _sys.path != expected_path:
        raise BootstrapV5Error("initial isolated sys.path changed")
    expected_meta = [
    ...
    if observed_meta != expected_meta:
        raise BootstrapV5Error("initial import finder chain changed")
    expected_modules = {
        "__future__",
        "__main__",
        "_abc",
...
if any(
    ...
    raise BootstrapV5Error("phase-zero module origin changed")
...
def _reject_reparse_chain(path: str, *, require_target: bool = True) -> str:
    ...
    for part in [item for item in tail.replace("/", "\\").split("\\") if item]:
        ...
        try:
            ...
            continue
        if missing:
            raise BootstrapV5Error("filesystem ancestor identity is inconsistent")
        if int(getattr(info, "st_file_attributes", 0)) & _REPARSE_ATTRIBUTE:
            raise BootstrapV5Error(f"link/reparse ancestor is forbidden: {current}")
    if require_target:
        try:
            _nt.lstat(full)
...
def _lock_path(path: str, *, directory: bool) -> dict[str, object]:
    ...
    if existing is not None:
        if bool(existing["directory"]) != directory:
            raise BootstrapV5Error("stable handle kind changed")
        return existing
    ...
    if (
        ...
        raise BootstrapV5Error("opened stable handle resolves to a reparse point")
    if not directory and (
        ...
        _nt.close(descriptor)
        raise BootstrapV5Error("stable regular file must have exactly one hard link")
    ...
    if not directory:
        ...
        if len(raw) != int(descriptor_stat.st_size):
            _nt.close(descriptor)
            raise BootstrapV5Error("stable handle read was truncated")
...
if (
    ...
    raise BootstrapV5Error("pure SHA-256 self-test failed")
...
_STARTUP_PINS_PUBLIC: dict[str, dict[str, object]] = {
    "PYCACHE_SENTINEL": {
        "scope": "workspace",
        "relative": _PYCACHE_RELATIVE,
        "bytes": 67,
        "sha256": "932aa01b4026ed06cc675d1312fdf5f29f2247327319c3eaceec24fe7991f903",
    },
    "VENV_PYTHON": {
        "scope": "workspace",
...
def _argument_value(name: str) -> str:
    ...
    if values.count(name) != 1:
        raise BootstrapV5Error(f"exactly one {name} is required")
    index = values.index(name)
    if index + 1 >= len(values) or values[index + 1].startswith("--"):
        raise BootstrapV5Error(f"{name} requires a value")
...
def _argument_attestation() -> str:
    ...
    if len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise BootstrapV5Error("external launcher attestation must be a lowercase SHA-256")
    return value


def _argument_runtime_attestation() -> str:
    value = _argument_value("--powershell-runtime-attestation")
    if value != "9a197570fffc3399d9c8477ef0199e31ad950701de7b133df7c4669d42099be1":
        raise BootstrapV5Error("complete PowerShell runtime attestation changed")
    return value


def _argument_stage_zero_contract() -> str:
    value = _argument_value("--stage-zero-contract")
    if value != "P2_V5_PATH_IMMUTABLE_INLINE_STAGE_ZERO_V1":
        raise BootstrapV5Error("path-immutable stage-zero contract changed")
    return value


def _argument_stage_zero_attestation() -> str:
    value = _argument_value("--stage-zero-attestation")
    if len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise BootstrapV5Error("stage-zero attestation must be a lowercase SHA-256")
    return value


_WORKSPACE_TEXT = _reject_reparse_chain(_argument_root())
if not bool(_nt.stat(_WORKSPACE_TEXT).st_mode & 0o040000):
    raise BootstrapV5Error("workspace is not a directory")
_BASE_TEXT = _full(_sys.base_prefix)
_EXTERNAL_LAUNCHER_ATTESTATION = _argument_attestation()
_POWERSHELL_RUNTIME_ATTESTATION = _argument_runtime_attestation()
_STAGE_ZERO_CONTRACT = _argument_stage_zero_contract()
_STAGE_ZERO_ATTESTATION = _argument_stage_zero_attestation()
_PYCACHE_TEXT = _join(_WORKSPACE_TEXT, _PYCACHE_RELATIVE)
_reject_reparse_chain(_PYCACHE_TEXT)
_pycache_stat = _nt.lstat(_PYCACHE_TEXT)
if (
    not bool(int(_pycache_stat.st_mode) & 0o100000)
    or int(_pycache_stat.st_nlink) != 1
    or int(_pycache_stat.st_size) != 67
):
    raise BootstrapV5Error("canonical pycache sentinel must be a regular one-link file")
if _case(str(_sys.pycache_prefix)) != _case(_PYCACHE_TEXT):
    raise BootstrapV5Error("canonical held pycache sentinel prefix changed")
...
def _verify_phase_pin(path: str, *, size: int, digest: str, label: str) -> bytes:
    ...
    if not isinstance(raw, bytes) or len(raw) != size or _pure_sha256(raw) != digest:
        raise BootstrapV5Error(f"phase-zero pin changed: {label}")
    return raw
...
for _startup_role, _startup_pin in _STARTUP_PINS_PUBLIC.items():
    ...
    if _startup_scope not in {"workspace", "base"}:
        raise BootstrapV5Error("startup pin scope changed")
...
if _case(_sys.executable) != _case(_expected_venv_python):
    raise BootstrapV5Error("canonical workspace interpreter is required")
if not _sys.orig_argv or _case(_sys.orig_argv[0]) != _case(_expected_base_python):
    raise BootstrapV5Error("canonical base interpreter origin changed")
...
if not isinstance(_pyvenv_raw, bytes):
    raise BootstrapV5Error("pyvenv.cfg stable buffer changed")
try:
    _pyvenv_text = _pyvenv_raw.decode("utf-8")
except UnicodeDecodeError as exc:
    raise BootstrapV5Error("pyvenv.cfg is not UTF-8") from exc
_pyvenv_home = [
...
if len(_pyvenv_home) != 1 or _case(_pyvenv_home[0]) != _case(_BASE_TEXT):
    raise BootstrapV5Error("pyvenv.cfg base-prefix resolution changed")
...
for _startup_module, _startup_relative in _startup_sources.items():
    ...
    if (
        ...
        raise BootstrapV5Error(f"pre-hook startup source origin changed: {_startup_module}")
    ...
    if not isinstance(_startup_cached, str) or not _case(_startup_cached).startswith(
        ...
        raise BootstrapV5Error(f"startup cache destination changed: {_startup_module}")
    try:
        _nt.lstat(_startup_cached)
    except FileNotFoundError:
        pass
    else:
        raise BootstrapV5Error(f"startup bytecode cache unexpectedly exists: {_startup_module}")
...
for _native_startup in ("_codecs_kr", "_multibytecodec"):
    ...
    if (
        ...
        raise BootstrapV5Error(f"pre-hook native module origin changed: {_native_startup}")
...
if _case(str(_trusted_hashlib.__spec__.origin)) != _case(_HASHLIB_PATH):
    raise BootstrapV5Error("authenticated _hashlib origin changed")
...
def _pycache_sentinel_intact() -> bool:
    entry = _PHASE_LOCKS.get(_case(_PYCACHE_TEXT))
    if entry is None or entry.get("directory") is not False:
        return False
    raw = entry.get("raw")
    try:
        path_stat = _nt.lstat(_PYCACHE_TEXT)
        descriptor_stat = _nt.fstat(int(entry["descriptor"]))
    except (FileNotFoundError, OSError):
        return False
    return (
        isinstance(raw, bytes)
        and len(raw) == 67
        and _fast_sha256(raw)
        == "932aa01b4026ed06cc675d1312fdf5f29f2247327319c3eaceec24fe7991f903"
        and _identity_stat(path_stat) == entry["identity"]
        and _identity_stat(descriptor_stat) == entry["identity"]
        and int(path_stat.st_nlink) == 1
    )


def _external_startup_report() -> dict[str, object]:
    if not _pycache_sentinel_intact():
        raise BootstrapV5Error("canonical pycache sentinel changed")
    for role, pin in _STARTUP_PINS_PUBLIC.items():
        ...
        if entry is None or entry["directory"] is not False:
            raise BootstrapV5Error(f"startup stable handle disappeared: {role}")
        ...
        if (
            ...
            raise BootstrapV5Error(f"startup stable identity changed: {role}")
        ...
        if size != pin["bytes"] or digest.hexdigest() != pin["sha256"]:
            raise BootstrapV5Error(f"startup same-handle final hash changed: {role}")
    return {
        ...
        "stage_zero_contract": _STAGE_ZERO_CONTRACT,
        "stage_zero_attestation_sha256": _STAGE_ZERO_ATTESTATION,
        "powershell_runtime_inventory_sha256": _POWERSHELL_RUNTIME_ATTESTATION,
        "powershell_runtime_directories": 53,
        "powershell_runtime_files": 983,
        "powershell_runtime_file_bytes": 296034085,
        "powershell_runtime_externally_held_before_host_start": True,
        "stage_one_authenticated_before_memory_evaluation": True,
        "stage_one_path_execution": False,
        ...
        "canonical_pycache_sentinel_regular_held_intact": True,
...
def _inventory_stdlib() -> tuple[int, int, int, str]:
    ...
    while pending:
        ...
        for child in children:
            ...
            if attributes & _REPARSE_ATTRIBUTE:
                raise BootstrapV5Error(f"stdlib reparse entry is forbidden: {child.path}")
            ...
            if child.is_dir(follow_symlinks=False):
                ...
                raise BootstrapV5Error("stdlib contains a special filesystem entry")
    ...
    for kind, relative, path in records:
        ...
        if not isinstance(raw, bytes):
            raise BootstrapV5Error("stdlib file buffer changed type")
...
if _STDLIB_INVENTORY != (
    ...
    raise BootstrapV5Error(f"authenticated stdlib inventory changed: {_STDLIB_INVENTORY}")
    ...
    if _relative.startswith("Lib/") and _relative.endswith(".py"):
        ...
        if _module_parts:
            ...
            if _module_name in _SOURCE_MODULES:
                raise BootstrapV5Error(f"duplicate stdlib source module: {_module_name}")
        ...
        if _module_name in _NATIVE_MODULES:
            raise BootstrapV5Error(f"duplicate stdlib native module: {_module_name}")
...
_TRUSTED_PINS: dict[str, dict[str, Any]] = {
    "CONFIG": {
        "path": "configs/experiments/p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5.json",
        "bytes": 17524,
        "sha256": "66dbab0a90ce3941edebcd3c9dc4d5a6139f03ed621a26312a30f8ac932b2b28",
    },
    "HELPER": {
        "path": "src/p2_restore/joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5.py",
        "bytes": 38542,
        "sha256": "957e2cd3ef987c9c5741f3fac54c9f8cd4c9df77b26c1bf6c8e213179d58cc4b",
    },
    "CLI": {
        "path": "scripts/verify_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5.py",
        "bytes": 2113,
        "sha256": "a57dcaae076d9278bc8d5c7df86344cdfe7b07b65e539964206ad1dde8149828",
    },
    "TESTS": {
        "path": "tests/test_p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5.py",
        "bytes": 25586,
        "sha256": "65ec601f275215ea187988eee4fd781e486b76d5bc205a7ae497861c4498b037",
    },
}
_HELPER_MODULE = "p2_restore.joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5"
_CLI_MODULE = "p2_restore.joint_hydrographic_multitask_layer4_r3_compatibility_cli_v5"


def _relative_parts(relative: str) -> tuple[str, ...]:
    if not isinstance(relative, str) or not relative or "\\" in relative:
        raise BootstrapV5Error("path is not canonical POSIX relative text")
    pure = PurePosixPath(relative)
    if pure.is_absolute() or any(part in {"", ".", ".."} for part in pure.parts):
        raise BootstrapV5Error("path is not a contained relative path")
    return pure.parts
...
def _workspace_path(
    ...
    *,
    ...
    try:
        ...
        raise BootstrapV5Error(f"path escaped workspace: {relative}") from exc
    if must_exist:
        if kind == "file" and not resolved.is_file():
            raise BootstrapV5Error(f"regular file required: {relative}")
        if kind == "directory" and not resolved.is_dir():
            raise BootstrapV5Error(f"directory required: {relative}")
    return resolved
...
def _relative_from_path(path: Path) -> str:
    ...
    try:
        ...
        raise BootstrapV5Error("path is outside workspace") from exc


def _pin_tuple(pin: Mapping[str, Any]) -> tuple[str, int, str]:
    if set(pin) != {"path", "bytes", "sha256"}:
        raise BootstrapV5Error("pin field set changed")
    ...
    if (
        ...
        raise BootstrapV5Error("pin value changed")
...
def _unique_json(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise BootstrapV5Error(f"duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_json_constant(value: str) -> Any:
    raise BootstrapV5Error(f"non-finite JSON number: {value}")
...
def _parse_json_buffer(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise BootstrapV5Error(f"invalid JSON: {label}") from exc
    if not isinstance(value, dict):
        raise BootstrapV5Error(f"JSON root must be an object: {label}")
    return value
...
class _StableRegistry:
    def __init__(self) -> None:
        ...
        self._path_accessor = _lock_path

    def freeze_read_surface(self) -> None:
        locks = _PHASE_LOCKS
        case = _case
        reject_chain = _reject_reparse_chain

        def existing_only(path: str, *, directory: bool) -> dict[str, object]:
            full = reject_chain(path)
            entry = locks.get(case(full))
            if entry is None or bool(entry["directory"]) != directory:
                raise BootstrapV5Error("post-seal access is not an existing held path")
            return entry

        self._path_accessor = existing_only

    def existing_entry(self, path: str, *, directory: bool) -> dict[str, object]:
        return self._path_accessor(path, directory=directory)
    ...
    def lock_pin(self, pin: Mapping[str, Any], *, label: str) -> bytes:
        ...
        if prior is not None and prior != exact:
            raise BootstrapV5Error(f"conflicting stable pin: {label}")
        path = _workspace_path(relative, kind="file")
        entry = self._path_accessor(str(path), directory=False)
        raw = entry["raw"]
        if not isinstance(raw, bytes) or len(raw) != size or _fast_sha256(raw) != digest:
            raise BootstrapV5Error(f"stable authenticated bytes changed: {label}")
    ...
    def lock_many(self, pins: Mapping[str, Mapping[str, Any]]) -> None:
        for relative in sorted(pins):
            ...
            if pin.get("path") != relative:
                raise BootstrapV5Error("stable registry key/path mismatch")
    ...
    def bytes_for_path(self, path: Path) -> bytes:
        ...
        if pin is None:
            raise BootstrapV5Error(f"unregistered semantic read: {relative}")
        ...
        if entry is None or not isinstance(entry.get("raw"), bytes):
            raise BootstrapV5Error("stable semantic buffer is unavailable")
        return entry["raw"]
    ...
    def seal_tree(
        ...
        while pending:
            ...
            locked = self._path_accessor(str(current), directory=True)
            ...
            for child in children:
                ...
                if int(getattr(info, "st_file_attributes", 0)) & _REPARSE_ATTRIBUTE:
                    raise BootstrapV5Error(f"tree reparse entry is forbidden: {child.path}")
                ...
                if child.is_dir(follow_symlinks=False):
                    ...
                    if pin is None:
                        raise BootstrapV5Error(f"tree file is not allowlisted: {relative}")
                    ...
                    raise BootstrapV5Error("tree contains a special entry")
        if observed != expected:
            missing = sorted(expected - observed)
            raise BootstrapV5Error(f"tree allowlist has missing files: {missing[:3]}")
    ...
    def final_reverify(self) -> dict[str, Any]:
        ...
        for relative in sorted(self._pins):
            ...
            if entry is None:
                raise BootstrapV5Error("stable file handle disappeared")
            descriptor = int(entry["descriptor"])
            if _identity_stat(_nt.fstat(descriptor)) != entry["identity"]:
                raise BootstrapV5Error("stable descriptor identity changed")
            if _identity_stat(_nt.lstat(str(path))) != entry["identity"]:
                raise BootstrapV5Error("stable path identity changed")
            if int(_nt.fstat(descriptor).st_nlink) != 1:
                raise BootstrapV5Error("stable regular-file hard-link count changed")
            _nt.lseek(descriptor, 0, 0)
            digest = _trusted_hashlib.openssl_sha256()
            size = 0
            while True:
                ...
                digest.update(block)
            if size != pin["bytes"] or digest.hexdigest() != pin["sha256"]:
                raise BootstrapV5Error("same-handle final hash changed")
            total += size
        for relative, identity in sorted(self._tree_directories.items()):
            ...
            if (
                ...
                raise BootstrapV5Error("held directory identity changed")
        return {
...
def _exec_buffer(
    *,
    ...
    if name in _sys.modules:
        raise BootstrapV5Error(f"authenticated module already loaded: {name}")
...
def _operational_firewall() -> dict[str, object]:
    baseline = _FIREWALL_SELF_TEST_BASELINE
    numeric = (
        "write_process_network_registry_attempts",
        "forbidden_import_attempts",
        "unexpected_import_attempts",
        "bytecode_read_attempts",
    )
    return {
        "installed": _FIREWALL["installed"],
        "dependency_ready": _FIREWALL["dependency_ready"],
        **{
            key: int(_FIREWALL[key]) - (0 if baseline is None else int(baseline[key]))
            for key in numeric
        },
    }


def _seal_native_capabilities() -> dict[str, object]:
    """Replace every retained mutation/process/native escape callable, then self-test it."""

    global _FIREWALL_SELF_TEST_BASELINE
    if _CAPABILITY_STATE["sealed"] is True:
        raise BootstrapV5Error("native capability seal may run only once")
    wrapped_names: list[str] = []

    def blocked(label: str):
        def reject(*_arguments: object, **_keywords: object) -> object:
            _CAPABILITY_STATE["blocked_calls_total"] = (
                int(_CAPABILITY_STATE["blocked_calls_total"]) + 1
            )
            raise PermissionError(f"P2 v5 sealed native capability rejected: {label}")

        reject.__name__ = "p2_v5_sealed_native_capability"
        reject.__module__ = __name__
        return reject

    winapi_module = _sys.modules.get("_winapi")
    msvcrt_module = _sys.modules.get("msvcrt")
    winreg_module = _sys.modules.get("winreg")
    if winapi_module is None or msvcrt_module is None or winreg_module is None:
        raise BootstrapV5Error("required native modules disappeared before sealing")
    subprocess_module = _sys.modules.get("subprocess")
    if subprocess_module is None:
        raise BootstrapV5Error("authenticated subprocess module was not loaded before sealing")
    for module in (winapi_module, msvcrt_module, winreg_module, subprocess_module):
        for name in tuple(vars(module)):
            value = getattr(module, name, None)
            if callable(value):
                setattr(module, name, blocked(f"{module.__name__}.{name}"))
                wrapped_names.append(f"{module.__name__}.{name}")
    ...
    for module_name in ("nt", "os"):
        module = _sys.modules.get(module_name)
        if module is None:
            continue
        for name in dangerous_nt:
            if callable(getattr(module, name, None)):
                setattr(module, name, blocked(f"{module_name}.{name}"))
                wrapped_names.append(f"{module_name}.{name}")
    shutil_module = _sys.modules.get("shutil")
    if shutil_module is not None:
        for name in {
            "chown",
            "copy",
            "copy2",
            ...
            if callable(getattr(shutil_module, name, None)):
                setattr(shutil_module, name, blocked(f"shutil.{name}"))
                wrapped_names.append(f"shutil.{name}")
    imp_module = _sys.modules.get("_imp")
    if imp_module is None:
        raise BootstrapV5Error("built-in import broker disappeared before sealing")
    for name in {
        "create_builtin",
        "create_dynamic",
        "exec_builtin",
        ...
        if callable(getattr(imp_module, name, None)):
            setattr(imp_module, name, blocked(f"_imp.{name}"))
            wrapped_names.append(f"_imp.{name}")

    _STABLE_REGISTRY.freeze_read_surface()
    _sys.meta_path[:] = []
    globals().pop("_winapi", None)
    globals().pop("_msvcrt", None)
    for broker_name in (
        "_lock_path",
        "_verify_phase_pin",
        "_inventory_stdlib",
        "_exec_buffer",
        "_AuthenticatedSourceLoader",
        "_AuthenticatedFinder",
        "_AUTH_FINDER",
        "_import_core",
        "_import_external",
    ):
        globals().pop(broker_name, None)

    main_module = _sys.modules.get("__main__")
    retained: list[str] = []
    if main_module is not None:
        for name, value in vars(main_module).items():
            owner = str(getattr(value, "__module__", ""))
            code = getattr(value, "__code__", None)
            native_broker_names = set(getattr(code, "co_names", ())) & {
                "_winapi",
                "_msvcrt",
                "CreateFile",
                "open_osfhandle",
            ...
            if (
                callable(value)
                and owner in {"_winapi", "msvcrt", "winreg", "_ctypes"}
            ) or native_broker_names:
                retained.append(name)
    if retained:
        raise BootstrapV5Error(
            f"retained original or broker native callable in __main__: {retained}"
        )
    _CAPABILITY_STATE["retained_main_native_callables"] = retained
    _CAPABILITY_STATE["wrapped_dangerous_callable_count"] = len(wrapped_names)
    _CAPABILITY_STATE["sealed"] = True

    probes = (
        ("_winapi.CreateFileMapping", lambda: winapi_module.CreateFileMapping(-1, 0, 4, 0, 4096, "Local\\P2V5BlockedProbe")),
        ("_winapi.WriteFile", lambda: winapi_module.WriteFile(-1, b"P2V5_BLOCKED", 0)),
        ("_winapi.CreateProcess", lambda: winapi_module.CreateProcess("blocked", "blocked", None, None, False, 0, None, None, None)),
        ("winreg.OpenKey", lambda: winreg_module.OpenKey(0, "Software")),
        ("subprocess.Popen", lambda: subprocess_module.Popen(["blocked"])),
        ("nt.write", lambda: _nt.write(1, b"P2V5_BLOCKED")),
        ("_imp.create_builtin", lambda: imp_module.create_builtin(None)),
    )
    observed: list[str] = []
    for label, probe in probes:
        try:
            ...
            _CAPABILITY_STATE["successful_write_process_network_registry_actions"] = (
                int(_CAPABILITY_STATE["successful_write_process_network_registry_actions"]) + 1
            )
            raise BootstrapV5Error(f"sealed capability probe unexpectedly succeeded: {label}")
    for name in ("ctypes", "socket", "mmap"):
        try:
            __import__(name)
        except ImportError:
            observed.append(f"import:{name}")
        else:
            raise BootstrapV5Error(f"forbidden native module import succeeded: {name}")
    for name in ("_winapi", "nt"):
        saved_module = _sys.modules.pop(name)
        try:
            try:
                __import__(name)
            except ImportError:
                observed.append(f"fresh-import:{name}")
            else:
                raise BootstrapV5Error(f"fresh built-in module import succeeded: {name}")
        finally:
            _sys.modules[name] = saved_module
    if int(_CAPABILITY_STATE["blocked_calls_total"]) != 7:
        raise BootstrapV5Error("real native capability self-test count changed")
    _FIREWALL_SELF_TEST_BASELINE = {
        key: int(value)
        for key, value in _FIREWALL.items()
        if key not in {"installed", "dependency_ready"}
    }
    return {
        **_CAPABILITY_STATE,
        "real_blocked_probes": observed,
        "operational_firewall": _operational_firewall(),
    }


def _capability_report() -> dict[str, object]:
    if _CAPABILITY_STATE["sealed"] is not True:
        raise BootstrapV5Error("native capability surface was not sealed")
    if (
        int(_CAPABILITY_STATE["blocked_calls_total"])
        != int(_CAPABILITY_STATE["expected_self_test_blocked_calls"])
        or int(_CAPABILITY_STATE["successful_write_process_network_registry_actions"]) != 0
        or _CAPABILITY_STATE["retained_main_native_callables"] != []
        or any(
            int(value) != 0
            for key, value in _operational_firewall().items()
            if key not in {"installed", "dependency_ready"}
        )
    ):
        raise BootstrapV5Error("post-seal zero-action capability state changed")
    return {
        **_CAPABILITY_STATE,
        "operational_firewall": _operational_firewall(),
        "real_self_test_probe_count": 12,
        "real_self_test_effect_count": 0,
    }
    ...
    for name, module in sorted(_sys.modules.items()):
        ...
        if not matches:
            if name == "__main__" and case == _case(str(Path(__file__).resolve(strict=True))):
                continue
            raise BootstrapV5Error(f"loaded module origin is not authenticated: {name}")
        file_modules.append({"module": name, "origin": matches[0]})
    if any("site-packages" in item["origin"].casefold() for item in file_modules):
        raise BootstrapV5Error("site-packages module entered the runtime")
    return {
...
def _assert_runtime() -> None:
    ...
    if (
        ...
        or not _pycache_sentinel_intact()
        or tuple(_sys.path) != _INITIAL_SYS_PATH
        or _sys.meta_path != expected_meta_path
        or _loaded_forbidden()
        or _operational_firewall()
        != {
            "installed": True,
...
def _dependency_report() -> dict[str, Any]:
    ...
    return {
        ...
        "meta_path": [
            str(getattr(finder, "__name__", type(finder).__name__))
            for finder in _sys.meta_path
        ],
        ...
        "pycache_sentinel_regular_held_intact": _pycache_sentinel_intact(),
        "stdlib_inventory": {
        ...
        "firewall": _operational_firewall(),
        "native_capability_surface": _capability_report(),
    }


def _parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hermetic read-only P2 Layer-4 r3 compatibility verifier v5."
    )
    ...
    parser.add_argument("--powershell-runtime-attestation", required=True)
    parser.add_argument("--stage-zero-contract", required=True)
    parser.add_argument("--stage-zero-attestation", required=True)
...
def main() -> int:
    ...
    if arguments.root.resolve(strict=True) != workspace:
        raise BootstrapV5Error("parsed workspace changed")
    if arguments.external_launcher_attestation != _EXTERNAL_LAUNCHER_ATTESTATION:
        raise BootstrapV5Error("parsed external launcher attestation changed")
    if arguments.powershell_runtime_attestation != _POWERSHELL_RUNTIME_ATTESTATION:
        raise BootstrapV5Error("parsed PowerShell runtime attestation changed")
    if arguments.stage_zero_contract != _STAGE_ZERO_CONTRACT:
        raise BootstrapV5Error("parsed stage-zero contract changed")
    if arguments.stage_zero_attestation != _STAGE_ZERO_ATTESTATION:
        raise BootstrapV5Error("parsed stage-zero attestation changed")
    ...
    if (
        ...
        raise BootstrapV5Error("canonical interpreter pin changed")
    ...
    if (
        ...
        raise BootstrapV5Error("pyvenv.cfg pin changed")
    observed: dict[str, dict[str, Any]] = {}
    buffers: dict[str, bytes] = {}
    for role in _EXPECTED_ROLES:
        pin = _TRUSTED_PINS[role]
        raw = _STABLE_REGISTRY.lock_pin(pin, label=f"v5 {role}")
        observed[role] = dict(pin)
        buffers[role] = raw
    config = _parse_json_buffer(buffers["CONFIG"], "v5 config")
    if config.get("implementation_roles") != {
        role: _TRUSTED_PINS[role]["path"] for role in _EXPECTED_ROLES
    }:
        raise BootstrapV5Error("config implementation role paths changed")
    ...
    if requested is not None:
        ...
        if requested_path.resolve(strict=True) != canonical:
            raise BootstrapV5Error("alternate v5 config is forbidden")
    ...
    if not isinstance(bootstrap_raw, bytes):
        raise BootstrapV5Error("bootstrap source buffer changed")
    bootstrap_pin = {
    ...
    context = {
        ...
        "arguments": {
            ...
            "powershell_runtime_attestation": arguments.powershell_runtime_attestation,
            "stage_zero_contract": arguments.stage_zero_contract,
            "stage_zero_attestation": arguments.stage_zero_attestation,
        },
        "config": config,
        ...
        "seal_native_capabilities": _seal_native_capabilities,
        "native_capability_report": _capability_report,
        "external_startup_report": _external_startup_report,
        "startup_pins_public": _STARTUP_PINS_PUBLIC,
        "pycache_prefix_relative": _PYCACHE_RELATIVE,
        "pycache_sentinel_intact": _pycache_sentinel_intact,
        "firewall": _FIREWALL,
    }
    _assert_runtime()
    helper = _exec_buffer(
        ...
        raw=buffers["HELPER"],
        injected={"_P2_V5_BOOTSTRAP_CONTEXT": context},
    )
    _assert_runtime()
    cli = _exec_buffer(
        name=_CLI_MODULE,
        ...
        raw=buffers["CLI"],
        injected={
            "_P2_V5_BOOTSTRAP_CONTEXT": context,
            "_P2_V5_AUTHENTICATED_HELPER": helper,
        },
    )
    ...
    if {
        role: {
            "path": pin["path"],
            "bytes": len(_STABLE_REGISTRY.lock_pin(pin, label=f"v5 final {role}")),
            "sha256": _fast_sha256(_STABLE_REGISTRY.lock_pin(pin, label=f"v5 final hash {role}")),
        }
        for role, pin in _TRUSTED_PINS.items()
    } != _TRUSTED_PINS:
        raise BootstrapV5Error("v5 implementation changed during verification")
    current_bootstrap = _STABLE_REGISTRY.existing_entry(
        str(bootstrap_path), directory=False
    )["raw"]
    if (
        ...
        raise BootstrapV5Error("bootstrap changed during verification")
    _assert_runtime()
    report["authenticated_bootstrap"] = {
        "identity": _IDENTITY,
        "observed_pin": bootstrap_pin,
        "all_v5_roles_authenticated_before_execution": True,
        "authenticated_buffer_compile_exec_only": True,
        "source_file_loader_or_pyc_used_for_v5": False,
        ...
        "external_launcher_and_host_independent_pin_required": True,
        "child_self_authenticates_prehook_execution": False,
        "write_process_network_registry_attempts": _operational_firewall()[
            "write_process_network_registry_attempts"
        ],
        "forbidden_import_attempts": _operational_firewall()["forbidden_import_attempts"],
        "unexpected_import_attempts": _operational_firewall()["unexpected_import_attempts"],
        "bytecode_read_attempts": _operational_firewall()["bytecode_read_attempts"],
        "native_capability_surface": _capability_report(),
    }
