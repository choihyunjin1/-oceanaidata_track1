"""Independent QA for sealed P2 v47 pooled-profile/context CrossNet run."""
...
def _load_truth(
    ...
    if np.any(positions < 0):
        raise RuntimeError("v47 independent truth alignment failed")
...
def _fit_receipts(
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            ...
            receipts_ok &= fit["parameters"] == 5015
    return {
...
def main() -> None:
    ...
    contract = runner._cross_contract_receipt()
    ...
    checks = {
        ...
        **fit_checks,
        "new_cross_axis": semantic["classification"]
        == "NEW_P2_IDENTITY_INITIALIZED_POOLED_PROFILE_CONTEXT_CROSS_NETWORK",
        ...
        "cross_contract": contract["cross_layer_count"] == 1
        and contract["cross_input_width"] == 75
        and contract["cross_weight_initial_maximum_abs"] == 0.0
        and contract["cross_bias_initial_maximum_abs"] == 0.0
        and contract["parameters"] == 5015
        and contract["parameter_tensors"] == 12
        ...
        and contract["identity_equation_maximum_abs_error"] == 0.0
        and contract["learned_cross_changes_function_maximum_abs"] > 0.0
        and contract["nonzero_equation_maximum_abs_error"] <= 1e-6
        and contract["batch_composition_maximum_abs_error"] <= 1e-6
        and contract["gradients_finite"]
        and contract["cross_weight_gradient_finite_nonzero"]
        and contract["cross_bias_gradient_finite_nonzero"]
    ...
    qa = {
        "schema_version": (
            "p2.pooled_context_cross_network_deepset.independent_qa.20260901.v47"
        ),
