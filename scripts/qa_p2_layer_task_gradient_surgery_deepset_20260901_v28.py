"""Independent QA for sealed P2 v28 target-layer PCGrad run."""
...
def main() -> None:
    ...
    terminal_path = runner.REPORT / "terminal-amendment.json"
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        group_mass_ok &= receipt["weight_receipt"]["pcgrad_task_labels"] == [2, 3, 4]
        for fit in receipt["fit_receipts"]:
            values = [
                fit["loss_first"],
                fit["loss_last"],
                fit["negative_dot_projection_share"],
                fit["projection_delta_l2_mean_per_batch"],
                fit["projection_delta_l2_max"],
            ]
            fit_receipts_ok &= bool(np.isfinite(values).all())
            fit_receipts_ok &= fit["epochs"] == 60
            fit_receipts_ok &= fit["task_labels"] == [2, 3, 4]
            fit_receipts_ok &= fit["task_order"] == [2, 3, 4]
            fit_receipts_ok &= fit["directional_task_pairs"] == fit["batches"] * 6
            fit_receipts_ok &= fit["missing_task_batches"] == 0
            fit_receipts_ok &= fit["input_gradient_penalty"] == 0
            fit_receipts_ok &= fit["parameter_neighborhood_perturbation"] == 0
            fit_receipts_ok &= fit["data_augmentation"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True
            pcgrad_active &= fit["negative_dot_projections"] > 0
    ...
    projection = runner._projection_contract_receipt()
    isolation = runner._isolation_receipt()
    amendment_path = ROOT / config["authorization_evidence"][
        "prospective_gate_amendment"
    ]
    checks = {
        "terminal": terminal["status"].startswith("EXPLORATORY_"),
        "post_metric_failure_preserved_without_rerun": (
            terminal["technical_event"]["classification"]
            == "POST_METRIC_REPORT_ORDER_FAILURE"
            and terminal["technical_event"]["same_id_rerun"] is False
            and terminal["technical_event"]["metrics_retrained"] is False
            and terminal["technical_event"]["artifact_result_modified"] is False
        ),
        "exactly_nine_fits": result["fit_count"] == 9,
        "new_target_layer_pcgrad_axis": semantic["classification"]
        == "NEW_P2_FIXED_TARGET_LAYER_TASK_PCGRAD_OPTIMIZATION",
        ...
        "semantic_distinctions_all_true": all(
            ...
            for name in (
                ...
                "v26_mixup_distinguished",
            )
        ),
        "projection_formula_and_order": (
            projection["fixed_task_order"] == [2, 3, 4]
            and projection["opposite_gradient_projection_count"] == 2
            and projection["opposite_gradient_residual_l2_max"] <= 1e-12
            and projection["zero_conflict_exact_noop"]
            and projection["byte_identical_replay"]
        ),
        ...
        "pcgrad_actually_used": pcgrad_active,
        ...
        == terminal["hashes"]["v13_runner"],
        "gate_amendment_hash": runner.v12.sha256_file(amendment_path)
        == terminal["hashes"]["prospective_gate_amendment"],
        ...
        "prospective_fold_layer_gate": metric_tree_close(
            independent_local_gate,
            terminal["candidate"]["prospective_fold_layer_gate"],
        ),
        "effective_safety_pass": terminal["candidate"][
    ...
    qa = {
        "schema_version": "p2.layer_task_gradient_surgery_deepset.independent_qa.20260901.v28",
        ...
        "hashes": {
            ...
            "terminal_amendment": runner.v12.sha256_file(terminal_path),
            ...
            "prospective_gate_amendment": runner.v12.sha256_file(amendment_path),
        },
    }
