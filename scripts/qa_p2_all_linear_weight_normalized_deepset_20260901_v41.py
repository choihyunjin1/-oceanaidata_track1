"""Independent QA for sealed P2 v41 all-Linear WeightNorm run."""
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            ...
            fit_receipts_ok &= fit["parameters"] == 4994
            fit_receipts_ok &= fit["parameter_tensors"] == 15
            fit_receipts_ok &= fit["loss"] == config["training"]["objective"]
            fit_receipts_ok &= fit["weight_norm_module_count"] == 5
            fit_receipts_ok &= fit["linear_names"] == [
                "element.0",
                "element.2",
                "head.0",
                "head.2",
            ...
            fit_receipts_ok &= fit["spectral_buffer_count"] == 0
            fit_receipts_ok &= fit["batch_stat_buffer_count"] == 0
            fit_receipts_ok &= fit["bias_parametrized"] is False
            fit_receipts_ok &= len(fit["initial_parametrization_state_sha256"]) == 64
            fit_receipts_ok &= len(fit["final_parametrization_state_sha256"]) == 64
            state_hashes_unique &= (
                fit["initial_parametrization_state_sha256"]
                != fit["final_parametrization_state_sha256"]
            )
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True

    semantic = runner.semantic_audit(config)
    norm = runner._weight_norm_contract_receipt()
    ...
    checks = {
        ...
        "new_p2_weight_norm_axis": semantic["classification"]
        == "NEW_P2_ALL_LINEAR_PER_OUTPUT_WEIGHT_NORMALIZATION",
        "p2_execution_hits_zero": semantic["repository_p2_exact_execution_hits"]
        == 0,
        "spectral_family_not_reopened": semantic[
            "v27_spectral_family_not_reopened"
        ]
        and semantic["v27_code_import_count"] == 0
        and semantic["spectral_tolerance_changes"] == 0,
        "semantic_distinctions": all(
            semantic[name]
            for name in (
                "v29_lookahead_distinguished",
                "v34_gradient_centralization_distinguished",
                "v35_radam_distinguished",
        ...
        "initial_function_preserved": norm["initial_function_preserved"]
        and norm["initial_function_maximum_abs_error"] <= 1e-6,
        "exact_five_weight_norms": norm["linear_module_count"] == 5
        and norm["linear_names_exact"]
        and norm["parametrization_class_names"] == ["_WeightNorm"] * 5,
        "magnitude_direction_contract": norm["maximum_initial_magnitude_error"]
        <= 1e-7
        and norm["maximum_unit_direction_norm_error"] <= 1e-6
        and norm["maximum_effective_magnitude_error"] <= 1e-6,
        "no_spectral_or_batch_state": not norm["buffer_names"]
        and not norm["spectral_or_power_iteration_state"]
        and not norm["bias_parametrized"],
        "deterministic_inference": norm["deterministic_inference"],
        "masked_future_isolation": isolation["masked_token_maximum_abs_error"]
        <= 1e-6,
        "permutation_invariance": isolation["permutation_maximum_abs_error"]
        <= 1e-6,
        "repeat_invariance": isolation["repeat_maximum_abs_error"] <= 1e-6,
        "fit_receipts": fit_receipts_ok,
        "state_hashes_changed_by_training": state_hashes_unique,
        ...
        "official_access_zero": all(
            int(counters[name]) == 0 for name in official_names
        ),
    }
    checks = {name: bool(value) for name, value in checks.items()}
    qa = {
        "schema_version": "p2.all_linear_weight_normalized_deepset.independent_qa.20260901.v41",
