"""Independent QA for sealed P2 v38 output spectral-decoupling run."""
...
def main() -> None:
    ...
    for fold_name, receipt in result["training"]["folds"].items():
        ...
        for fit in receipt["fit_receipts"]:
            values = [
                fit["base_loss_first"],
                fit["base_loss_last"],
                fit["output_penalty_first"],
                fit["output_penalty_last"],
            ...
            fit_receipts_ok &= fit["output_penalty_steps"] == fit["optimizer_steps"]
            fit_receipts_ok &= fit["coefficient"] == 0.01
            fit_receipts_ok &= fit["target_dependent_penalty_terms"] == 0
            fit_receipts_ok &= fit["parameter_norm_steps"] == 0
            fit_receipts_ok &= fit["input_gradient_steps"] == 0
            fit_receipts_ok &= fit["environment_alignment_steps"] == 0
            fit_receipts_ok &= fit["row_deletion"] == 0
            fit_receipts_ok &= fit["loss_finite"] is True

    semantic = runner.semantic_audit(config)
    penalty_receipt = runner._penalty_contract_receipt()
    ...
    checks = {
        ...
        "new_p2_output_axis": semantic["classification"]
        == "NEW_P2_NORMALIZED_RESIDUAL_OUTPUT_SPECTRAL_DECOUPLING",
        "p2_execution_hits_zero": semantic["repository_p2_exact_execution_hits"] == 0,
        "cross_problem_adjacency_disclosed": semantic[
            "p1_v46_cross_problem_adjacency_disclosed"
        ],
        "semantic_distinctions": all(
            semantic[name]
            for name in (
                "v21_likelihood_distinguished",
                "v23_input_gradient_distinguished",
                "v27_parameter_spectral_norm_distinguished",
                "v32_weighted_mse_distinguished",
                "v37_latent_cmd_distinguished",
            )
        ),
        "official_feedback_selection_zero": not semantic[
            "official_v23_feedback_used_for_selection"
        ],
        "penalty_contract": penalty_receipt["formula_exact"]
        and penalty_receipt["gradient_exact"]
        and penalty_receipt["zero_output_noop"]
        and penalty_receipt["target_values_used"] == 0,
    ...
    qa = {
        "schema_version": "p2.spectral_decoupled_output_regularized_deepset.independent_qa.20260901.v38",
