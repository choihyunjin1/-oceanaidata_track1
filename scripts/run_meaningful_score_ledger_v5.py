"""Canonical read/replay/append CLI for the binary-safe v5 score ledger."""
...
from ocean_goal.meaningful_score_ledger_v5 import (
    CONTRACT_RELATIVE,
    CONTRACT_SHA256,
    LEDGER_RELATIVE,
...
def check(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "failed_v4_lineage_pins": failed_v4,
        ...
        "pre_init_qa_schema": "meaningful_score_ledger_v5.pre_init_qa.v1",
        "pre_init_qa_required_decision": "GO_INITIALIZE_V5_LEDGER",
...
def initialize(root: Path, *, append: bool, qa_receipt: Path | None) -> dict[str, Any]:
    ...
    if report["ledger_exists"]:
        if append:
            raise ContractError("v5 ledger is already initialized; second init forbidden")
    ...
    if qa_receipt is None:
        raise ContractError("canonical independent v5 pre-init QA receipt is required")
