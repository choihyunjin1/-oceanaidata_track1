...
This source is never imported through the ambient import system.  The v5
...
outside Python in the independently pinned v5 launcher.
"""
...
try:
    _BOOTSTRAP = _P2_V5_BOOTSTRAP_CONTEXT  # type: ignore[name-defined]  # noqa: F821
except NameError as exc:  # pragma: no cover - subprocess contract test
    raise RuntimeError("P2 v5 helper requires the authenticated bootstrap") from exc


IDENTITY = "P2_JOINT_HYDROGRAPHIC_MULTITASK_LAYER4_R3_COMPATIBILITY_VERIFIER_V5"
R3_MODULE = "p2_restore.joint_hydrographic_multitask_layer4_contract_r3_v5_authenticated"
...
class CompatibilityVerifierV5Error(RuntimeError):
    """The v5 trust, stable-byte, or compatibility contract failed closed."""
...
def _pin_key(pin: Mapping[str, Any]) -> tuple[str, int, str]:
    if set(pin) != {"path", "bytes", "sha256"}:
        raise CompatibilityVerifierV5Error("pin field set changed")
    path = pin.get("path")
    size = pin.get("bytes")
    ...
    if (
        ...
        raise CompatibilityVerifierV5Error("pin value changed")
...
def _register_pin(
    ...
    *,
    ...
    if prior is not None and prior != exact:
        raise CompatibilityVerifierV5Error(f"conflicting pin for {label}: {path}")
    registry[path] = exact
...
def _validate_config() -> dict[str, Any]:
    ...
    if (
        ...
        != "p2_joint_hydrographic_multitask_layer4.r3_compatibility_verifier.v5"
        or config.get("identity") != IDENTITY
        or config.get("verifier_only") is not True
        or config.get("check_only_default") is not True
        or config.get("append_only_successor_of_v4") is not True
        ...
        != {
            "CONFIG": (
                "configs/experiments/"
                "p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5.json"
            ),
            "HELPER": (
                "src/p2_restore/joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5.py"
            ),
            "CLI": ("scripts/verify_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5.py"),
            "TESTS": (
                "tests/test_p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v5.py"
            ),
        }
        or config.get("canonical_bootstrap")
        != "scripts/bootstrap_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5.py"
        or config.get("canonical_external_launcher")
        != "scripts/launch_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5.ps1"
        or config.get("canonical_pycache_sentinel")
        != {
            "path": (
                "configs/experiments/p2_joint_hydrographic_multitask_layer4_r3_"
                "compatibility_verifier_v5.pycache_sentinel"
            ),
            "bytes": 67,
            "sha256": "932aa01b4026ed06cc675d1312fdf5f29f2247327319c3eaceec24fe7991f903",
            "kind": "PINNED_HELD_REGULAR_FILE",
            "nlink": 1,
        }
        or config.get("canonical_inline_stage_zero_source")
        != "scripts/launch_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v5_stage0.ps1"
    ):
        raise CompatibilityVerifierV5Error("v5 static identity or prohibition changed")
    ...
    if (
        ...
        != [
            ...
            "-S",
            "-B",
            "-X pycache_prefix=<canonical-held-regular-file-sentinel>",
        ]
        ...
        or runtime.get("canonical_pycache_sentinel_must_be_regular_held_and_pinned")
        is not True
        or runtime.get("external_launcher_execution")
        != "AUTHENTICATED_UTF8_BYTES_EVALUATED_IN_MEMORY_BY_PATH_IMMUTABLE_ENCODED_STAGE_ZERO"
        or runtime.get("inline_stage_zero_path_execution_allowed") is not False
        or runtime.get("stage_zero_holds_stage_one_until_child_exit") is not True
        ...
        or runtime.get("successful_write_process_network_registry_actions_required") != 0
        or runtime.get("retained_original_winapi_callable_globals_allowed") is not False
        or runtime.get("powershell_runtime_inventory")
        != {
            "root_relation": "DIRECTORY_CONTAINING_EXACT_P2_POWERSHELL_HOST",
            "directories": 53,
            "files": 983,
            ...
            "sha256": "9a197570fffc3399d9c8477ef0199e31ad950701de7b133df7c4669d42099be1",
            "external_parent_must_pin_and_hold_before_host_start": True,
            "stage_zero_same_handle_rehash_required": True,
        }
        ...
        raise CompatibilityVerifierV5Error("v5 runtime or stable-read contract changed")
    if _BOOTSTRAP["observed_implementation_pins"] != _BOOTSTRAP["trusted_pins"]:
        raise CompatibilityVerifierV5Error("v5 implementation pin set changed")
    if runtime["external_startup_file_pins"] != _BOOTSTRAP["startup_pins_public"]:
        raise CompatibilityVerifierV5Error("external startup pin set changed")
    return dict(config)
...
def _verify_pin_map(expected: Mapping[str, Any], *, label: str) -> dict[str, Any]:
    ...
    if observed != dict(expected):
        raise CompatibilityVerifierV5Error(f"{label} pin map changed")
    return observed


def _verify_v4_disposition(config: Mapping[str, Any]) -> dict[str, Any]:
    pins = config["v4_disposition_pins"]
    receipt = _stable_json(pins["OWNER_NO_GO"], label="v4 owner NO-GO")
    tombstone = _stable_json(pins["EXECUTION_TOMBSTONE"], label="v4 tombstone")
    expected = config["expected_v4_no_go"]
    codes = [item.get("code") for item in receipt.get("findings", [])]
    if (
        receipt.get("verdict") != expected["verdict"]
        or receipt.get("p0_count") != 0
        or receipt.get("p1_count") != 3
        ...
        or receipt.get("reviewed_v4_pins") != config["v4_implementation_pins"]
        or codes != expected["required_finding_codes"]
        or tombstone.get("owner_no_go_receipt") != pins["OWNER_NO_GO"]
        or tombstone.get("compatibility_must_fail_closed") is not True
        or tombstone.get("execution_prohibited") is not True
        or tombstone.get("independent_qa_receipt_created") is not False
        or tombstone.get("execution_authorization_created") is not False
        or tombstone.get("attempt_lock_created") is not False
    ):
        raise CompatibilityVerifierV5Error("v4 independent NO-GO lineage changed")
    return {
...
def _verify_v3_disposition(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise CompatibilityVerifierV5Error("v3 independent NO-GO lineage changed")
    ...
    if (
        ...
        raise CompatibilityVerifierV5Error("v2/v1 lineage inside v3 disposition changed")
    return {
        "verdict": expected["verdict"],
        "finding_codes": codes,
        "owner_no_go": dict(pins["OWNER_NO_GO"]),
...
def _verify_v2_disposition(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise CompatibilityVerifierV5Error("v2 owner NO-GO lineage changed")
    ...
    if (
        ...
        raise CompatibilityVerifierV5Error("v1 P1=3 lineage changed")
    return {
        "verdict": expected["verdict"],
        "finding_codes": codes,
        "owner_no_go": dict(pins["OWNER_NO_GO"]),
...
def _build_semantic_registry(
    ...
    _collect_pins(config["v4_implementation_pins"], pins, label="v4 implementation")
    _collect_pins(config["v4_disposition_pins"], pins, label="v4 disposition")
    ...
    if v1.get("identity") != "P2_JOINT_HYDROGRAPHIC_MULTITASK_LAYER4_R3_COMPATIBILITY_VERIFIER_V1":
        raise CompatibilityVerifierV5Error("frozen v1 identity changed")
    ...
    if not isinstance(artifacts, Mapping):
        raise CompatibilityVerifierV5Error("r3 manifest artifact map changed")
    for relative, pin in artifacts.items():
        if not isinstance(relative, str) or not isinstance(pin, Mapping):
            raise CompatibilityVerifierV5Error("r3 manifest artifact pin changed")
        full = dict(pin)
        if full.get("path") != relative:
            raise CompatibilityVerifierV5Error("manifest pin path changed")
...
def _seal_trees(
    ...
    if output_inventory != v1["r3_output_inventory"]:
        raise CompatibilityVerifierV5Error("r3 output inventory changed")
    if control_inventory != v1["r3_control_inventory"]:
        raise CompatibilityVerifierV5Error("r3 control inventory changed")
    v1_control_root = (
    ...
    v4_inventory = stable.seal_tree(
        "artifacts/p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v4_control",
        pins,
    )
    if (
        ...
        or v4_inventory["directories"] != 0
        or v4_inventory["files"] != 2
    ):
        raise CompatibilityVerifierV5Error("compatibility disposition tree changed")
    v5_control = config["canonical_paths"]["v5_control"]
    if _BOOTSTRAP["path_exists"](v5_control):
        raise CompatibilityVerifierV5Error("v5 control must remain absent before fresh QA")
    return {
        ...
        "v4_disposition": v4_inventory,
        "v5_control_exists": False,
    }
...
def _load_r3_guard(pin: Mapping[str, Any]) -> Any:
    ...
    if R3_MODULE in sys.modules:
        raise CompatibilityVerifierV5Error("authenticated r3 module already loaded")
    ...
    if _loaded_forbidden():
        raise CompatibilityVerifierV5Error("r3 guard imported forbidden code")
    return module
...
def _verify_fold_audits(
    ...
    if set(fold_audits) != set(order) or set(commitment_audits) != set(order):
        raise CompatibilityVerifierV5Error("fold audit exact key set changed")
    ...
    for index, name in enumerate(order):
        ...
        if not isinstance(audit, Mapping) or dict(audit) != dict(committed):
            raise CompatibilityVerifierV5Error(f"fold audit differs from commitment: {name}")
        ...
        if (
            ...
            raise CompatibilityVerifierV5Error(f"fold audit identity changed: {name}")
        for field in forbidden:
            value = audit.get(field)
            if not isinstance(value, int) or isinstance(value, bool) or value != 0:
                raise CompatibilityVerifierV5Error(
...
def _verify_v9(anchor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(records) != anchor["record_count"] or sequences != anchor["sequences"]:
        raise CompatibilityVerifierV5Error("v9 sequence changed")
    for index, record in enumerate(records):
        ...
        if hashlib.sha256(_canonical_json_bytes(unsigned)).hexdigest() != claimed:
            raise CompatibilityVerifierV5Error("v9 event hash changed")
        if index and record.get("previous_event_sha256") != records[index - 1]["event_sha256"]:
            raise CompatibilityVerifierV5Error("v9 event chain changed")
    head = records[-1]
    uploads = sum(
    ...
    if (
        ...
        raise CompatibilityVerifierV5Error("v9 head or upload state changed")
    return {
        "pin": pin,
        "sequences": sequences,
...
def _run_original_verifier(
    ...
    try:
        ...
        if loaded != r3_config:
            raise CompatibilityVerifierV5Error("r3 config deep value changed")
        try:
        ...
        if any(getattr(r3, name) is not original for name, original in originals.items()):
            raise CompatibilityVerifierV5Error("r3 stable-byte adapters were not restored")
    return {
...
def _verify_result(v1: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, bool]]:
    ...
    if list(receipt.get("fold_blind_input_audits", {})) != contract["persisted_mapping_order"]:
        raise CompatibilityVerifierV5Error("persisted fold audit order changed")
    ...
    if not all(checks.values()):
        ...
        raise CompatibilityVerifierV5Error(f"research-only result changed: {failed}")
...
def verify_static_compatibility() -> dict[str, Any]:
    ...
    v4_disposition = _verify_v4_disposition(config)
    ...
    v4_implementation = _verify_pin_map(config["v4_implementation_pins"], label="v4 implementation")
    ...
    native_capability_surface = _BOOTSTRAP["seal_native_capabilities"]()
    ...
    if trees_after != trees_before:
        raise CompatibilityVerifierV5Error("protected tree changed during verification")
    final_registry = _BOOTSTRAP["stable_registry"].final_reverify()
    _BOOTSTRAP["assert_runtime"]()
    if _loaded_forbidden():
        raise CompatibilityVerifierV5Error("forbidden module appeared during verification")
    counters = dict(config["static_counters"])
    report = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.r3_compatibility_check.v5",
        "status": (
            "PASS_OWNER_STATIC_EXTERNAL_STARTUP_ROOT_STABLE_R3_COMPATIBILITY_"
            "AWAITING_FRESH_INDEPENDENT_QA_RESEARCH_ONLY_LOCAL_FAIL"
        ),
        "identity": IDENTITY,
        "v5_implementation_pins": dict(_BOOTSTRAP["trusted_pins"]),
        ...
        "v4_implementation_pins": v4_implementation,
        ...
        "native_capability_surface": native_capability_surface,
        "v5_control_exists": False,
