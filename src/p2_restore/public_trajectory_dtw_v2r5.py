"""Sparse-key integrity recovery for deterministic P2 trajectory transfer v2r5.
...
def exact_time_contract(values: Sequence[Any] | pd.Series | pd.Index) -> dict[str, Any]:
    """Validate sparse observed timestamps against the frozen canonical grid.

    The exact anchor contains only keys with independently observed truth, so
    absent timestamps are legitimate.  Every *present* timestamp must still be
    an exact ten-minute point on the registered 61-day grid.
    """
    ...
    expected_start_ns = int(expected_start.tz_convert("UTC").as_unit("ns").value)
    expected_end_ns = int(expected_end.tz_convert("UTC").as_unit("ns").value)
    offsets = unique - expected_start_ns
    off_grid_count = int(
        np.count_nonzero(
            (unique < expected_start_ns)
            | (unique > expected_end_ns)
            | (offsets % expected_step != 0)
        )
    )
    if off_grid_count:
        raise ValueError("exact timestamp is off the frozen 10-minute UTC-ns grid")
    full_grid_timestamps = int(
        (expected_end_ns - expected_start_ns) // expected_step + 1
    )
    if len(unique) > full_grid_timestamps:
        raise ValueError("exact timestamp count exceeds the frozen grid")
    diffs = np.diff(unique)
    gap_intervals = int(np.count_nonzero(diffs != expected_step))
    return {
        "minimum_kst": kst.min().isoformat(),
        "maximum_kst": kst.max().isoformat(),
        "kst_days": day_count,
        "unique_timestamps": int(len(unique)),
        "full_grid_timestamps": full_grid_timestamps,
        "missing_union_timestamps": int(full_grid_timestamps - len(unique)),
        "gap_intervals": gap_intervals,
        "off_grid_timestamps": off_grid_count,
        "canonical_step_ns": expected_step,
...
def exact_sparse_key_contract(
    frame: pd.DataFrame,
    *,
    expected_rows: int = 26273,
) -> dict[str, Any]:
    """Validate exact sparse ``(time, layer)`` keys before any scoring."""

    if not {"time", "layer"}.issubset(frame.columns) or len(frame) != expected_rows:
        raise ValueError("exact sparse key schema or registered row count differs")
    keys = frame[["time", "layer"]].copy()
    keys["_time_ns"] = normalize_utc_ns(keys["time"])
    keys["layer"] = pd.to_numeric(keys["layer"], errors="raise").astype("int16")
    duplicate_count = int(keys.duplicated(["_time_ns", "layer"]).sum())
    if duplicate_count:
        raise ValueError("exact sparse (time, layer) key is duplicated")
    layers = tuple(sorted(int(value) for value in keys["layer"].unique()))
    if layers != TARGET_LAYERS:
        raise ValueError("exact sparse layer set differs from registration")
    time_contract = exact_time_contract(keys["time"])
    full_grid = int(time_contract["full_grid_timestamps"])
    layer_rows = {
        str(layer): int((keys["layer"] == layer).sum()) for layer in TARGET_LAYERS
    }
    return {
        **time_contract,
        "rows": int(len(keys)),
        "unique_time_layer_keys": int(len(keys)),
        "duplicate_time_layer_keys": duplicate_count,
        "layers": list(layers),
        "layer_rows": layer_rows,
        "layer_missing_from_full_grid": {
            layer: int(full_grid - count) for layer, count in layer_rows.items()
        },
    }
...
def verify_anchor_against_observations(
    ...
    *,
    ...
    time_contract = (
        exact_sparse_key_contract(trusted, expected_rows=expected_rows)
