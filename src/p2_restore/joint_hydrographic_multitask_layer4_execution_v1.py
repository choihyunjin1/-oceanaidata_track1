...
def build_execution_plan(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.execution_plan.v1",
        "stage": guard.STAGE,
        "problem": "P2",
...
def _predict_panel_temperature(
    ...
    *,
    ...
    if physical.shape != (len(panel.times), 3) or not np.isfinite(physical).all():
        raise ValueError("Layer-4 physical prediction is invalid")
    return physical
...
def _fit_predict_cell(
    ...
    *,
    ...
    temperature = _predict_panel_temperature(
        model,
        panel,
        normalizer,
        config=config,
    ...
    torch.save(
        {
            "schema_version": "p2_joint_hydrographic_multitask_layer4.model_bundle.v1",
            "fold": fold_name,
            "fraction": float(fraction),
            "seed": int(seed),
...
def _evaluate_after_commitment(
    *,
    ...
    bootstrap_receipt = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.bootstrap_receipt.v1",
    ...
    evidence = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.learning_curve_evidence.v1",
        "problem": "P2",
        "comparison_mode": guard.MODE,
        "exact_official_incumbent_comparison": False,
    ...
    decision = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.gate_decision.v1",
        "status": (
