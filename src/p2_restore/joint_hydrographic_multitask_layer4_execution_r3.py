...
def build_execution_plan(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.execution_plan.r3",
        "stage": guard.STAGE,
        "problem": "P2",
        ...
        "prior_r2_optimizer_steps_not_reused": 392,
        "fresh_r3_optimizer_steps": derived_steps,
        "full_fit_jobs": 0,
...
def _validate_assembled_layer4_prediction(
    prediction: Any,
    reference: Any,
    layer4: Any,
    *,
    ...
    """Enforce exact preserved values and apply physical bounds only to modified Layer 4."""

    np = np_module
    values = np.asarray(prediction, dtype=np.float64)
    baseline = np.asarray(reference, dtype=np.float64)
    mask = np.asarray(layer4)
    bounds = np.asarray(clip_bounds, dtype=np.float64)
    if (
        values.ndim != 1
        ...
        raise ValueError("assembled Layer-4 prediction contract is invalid")
    preserved = values[~mask]
    preserved_baseline = baseline[~mask]
    if (
        not np.isfinite(preserved).all()
        or not np.isfinite(preserved_baseline).all()
        or not np.array_equal(preserved, preserved_baseline)
        or preserved.astype("<f8", copy=False).tobytes(order="C")
        != preserved_baseline.astype("<f8", copy=False).tobytes(order="C")
    ):
        raise AssertionError("sealed Stage-A Layer-2/3 values changed or became non-finite")
    modified = values[mask]
    if (
        not np.isfinite(modified).all()
        or (modified < bounds[0]).any()
        or (modified > bounds[1]).any()
    ):
        raise AssertionError("modified Layer-4 prediction is non-finite or outside physical bounds")
    return values
...
def _fit_predict_cell(
    ...
    *,
    ...
    clip_bounds = tuple(float(value) for value in recipe["layer4_clip_c"])
    prediction = reference.copy()
    prediction[layer4] = temperature[positions[layer4], 2]
    prediction[layer4] = _csv_float_roundtrip(
        np.clip(prediction[layer4], clip_bounds[0], clip_bounds[1]),
        pd_module=pd,
        np_module=np,
    )
    prediction = _validate_assembled_layer4_prediction(
        prediction,
        reference,
        layer4,
    ...
    torch.save(
        {
            "schema_version": "p2_joint_hydrographic_multitask_layer4.model_bundle.r3",
            "fold": fold_name,
            "fraction": float(fraction),
            "seed": int(seed),
    ...
    receipt = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.cell_receipt.r3",
        "fold": fold_name,
        "fraction": float(fraction),
        "seed": int(seed),
...
def _fold_commitment_payload(
    *,
    ...
    return {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.fold_commitment.r3",
...
def _aggregate_commitment_payload(
    *,
    ...
    return {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.prediction_commitment.r3",
...
def _evaluate_after_commitment(
    *,
    ...
    metrics = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.metrics.r3",
        "comparison_mode": guard.MODE,
        "exact_official_incumbent_comparison": False,
        "points": points,
    ...
    bootstrap_receipt = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.bootstrap_receipt.r3",
    ...
    evidence = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.learning_curve_evidence.r3",
        "problem": "P2",
        "comparison_mode": guard.MODE,
        "exact_official_incumbent_comparison": False,
    ...
    decision = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.gate_decision.r3",
        "status": (
...
def execute_layer4_curve(
    *,
    ...
    try:
        ...
        training_receipt = {
            "schema_version": "p2_joint_hydrographic_multitask_layer4.training_receipt.r3",
            "started_at_kst": started,
            "completed_at_kst": _now_kst(),
            "config": {"path": guard.CONFIG_RELATIVE, "sha256": guard.CONFIG_SHA256},
            "failure_evidence_pins": fresh_preflight["operational_snapshot"][
        ...
        manifest = {
            "schema_version": "p2_joint_hydrographic_multitask_layer4.manifest.r3",
            ...
            "failure_evidence_pins": end_preflight["operational_snapshot"][
                "failure_evidence_pins"
            ],
            "implementation_correction": dict(canonical["implementation_correction"]),
        ...
        seal = {
            "schema_version": "p2_joint_hydrographic_multitask_layer4.seal.r3",
            "complete": True,
        ...
        return {
            "schema_version": "p2_joint_hydrographic_multitask_layer4.execution_result.r3",
            "status": decision["status"],
            "local_qualification": bool(decision["local_qualification"]),
            "official_promotion_allowed": False,
