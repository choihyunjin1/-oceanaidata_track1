"""Corrected deterministic zero-fit public-trajectory transfer for P2 v2.
...
def verify_candidate_against_truth(
    ...
    left_digest = key_digest(trusted_anchor)
    right_digest = key_digest(candidate_work)
    ...
    result = result.sort_values(["time", "layer"], kind="mergesort").reset_index(drop=True)
...
def validate_source_before_query(
    ...
    validate_source_end_before_query_start(
        int(source_ns.max()) + step_ns,
        int(query_ns.min()),
    )


def validate_source_end_before_query_start(
    source_end_exclusive: Any,
    query_start: Any,
) -> dict[str, int]:
    """Prove the registered seven-day embargo against query *start*.
    ...
    query_start_ns = one_ns(query_start)
    embargo_ns = SOURCE_EMBARGO_DAYS * 86_400 * 1_000_000_000
    observed = query_start_ns - source_end_ns
    ...
    return {
        ...
        "query_start_utc_ns": query_start_ns,
        "embargo_ns": observed,
...
class HistoricalTrajectoryMaterializer:
    ...
    def load_p100_anchor(self, p100_anchor: pd.DataFrame) -> dict[str, Any]:
        ...
        verified, certificate = verify_anchor_against_observations(
            self._observations,
            p100_anchor,
            anchor_prediction_column="INCUMBENT_NOOP",
            expected_rows=78156,
        )
        fold_keys = p100_anchor[["time", "layer", "fold", "station"]].copy()
        merged = align_on_utc_ns(verified, fold_keys, key_columns=("layer",))
        verified["fold"] = merged["fold"].astype(str).to_numpy()
        verified["station"] = merged["station"].astype(str).to_numpy()
    ...
    def _daily_correction(
        ...
        query_end = day_start_kst.tz_convert("UTC")
        query_start = query_end - pd.Timedelta(days=cell.context_days)
        source_limit = query_start - pd.Timedelta(days=SOURCE_EMBARGO_DAYS)
        ...
        for _, _, index in prefilter:
            ...
            validate_source_end_before_query_start(end, query_start)
        ...
        embargo_ns = int(query_start.as_unit("ns").value) - max(selected_end_ns)
        return daily, {
            ...
            "query_start_utc_ns": int(query_start.as_unit("ns").value),
