...
This source is never imported through the ambient import system.  The v3
bootstrap authenticates its exact bytes, compiles that buffer, and injects a
closed trust context before execution.
"""
...
try:
    _BOOTSTRAP = _P2_V3_BOOTSTRAP_CONTEXT  # type: ignore[name-defined]  # noqa: F821
except NameError as exc:  # pragma: no cover - subprocess contract test
    raise RuntimeError("P2 v3 helper requires the authenticated bootstrap") from exc


IDENTITY = "P2_JOINT_HYDROGRAPHIC_MULTITASK_LAYER4_R3_COMPATIBILITY_VERIFIER_V3"
R3_MODULE = "p2_restore.joint_hydrographic_multitask_layer4_contract_r3_v3_authenticated"
...
class CompatibilityVerifierV3Error(RuntimeError):
    """The v3 trust, stable-byte, or compatibility contract failed closed."""
...
def _pin_key(pin: Mapping[str, Any]) -> tuple[str, int, str]:
    if set(pin) != {"path", "bytes", "sha256"}:
        raise CompatibilityVerifierV3Error("pin field set changed")
    path = pin.get("path")
    size = pin.get("bytes")
    ...
    if (
        ...
        raise CompatibilityVerifierV3Error("pin value changed")
...
def _register_pin(
    ...
    *,
    ...
    if prior is not None and prior != exact:
        raise CompatibilityVerifierV3Error(f"conflicting pin for {label}: {path}")
    registry[path] = exact
...
def _validate_config() -> dict[str, Any]:
    ...
    if (
        ...
        != "p2_joint_hydrographic_multitask_layer4.r3_compatibility_verifier.v3"
        ...
        != {
            ...
            "CLI": ("scripts/verify_p2_joint_hydrographic_multitask_layer4_r3_compatibility_v3.py"),
            "TESTS": (
                "tests/test_p2_joint_hydrographic_multitask_layer4_r3_compatibility_verifier_v3.py"
            ),
        }
    ):
        raise CompatibilityVerifierV3Error("v3 static identity or prohibition changed")
    ...
    if (
        ...
        raise CompatibilityVerifierV3Error("v3 runtime or stable-read contract changed")
    if _BOOTSTRAP["observed_implementation_pins"] != _BOOTSTRAP["trusted_pins"]:
        raise CompatibilityVerifierV3Error("v3 implementation pin set changed")
    return dict(config)
...
def _verify_pin_map(expected: Mapping[str, Any], *, label: str) -> dict[str, Any]:
    ...
    if observed != dict(expected):
        raise CompatibilityVerifierV3Error(f"{label} pin map changed")
    return observed
...
def _verify_v2_disposition(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise CompatibilityVerifierV3Error("v2 owner NO-GO lineage changed")
    ...
    if (
        ...
        raise CompatibilityVerifierV3Error("v1 P1=3 lineage changed")
    return {
...
def _build_semantic_registry(
    ...
    if v1.get("identity") != "P2_JOINT_HYDROGRAPHIC_MULTITASK_LAYER4_R3_COMPATIBILITY_VERIFIER_V1":
        raise CompatibilityVerifierV3Error("frozen v1 identity changed")
    ...
    if not isinstance(artifacts, Mapping):
        raise CompatibilityVerifierV3Error("r3 manifest artifact map changed")
    for relative, pin in artifacts.items():
        if not isinstance(relative, str) or not isinstance(pin, Mapping):
            raise CompatibilityVerifierV3Error("r3 manifest artifact pin changed")
        full = dict(pin)
        if full.get("path") != relative:
            raise CompatibilityVerifierV3Error("manifest pin path changed")
...
def _seal_trees(
    ...
    if output_inventory != v1["r3_output_inventory"]:
        raise CompatibilityVerifierV3Error("r3 output inventory changed")
    if control_inventory != v1["r3_control_inventory"]:
        raise CompatibilityVerifierV3Error("r3 control inventory changed")
    v1_control_root = (
    ...
    if (
        ...
        raise CompatibilityVerifierV3Error("compatibility disposition tree changed")
    v3_control = config["canonical_paths"]["v3_control"]
    if _BOOTSTRAP["path_exists"](v3_control):
        raise CompatibilityVerifierV3Error("v3 control must remain absent before fresh QA")
    return {
...
def _load_r3_guard(pin: Mapping[str, Any]) -> Any:
    ...
    if R3_MODULE in sys.modules:
        raise CompatibilityVerifierV3Error("authenticated r3 module already loaded")
    ...
    if _loaded_forbidden():
        raise CompatibilityVerifierV3Error("r3 guard imported forbidden code")
    return module
...
def _verify_fold_audits(
    ...
    if set(fold_audits) != set(order) or set(commitment_audits) != set(order):
        raise CompatibilityVerifierV3Error("fold audit exact key set changed")
    ...
    for index, name in enumerate(order):
        ...
        if not isinstance(audit, Mapping) or dict(audit) != dict(committed):
            raise CompatibilityVerifierV3Error(f"fold audit differs from commitment: {name}")
        ...
        if (
            ...
            raise CompatibilityVerifierV3Error(f"fold audit identity changed: {name}")
        for field in forbidden:
            value = audit.get(field)
            if not isinstance(value, int) or isinstance(value, bool) or value != 0:
                raise CompatibilityVerifierV3Error(
...
def _verify_v9(anchor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(records) != anchor["record_count"] or sequences != anchor["sequences"]:
        raise CompatibilityVerifierV3Error("v9 sequence changed")
    for index, record in enumerate(records):
        ...
        if hashlib.sha256(_canonical_json_bytes(unsigned)).hexdigest() != claimed:
            raise CompatibilityVerifierV3Error("v9 event hash changed")
        if index and record.get("previous_event_sha256") != records[index - 1]["event_sha256"]:
            raise CompatibilityVerifierV3Error("v9 event chain changed")
    head = records[-1]
    uploads = sum(
    ...
    if (
        ...
        raise CompatibilityVerifierV3Error("v9 head or upload state changed")
    return {
        "pin": pin,
        "sequences": sequences,
...
def _run_original_verifier(
    ...
    try:
        ...
        if loaded != r3_config:
            raise CompatibilityVerifierV3Error("r3 config deep value changed")
        try:
        ...
        if any(getattr(r3, name) is not original for name, original in originals.items()):
            raise CompatibilityVerifierV3Error("r3 stable-byte adapters were not restored")
    return {
...
def _verify_result(v1: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, bool]]:
    ...
    if list(receipt.get("fold_blind_input_audits", {})) != contract["persisted_mapping_order"]:
        raise CompatibilityVerifierV3Error("persisted fold audit order changed")
    ...
    if not all(checks.values()):
        ...
        raise CompatibilityVerifierV3Error(f"research-only result changed: {failed}")
...
def verify_static_compatibility() -> dict[str, Any]:
    ...
    disposition = _verify_v2_disposition(config)
    ...
    if trees_after != trees_before:
        raise CompatibilityVerifierV3Error("protected tree changed during verification")
    final_registry = _BOOTSTRAP["stable_registry"].final_reverify()
    _BOOTSTRAP["assert_runtime"]()
    if _loaded_forbidden():
        raise CompatibilityVerifierV3Error("forbidden module appeared during verification")
    counters = dict(config["static_counters"])
    report = {
        "schema_version": "p2_joint_hydrographic_multitask_layer4.r3_compatibility_check.v3",
        "status": "PASS_STABLE_AUTHENTICATED_R3_COMPATIBILITY_RESEARCH_ONLY_LOCAL_FAIL",
        "identity": IDENTITY,
        "v3_implementation_pins": dict(_BOOTSTRAP["trusted_pins"]),
