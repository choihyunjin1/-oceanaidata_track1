"""Fail-closed, offline-only access to the synthetic TabPFN-2.6 weights.
...
MODEL_VERSION = "v2.6"
CLASSIFIER_FILENAME = "tabpfn-v2.6-classifier-v2.6_default.ckpt"
REGRESSOR_FILENAME = "tabpfn-v2.6-regressor-v2.6_default.ckpt"
CLASSIFIER_BYTES = 43_044_699
REGRESSOR_BYTES = 51_576_286
CLASSIFIER_SHA256 = "0578fa56f97e11024e31735aaec2c4e7332584b7730242fbaf6c0bbd0299206a"
REGRESSOR_SHA256 = "c855169116d79d2c957a29107ffc04464555f39aec0eef90c90390f2c2b2008b"
DEFAULT_ROOT = Path("artifacts/tabpfn26")
LICENSE_RECEIPT_ENV = "TABPFN26_LICENSE_RECEIPT_PATH"
CLASSIFIER_PATH_ENV = "TABPFN26_CLASSIFIER_PATH"
REGRESSOR_PATH_ENV = "TABPFN26_REGRESSOR_PATH"


class TabPFN26ContractError(RuntimeError):
    ...
    *, workspace: str | Path, env: dict[str, str] | None = None
    ...
    try:
        ...
        package = {
            ...
            "v2_6_enum_available": ModelVersion.V2_6.value == MODEL_VERSION,
        }
        if package["tabpfn"] != PACKAGE_VERSION:
            blockers.append(
                f"tabpfn package mismatch: expected {PACKAGE_VERSION}, got {package['tabpfn']}"
            )
        if not package["v2_6_enum_available"]:
            blockers.append("installed tabpfn package lacks ModelVersion.V2_6")
    ...
    return {
        "schema_version": "ocean.tabpfn26.offline_preflight.v1",
...
def require_ready(
    *, workspace: str | Path, env: dict[str, str] | None = None
    ...
    if report["status"] != "READY":
        raise TabPFN26ContractError("; ".join(report["blockers"]))
...
def make_classifier(
    ...
    *,
    ...
    if path.name != CLASSIFIER_FILENAME:
        raise TabPFN26ContractError("classifier checkpoint basename changed")
    os.environ["TABPFN_NO_BROWSER"] = "1"
...
def make_regressor(
    ...
    *,
    ...
    if path.name != REGRESSOR_FILENAME:
        raise TabPFN26ContractError("regressor checkpoint basename changed")
    os.environ["TABPFN_NO_BROWSER"] = "1"
