...
runner v6 has authenticated its external authorization, seal, complete
...
def _historical_target_rows(
    ...
    requested = train_target_free.iloc[positions].loc[:, list(KEY_COLUMNS)].copy()
    requested.insert(0, "row_position", np.asarray(positions, dtype=np.int64))
    aligned = requested.merge(
        historical_targets,
        on=["row_position", *KEY_COLUMNS],
        how="left",
        validate="one_to_one",
        sort=False,
    )
    if len(aligned) != len(positions) or aligned[["label", "anomaly_type"]].isna().any().any():
        raise RuntimeError("requested fit/inner target is outside bounded historical view")
    if not np.array_equal(
        aligned["row_position"].to_numpy(dtype=np.int64),
        np.asarray(positions, dtype=np.int64),
    ):
        raise RuntimeError("bounded historical target order changed")
    parsed = pd.to_datetime(aligned["time"], errors="raise", utc=True, format="mixed")
    cutoff = pd.Timestamp(HISTORICAL_TARGET_CUTOFF_INCLUSIVE).tz_convert("UTC")
    if parsed.max() > cutoff:
        raise RuntimeError("protected outer target requested before prediction freeze")
    return aligned
...
def _make_surface(
    ...
    *,
    ...
    target_rows = _historical_target_rows(train, historical_targets, positions)
    truth = target_rows["label"].to_numpy(dtype=np.int8)
    anomaly_type = target_rows["anomaly_type"].reset_index(drop=True)
...
def _fit_anchor_surfaces(
    ...
    train = state["train_target_free"]
    historical_targets = state["historical_targets"]
    bundle = state["bundle"]
    ...
    for window in closure["out_of_sample_anchor_surfaces"]["windows"]:
        ...
        fit_target = _historical_target_rows(
            train,
            historical_targets,
            fit_positions,
        )["label"].to_numpy(dtype=np.int8)
...
def _parse_full_train_targets_after_freeze(state: Mapping[str, Any]) -> pd.DataFrame:
    raw = bytes(state["train_source_opaque_bytes"])
    expected_sha256 = str(state["train_source_sha256"])
    if hashlib.sha256(raw).hexdigest() != expected_sha256:
        raise RuntimeError("held training-source bytes changed before post-freeze target parse")
    columns = [*KEY_COLUMNS, "label", "anomaly_type"]
    targets = pd.read_csv(io.BytesIO(raw), usecols=columns, low_memory=False)
    target_free = state["train_target_free"]
    if len(targets) != len(target_free):
        raise RuntimeError("post-freeze training target row count changed")
    if not targets.loc[:, list(KEY_COLUMNS)].equals(
        target_free.loc[:, list(KEY_COLUMNS)]
    ):
        raise RuntimeError("post-freeze training target keys differ from target-free view")
    if not np.isin(targets["label"].to_numpy(), [0, 1]).all():
        raise RuntimeError("post-freeze training target is not binary")
    return targets


def _left_censor_audit_after_freeze(
    ...
    for fold in folds:
        name = str(fold["name"])
        indices = np.asarray(fold["train_idx"], dtype=np.int64)
        frame = target_free.iloc[indices][["station", "layer", "time"]].copy()
        frame["label"] = full_targets.iloc[indices]["label"].to_numpy(dtype=np.int8)
        frame["_source_row"] = indices
        frame["_parsed"] = pd.to_datetime(
        ...
        labels = first["label"].to_numpy(dtype=np.int8)
        if not np.isin(labels, [0, 1]).all():
            raise RuntimeError(f"non-binary post-freeze left-censor label: {name}")
        counts[name] = int((labels == 1).sum())
    if tuple(counts) != FOLD_ORDER or any(counts.values()):
        raise RuntimeError(f"NO_GO_LEFT_CENSORED_POSITIVE_EVENT_AFTER_FREEZE: {counts}")
    return counts
...
def run_authorized_screen(
    ...
    # These are the first protected outer-target accesses in the numerical
    # module. The all-fold digest was durably journaled by _fit_outer above.
    held_truth = _parse_held_outer_truth_after_freeze(state)
    full_train_targets = _parse_full_train_targets_after_freeze(state)
    left_censor = _left_censor_audit_after_freeze(
        state["train_target_free"],
        full_train_targets,
        state["folds"],
    )
    ...
    metrics["post_freeze_target_firewall"] = {
        "status": "PASS_PROTECTED_TARGETS_FIRST_PARSED_AFTER_OUTER_FREEZE",
        "left_censored_positive_connected_event_count_by_fold": left_censor,
        "full_train_target_rows": len(full_train_targets),
        "outer_truth_rows": len(held_truth),
    }
