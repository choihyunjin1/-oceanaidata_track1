"""Single-use Gen5r4 curve engine; import is side-effect free."""
...
from p3_wave.hierarchical_residual_basis_dense72_contract_r4 import (
    COMPARISON_MODE,
    CONFIG_RELATIVE,
    CONFIG_SHA256,
    FOLD_ORDER,
...
def _run_curve(
    *,
    ...
    raw = preflight["raw"]
    raw_identity = validate_canonical_raw_memmap(
        root,
        raw,
        preflight["input_paths"]["sequence_cache/train_values.npy"],
        config,
    )
    if raw_identity != preflight["operational_snapshot"]["raw"]:
        raise PermissionError("r4 engine raw memmap differs from the locked snapshot")
    ...
    if tuple(fold.name for fold in folds) != FOLD_ORDER or compact.shape != (24_360, 591):
        raise PermissionError("r4 operational fold or compact surface changed")
    ...
    for fold_index, fold in enumerate(folds):
        ...
        for fraction in PREFIX_FRACTIONS:
            ...
            if payload.forbidden_scalar_decodes != 0 or not np.all(payload.current_hs == 0.0):
                raise PermissionError("selective r4 target accessor contract changed")
            ...
            if (
                ...
                raise PermissionError("selective r4 current-hs accessor contract changed")
            ...
            for seed in predecessor["validation"]["seed_replicates"]:
                ...
                if (
                    ...
                    raise RuntimeError(
                        "saved r4 model or raw-delta seal failed exact blind reproduction"
                    )
                ...
                raw_commitment = {
                    "schema_version": "p3_gen5r4_dense72.raw_delta_cell_commitment.r1",
        ...
        if (
            ...
            raise PermissionError("r4 fold did not seal 15 target/current-blind raw cells")
        raw_fold_relative = f"commitments/raw_folds/{fold.name}.json"
        raw_fold_path = _commitment_path(stage, raw_fold_relative, protected)
        raw_fold_payload = {
            "schema_version": "p3_gen5r4_dense72.raw_delta_fold_commitment.r1",
            "fold_index": fold_index,
            "fold": fold.name,
        ...
        for raw_row in raw_records:
            ...
            commitment = {
                "schema_version": "p3_gen5r4_dense72.blind_cell_commitment.r1",
                "fold_index": fold_index,
                "fold": fold.name,
        ...
        if len(fold_rows) != 15:
            raise PermissionError("r4 fold did not seal exactly 15 final blind cells")
        fold_relative = f"commitments/folds/{fold.name}.json"
        fold_path = _commitment_path(stage, fold_relative, protected)
        fold_payload = {
            "schema_version": "p3_gen5r4_dense72.blind_fold_commitment.r1",
            "fold_index": fold_index,
            "fold": fold.name,
    ...
    if len(receipts) != 45 or sum(row["optimizer_steps"] for row in receipts) != expected_steps:
        raise AssertionError("r4 fit-cell or optimizer-step accounting changed")
    ...
    complete_payload = {
        "schema_version": "p3_gen5r4_dense72.predictions_complete.r1",
    ...
    if (
        ...
        raise PermissionError("r4 validation target firewall failed")
    ...
    for fraction in PREFIX_FRACTIONS:
        ...
        for frame in ordered[1:]:
            if not frame[keys].equals(ordered[0][keys]) or not np.array_equal(
                ...
                raise ValueError("r4 seed OOF alignment changed")
...
def execute_curve_stage(
    *,
    ...
    stage = Path(tempfile.mkdtemp(prefix="p3_gen5r4_dense72_", dir=tmp_root))
    authorize_curve_phase(
        capability,
        root=root,
        config=config,
    ...
    reproducibility = {
        ...
        "target_free_pinned_split_no_r4_hs_recompute": True,
    ...
    if input_after != preflight["input_snapshot"]:
        raise RuntimeError("r4 immutable inputs changed during execution")
    metrics = {
        "created_at_kst": _now(),
    ...
    if paths["output"].exists():
        raise FileExistsError("canonical r4 output appeared before atomic move")
    stage.replace(paths["output"])
    return {
        "schema_version": "p3_hierarchical_residual_basis.gen5r4_dense72.run.r1",
        "status": status,
